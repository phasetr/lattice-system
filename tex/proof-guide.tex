...
hard-core constraint imposed; we phrase it on the \emph{ground subspace} at filling $N_e$:
\texttt{tJFillingHardcoreStates} (unit $N_e$-electron states in the no-double-occupancy
subspace $H_{N_e}^{\rm hc}$), \texttt{tJGroundEnergyAtFilling} (their minimum energy), and
\texttt{tJGroundSubmodule} ($H$-eigenspace at the ground energy $\sqcap$ the $N_e$-electron
sector $\sqcap$ $H_{N_e}^{\rm hc}$).  \textbf{Proposition
...
\texttt{IsMaximalSpinMultipletSubmodule}~$N\,(\texttt{tJGroundSubmodule}\dots)\,N_e$ (the shared
predicate reused from Mielke's Theorem 11.13 and the general flat-band Theorem 11.15).  Tasaki's proof is a Perron--Frobenius /
