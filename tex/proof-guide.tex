...
\begin{lemma}[\texttt{gibbsExp\_commute\_self}]
$[e^{-\beta H}, H] = 0$.
\end{lemma}
...
\begin{lemma}[\texttt{gibbsState\_commute\_self}]
...
\texttt{gibbsExp\_commute\_self} via \texttt{Commute.smul\_left}.
\end{proof}
...
\begin{proof}[Proof sketch]
Using $\rho^\dagger = \rho$, $O^\dagger = O$, and the cyclic property
of the trace,
\[
  \overline{\operatorname{Tr}(\rho O)}
  = \operatorname{Tr}((\rho O)^\dagger)
  = \operatorname{Tr}(O^\dagger \rho^\dagger)
  = \operatorname{Tr}(O \rho)
  = \operatorname{Tr}(\rho O).
\]
The Lean proof uses \texttt{Matrix.trace\_conjTranspose},
\texttt{Matrix.conjTranspose\_mul}, the Hermiticity of
$\rho$ (\texttt{gibbsState\_isHermitian}) and of $O$, and
