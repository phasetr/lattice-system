...
\emph{Source file:} \texttt{LatticeSystem/Math/PerronFrobenius.lean}.
...
\textbf{Proof.}
Obtain the non-negative eigenvector $v$ from \texttt{exists\_nonneg\_eigenvec\_max}.
Apply \texttt{pos\_of\_nonneg\_eigenvec} (private lemma): if $v_i = 0$ for some $i$, then
$(Av)_i = 0$, so each $A_{ij}v_j = 0$; irreducibility via
\texttt{isIrreducible\_iff\_exists\_pow\_pos} propagates $v_j = 0$ to every $j$,
contradicting $v \neq 0$.
...
\hline
\texttt{exists\_nonneg\_eigenvec\_max} & sorry & Rayleigh equality step; Issue~\#405 \\
\texttt{exists\_pos\_eigenvec\_max} & proved (uses sorry above) & propagation argument \\
\texttt{pos\_eigenvec\_unique} & proved & $r = \sup_i w_i/v_i$ argument \\
\hline
\end{tabular}
\end{center}
...
\subsection*{TODO: Tasaki §2.5 antiferromagnetic deferred items (issue \#240)}
...
§\ref{subsec:neel-state-of}). The following subitems remain
deferred (large mathematical work, requiring infrastructure beyond
finite-dimensional matrix algebra):

\begin{itemize}
\item Marshall--Lieb--Mattis Theorem 2.2 (uniqueness + sign
