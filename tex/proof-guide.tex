...
feed the result to the block-path bridge.
...
classification.  The next paragraph supplies that classification; PF/min
identification and the boundary move-set choices when a strict local coefficient
vanishes remain separate later steps.
...
\texttt{dressedAxisSwappedAnisotropicHeisenbergSReMatrix\_raiseLowerStepS\_or\_eq\_zero\_of\_magSum\_eq},
\texttt{dressedAxisSwappedAnisotropicHeisenbergSReMatrix\_parityBondStepS\_or\_singleIonStepS\_or\_eq\_zero\_of\_magSum\_add\_two},
and
...
non-negativity and conditional irreducibility layers.  The boundary move-set
variants and the PF/min identification remain later steps.
...
remaining case~(ii) tasks are boundary move sets where a strict local
coefficient vanishes and PF/min identification for the shifted matrix on the two
bare axis-swapped parity blocks.
