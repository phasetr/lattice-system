...
\subsection*{Roadmap to multi-mode CAR}

The single-mode operators above are the building blocks.  The
multi-mode CAR algebra on a finite ordered lattice
$\Lambda = \mathrm{Fin}\,(N+1)$ will be obtained via the Jordan–Wigner
mapping
\[
  c_i \;=\; \Bigl(\prod_{j < i} \sigma^z_j\Bigr)\,\sigma^+_i,
  \qquad
  c_i^\dagger \;=\; \Bigl(\prod_{j < i} \sigma^z_j\Bigr)\,\sigma^-_i,
\]
where the parity string makes the otherwise-bosonic spin
raising / lowering operators anticommute across distinct sites.  Built
entirely from the spin infrastructure in
\texttt{LatticeSystem/Quantum/Pauli.lean} and
\texttt{LatticeSystem/Quantum/SpinHalf.lean}.  This is the next PR's
target.
