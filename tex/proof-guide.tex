...
$\tau=(1+4\nu^2)s$; we render the equivalence via its degeneracy consequence (the
$\mathrm{finrank}$ of the two ground subspaces agree in the limits).  \textbf{Theorem 11.26}
(\texttt{theorem\_11\_26}, documented axiom): for $d=1$, $N_e<K+1=|E|$ odd, in the limit
$t,U\uparrow\infty$ the ground states are
$\texttt{IsMaximalSpinMultipletSubmodule}~(3K+2)\,(\texttt{groundSubmoduleAtFilling}\dots)\,N_e$
--- total spin $S_{\rm tot}=N_e/2$ and $N_e+1$-fold, a metallic ferromagnet since the lowest
band is partially filled.  Tasaki derives it from Proposition 11.24 and Lemma 11.25
(Perron--Frobenius); the proof is deferred (Issue \#4198).
