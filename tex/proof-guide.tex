...
\item \texttt{totalSpinHalfRot\{1,2,3\}\_commute\_of\_commute},
  ...
  Tasaki eqs.~(2.2.12)$\to$(2.2.13), generic operator-level form.
...
The fully general operator-level statement is now formalized as
\texttt{totalSpinHalfRot\{1,2,3\}\_commute\_of\_commute}:
$\mathrm{Commute}\,A\,\hat S^{(\alpha)}_{\rm tot}
  \Rightarrow \mathrm{Commute}\,A\,\hat U^{(\alpha)}_{\theta,{\rm tot}}$.
Combined with PR~\#16's Heisenberg-form
\texttt{heisenbergHamiltonian\_commutator\_*}, this closes the
commutativity side of P1f'''.

The conjugation-form $(\hat U^{(\alpha)}_\theta)^\dagger \hat A
\hat U^{(\alpha)}_\theta = \hat A$ requires unitarity of
$\hat U^{(\alpha)}_{\theta,{\rm tot}}$, which in turn follows from
$-i\theta\,\hat S^{(\alpha)}_{\rm tot}$ being skew-adjoint and
\texttt{exp\_mem\_unitary\_of\_mem\_skewAdjoint}. Tracked as a
follow-up unit.
