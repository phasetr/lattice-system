...
needed for both \texttt{gibbsState\_trace} and
\texttt{gibbsExpectation\_one}. In the current Lean development this is
admitted with \texttt{sorry}. The standard argument is that $H$ is
Hermitian, hence diagonalizable with real eigenvalues $\lambda_i$, so
$Z = \sum_i e^{-\beta\lambda_i} > 0$. Formalizing this in Lean requires
a continuous-functional-calculus (CFC) assembly that is not yet
available in the project; it is deferred to a future PR.
\end{remark}
...
Gibbs state $\rho = e^{-\beta H}/Z$, $\operatorname{Tr}(\rho)=1$, $\langle\mathbf{1}\rangle=1$ & finite-dim & \emph{in progress} (1 sorry) \\
