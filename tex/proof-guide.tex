...
\[
  \sum_z \eta_z = |A|N
\]
by \path{magSumS_neelConfigOfS_complement}.  If \(x\in A\) and \(y\notin A\),
use \(\tau=\eta\) and update \(\sigma\) by setting
\(\sigma_x=N-1,\sigma_y=1\).  If \(x\notin A\) and \(y\in A\), reverse the
orientation: use \(\sigma=\eta\) and update \(\tau_x=1,\tau_y=N-1\).
