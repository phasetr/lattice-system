...
$-m_{\max} < M < m_{\max}$ require the $\hat S^\mp_{\rm tot}$
lowering-injectivity argument (subsequent PR).
...
once the raising-operator injectivity on
$\texttt{joint} \sqcap H_{M}$ for $M < m_{\max}$ is added (next PR).
...
the per-sector $\le 1$ bound at every $M$ in the spectrum; the
subsequent PR sums the $(2m_{\max}+1)$ such bounds via the
magnetisation direct-sum decomposition (\#889) to derive the final
$\dim_\mathbb{C}(\texttt{joint}) \le 2m_{\max}+1$.
...
diagonal $\hat S^z_{\rm tot}$-eigenvalue equation. This is the
framework piece for the per-sector decomposition; subsequent PRs
will prove (i) commutation with $\hat H$ and
$(\hat S_{\rm tot})^2$, (ii) joint preservation
$\texttt{magProjFn M v} \in \texttt{joint}$ for $v \in \texttt{joint}$,
and (iii) the decomposition
$v = \sum_k \texttt{magProjFn}(m_{\max} - k.\textrm{val})~v$,
closing $\texttt{joint} \subseteq \texttt{span}(\texttt{ladderIterateUp})$
and hence $\texttt{joint} = \texttt{span}(\texttt{ladderIterateUp})$
via PR~\#904's reverse inclusion.
...
vector in its joint-magnetisation sector. Summing over the
$2m_{\max}+1$ values of $k$ via PR~\#889 will identify
$\texttt{joint} = \texttt{span}(\texttt{ladderIterateUp})$ (next
PR), completing Tasaki §2.4 Theorem 2.1.
...
final summation step (next PR) uses the magnetisation direct-sum
decomposition (\#889) to derive
$\dim_\mathbb{C}(\texttt{joint}) \le 2m_{\max} + 1$, closing the
upper bound on the joint eigenspace and completing Tasaki §2.4
Theorem 2.1.
