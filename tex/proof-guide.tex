...
lemmas provide the finite-range bookkeeping used to iterate the
lowering and raising adjacent-sector energy comparisons across the
Theorem 2.3 multiplet.
...
real-sector minimality bridges.  This moves the remaining local
...
\[
\texttt{tasaki23\_heisenbergToyHamiltonianS\_mulVec\_of\_mem\_bipartiteToyGroundStateSubspacePredicted}.
\]
Combining this with the Casimir identity
\(\hat H_{\mathrm{toy},S}=2\,\hat S_A\cdot \hat S_{\bar A}\) gives
\[
\texttt{tasaki23\_two\_sublatticeSpinSDot\_mulVec\_of\_mem\_bipartiteToyGroundStateSubspacePredicted}.
\]
...
\[
\texttt{tasaki23\_two\_cross\_ladder\_mulVec\_of\_mem\_bipartiteToyGroundStateSubspacePredicted}.
\]
Thus the cross-sublattice spin-dot constraint is available in the
ladder-term form needed for the remaining comparison between
\(\hat S_A^-\Phi\) and \(\hat S_{\bar A}^-\Phi\).
