...
\item \texttt{IsInMagnetizationSubspace},
  \texttt{basisVec\_mem\_magnetizationSubspace}: magnetization
  subspace predicate and basis state membership (Tasaki
  eq.~(2.2.9)/(2.2.10)).
...
\item \texttt{totalSpinHalfRot\{1,2,3\}\_eq\_exp\_axiom} (\textbf{AXIOM}),
  \texttt{totalSpinHalfRot\{1,2,3\}\_eq\_exp}: Tasaki eq.~(2.2.11),
  $\hat U^{(\alpha)}_{\theta,{\rm tot}}
   = \exp[-i\theta \hat S^{(\alpha)}_{\rm tot}]$, currently
  axiomatized.  See §\ref{sec:open-items} below.
\end{itemize}
...
\subsection*{TODO: Problem 2.1.a for \texorpdfstring{$S \geq 1$}{S >= 1} (P1d''')}

\textbf{Statement} (Tasaki p.15): For any spin $S \geq 1$, every
operator on the single-site Hilbert space $h_0 = \mathbb{C}^{2S+1}$
(i.e.\ every $(2S+1) \times (2S+1)$ matrix) can be written as a
polynomial in $\hat 1, \hat S^{(1)}, \hat S^{(2)}, \hat S^{(3)}$.

\textbf{Proof outline} (Tasaki solution S.1, p.493):
\begin{enumerate}
\item The diagonal projector $|\psi^\sigma\rangle\langle\psi^\sigma|$
  is given by Lagrange interpolation in $\hat S^{(3)}$:
  \[
    |\psi^\sigma\rangle\langle\psi^\sigma|
    = \prod_{\substack{\tau = -S \\ \tau \neq \sigma}}^{S}
        \frac{\hat S^{(3)} - \tau \hat 1}{\sigma - \tau}.
  \]
\item Off-diagonal matrix units $|\psi^\tau\rangle\langle\psi^\sigma|$
  ($\tau \neq \sigma$) are obtained by applying $\hat S^\pm$ to a
  diagonal projector and normalizing.
\item The set $\{|\psi^\tau\rangle\langle\psi^\sigma|\}_{\tau,\sigma}$
  spans $M_{2S+1}(\mathbb{C})$; combined with Steps 1--2 this shows
  every matrix is a polynomial in $\hat 1, \hat S^{(\alpha)}$.
\end{enumerate}

\textbf{Status}: $S = 1/2$ is done as \texttt{pauliBasis} (P1d''). The
general-$S$ case requires generic \texttt{Fin\,(2S+1)} typing and a
Lagrange interpolation infrastructure; not started.

\subsection*{Axiom: Tasaki eq.~(2.2.11) (P1f'')}

\textbf{Statement}: For $\alpha \in \{1, 2, 3\}$ and $\theta \in
\mathbb{R}$,
\[
  \hat U^{(\alpha)}_{\theta,{\rm tot}}
    = \exp[-i\theta \hat S^{(\alpha)}_{\rm tot}]
    = \prod_{x \in \Lambda} \exp[-i\theta \hat S^{(\alpha)}_x].
\]

In Lean this is currently axiomatized as
\texttt{totalSpinHalfRot\{1,2,3\}\_eq\_exp\_axiom} in
\texttt{Quantum/TotalSpin.lean}, with thin re-exports
\texttt{totalSpinHalfRot\{1,2,3\}\_eq\_exp}.

\textbf{Why axiomatized}. The morally correct proof composes
\begin{enumerate}
\item $\hat U^{(\alpha)}_\theta = \exp[-(i\theta)\,\hat S^{(\alpha)}]$
  at the single-site level (P1e'',
  \texttt{spinHalfRot\{1,2,3\}\_eq\_exp}).
\item $\texttt{onSite}\,x\,(\exp B) = \exp(\texttt{onSite}\,x\,B)$,
  i.e.\ the bridge that the site-embedding ring homomorphism commutes
  with the matrix exponential. This would follow from
  \texttt{NormedSpace.map\_exp} applied to \texttt{onSiteRingHom}.
\item $\exp(\sum_x \texttt{onSite}\,x\,B)
  = \prod_x \exp(\texttt{onSite}\,x\,B)$ for distinct-site
  embeddings. This would follow from
  \texttt{Matrix.exp\_sum\_of\_commute}.
\end{enumerate}

\textbf{Step 2 is the actual blocker}. Our
\texttt{continuous\_onSite} is established under the canonical
Pi-product matrix topology
(\texttt{Matrix.instTopologicalSpace}), but
\texttt{NormedSpace.map\_exp} requires \texttt{NormedRing} and
\texttt{CompleteSpace} instances, which only resolve under the
Frobenius (or other operator) norm topology. The two topologies
coincide on finite-dimensional matrices, but Lean's instance
resolution does \emph{not} bridge them: even with
\texttt{open scoped Matrix.Norms.Frobenius} and explicit
\texttt{haveI : CompleteSpace \_ := FiniteDimensional.complete} declarations the
type-class search picks the Pi topology for
\texttt{continuous\_onSite} and the Frobenius topology for
\texttt{NormedSpace.map\_exp}, and they fail to unify.

A future PR should either (a) establish a topology-bridging lemma
between the Pi-product and Frobenius structures on
\texttt{Matrix\,(Fin n)\,(Fin n)\,$\mathbb{C}$}, (b) re-prove
\texttt{continuous\_onSite} under the Frobenius topology, or (c) find
an alternative formulation that avoids
\texttt{NormedSpace.map\_exp} entirely (e.g.\ by direct power-series
manipulation).

\subsection*{TODO: Magnetization-sector direct sum (eqs.~(2.2.9)/(2.2.10))}

\textbf{Statement} (Tasaki p.22): The full Hilbert space decomposes as
a direct sum over magnetization eigenspaces,
\[
  \mathcal H = \bigoplus_{M = -|\Lambda|S}^{|\Lambda|S} \mathcal H_M,
  \qquad
  \mathcal H_M := \{|\Phi\rangle \in \mathcal H \mid
    \hat S^{(3)}_{\rm tot} |\Phi\rangle = M |\Phi\rangle\}.
\]

\textbf{Status}: Only the predicate
\texttt{IsInMagnetizationSubspace} and the basis-state membership
lemma \texttt{basisVec\_mem\_magnetizationSubspace} are formalized.
The full direct-sum decomposition (a \texttt{DirectSum} /
\texttt{Submodule.iSup} statement that
$\mathcal H = \bigoplus_M \mathcal H_M$, plus orthogonality of
distinct sectors) requires further linear-algebra infrastructure (the
magnetization subspaces as \texttt{Submodule}\,$\mathbb{C}$\,$((\Lambda
\to \mathrm{Fin}\,2) \to \mathbb{C})$, and the direct-sum
decomposition theorem).

\subsection*{TODO: Tasaki Problem 2.2.c}

\textbf{Statement} (Tasaki p.23, eq.~(2.2.15)): An explicit averaged
state of the form
\[
  \frac{1}{4\pi} \int_0^{2\pi}\!\!d\varphi \int_0^\pi d\theta \sin\theta\;
    \hat U^{(3)}_\varphi \hat U^{(2)}_\theta\,
    |\!\uparrow_1\rangle |\!\downarrow_2\rangle
\]
equals (up to phase) the singlet $(1/\sqrt 2)(|\!\uparrow_1\rangle
|\!\downarrow_2\rangle - |\!\downarrow_1\rangle |\!\uparrow_2\rangle)$.
The problem asks to verify this and characterize states that fail to
be SU(2)-invariant.

\textbf{Status}: Not formalized. Requires a measure-theoretic
averaging construction (integration over the SU(2) parameter space),
which is beyond the current scope of finite-dim algebra. Tracked here
for a future infrastructure PR.

\subsection*{Roadmap status note: P1f''' (eqs.~(2.2.12)--(2.2.13))}

The Lean development proves $[\hat A, \hat S^{(\alpha)}_{\rm tot}] = 0$
and the corresponding finite-form $(\hat U^{(\alpha)}_\theta)^\dagger
\hat A \hat U^{(\alpha)}_\theta = \hat A$ for the \emph{Heisenberg-type}
operator $\hat A = \sum_{x,y} J(x,y)\,\hat S_x \cdot \hat S_y$ (axes
1, 2, 3 and the ladder $\hat S^{\pm}_{\rm tot}$). The fully general
operator-level statement (any $\hat A$ satisfying eq.~(2.2.12) also
satisfies eq.~(2.2.13)) is open and depends on the axiomatized
eq.~(2.2.11) above.
