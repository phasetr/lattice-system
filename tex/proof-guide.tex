...
\item \textbf{Theorem~2.4 status}.  The spin-$1/2$ case-(i) target uniqueness
  ...
  The general spin-$S$ axis-swap unitary instance is now discharged for the
  strict case~(i) degeneracy chain by \texttt{axisSwapUnitarySSpinS} and
  ...
  are now packaged, and the target wrappers from SU(2)-endpoint global
  uniqueness supply the strict gap over non-balanced non-empty sectors and the
  general spin-$S$ target uniqueness/zero-magnetization endpoint conditionally
  on that SU(2) uniqueness input.  Proving the SU(2)-endpoint global uniqueness
  input itself and Tasaki's case~(ii) route remain separate future work.
\end{itemize}
...
The \(\lambda=1\) edge of case~(i) is handled separately in
...
\texttt{spinHalf\_anisotropicHeisenbergS\_lambda\_one\_groundState\_zero\_magnetization\_of\_MLM\_casimir\_ladder\_t23\_pf}.
For general spin-\(S\), the target strict gap over non-balanced non-empty
sectors is now supplied conditionally from SU(2)-endpoint global ground-state
uniqueness; proving that SU(2) uniqueness input itself and case~(ii) remain
separate tasks.
...
\(E_{M_{\mathrm{bal}}}<E_M\) over all non-balanced non-empty sectors; case~(ii)
is still a separate route.
...
feed this strict gap to the PF-free target endpoint.  These statements are
still conditional on the SU(2)-endpoint global uniqueness input; they do not
prove Tasaki case~(ii).
