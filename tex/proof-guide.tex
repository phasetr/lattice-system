...
\begin{itemize}
  ...
  \item Problem 2.5.b graph-local star block bridge.  The declaration
        \texttt{graphLocalStarConfig} embeds a configuration on
        \(\mathrm{Option}(N_G(x))\) into the full graph configuration space
        after fixing an outside configuration \(\eta\).  Its pointwise rules
        \texttt{graphLocalStarConfig\_center},
        \texttt{graphLocalStarConfig\_neighbor}, and
        \texttt{graphLocalStarConfig\_outside} identify the center, neighbor,
        and outside coordinates.  The agreement-transfer helpers
        \texttt{graphLocalStarConfig\_agree\_off\_pair\_of\_option\_agree}
        and \texttt{option\_agree\_of\_graphLocalStarConfig\_agree\_off\_pair}
        reduce the two-site support condition for
        \(\hat{\mathbf S}_x\cdot\hat{\mathbf S}_y\) to the corresponding
        option-star support condition.  Consequently
        \texttt{spinSDot\_graphLocalStarConfig\_eq\_option} identifies every
        centered edge matrix element with the option-star edge.  Summing over
        \(y\in N_G(x)\) gives two block statements:
        \texttt{graphLocalClusterHamiltonianS\_apply\_eq\_zero\_of\_outside\_diff}
        says different outside restrictions do not couple, and
        \texttt{graphLocalClusterHamiltonianS\_block\_eq\_optionClusterHamiltonianS}
        identifies each fixed-outside block with the option-star Hamiltonian.
\end{itemize}
...
decomposition if that later becomes useful.  For Problem~2.5.b, the remaining
formal work is to lift the fixed-outside block identities for each
same-Hilbert-space graph-local star \(h_x\) to a full operator lower bound, so
that the Problem~2.5.a minimum-eigenvalue formula can be fed into the
finite-sum lower-bound bridge.
