...
\item \textbf{Theorem~2.4 status}.  The spin-$1/2$ case-(i) target uniqueness
  ...
  conditional path/target bridge, with the global crossing/degeneracy
  replacement still explicit.
\end{itemize}
...
These wrappers intentionally keep the missing global crossing/degeneracy
replacement explicit as balanced-sector/full-ground equality plus full
\(\texttt{finrank}\le2\); they do not reuse the case~(i) axis-swap sign
argument.
...
has the conditional path/target bridge described above, while the global
case~(ii) crossing/degeneracy replacement remains the next task.
