...
\item \textbf{Theorem~2.4 status}.  The spin-$1/2$ case-(i) target uniqueness
  ...
  Tasaki's case~(ii) route remains a separate future extension.
\end{itemize}
...
\texttt{spinHalf\_anisotropicHeisenbergS\_lambda\_one\_groundState\_zero\_magnetization\_of\_MLM\_casimir\_ladder\_t23\_pf}.
For general spin-\(S\), the remaining target energy input is the strict gap
over non-balanced non-empty sectors; case~(ii) remains a separate task.
