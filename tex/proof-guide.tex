...
\texttt{tasaki23\_signed\_lowering\_onA\_sum\_nonpos}.  These bounds
are deliberately weak: they decompose the remaining strict site-sum
positivity problem into the non-negative off-\(A\) part and the
non-positive on-\(A\) part, without yet proving that the former
...
\[
\texttt{tasaki23PredictedCasimirValue\_eq\_canonical\_of\_card\_notA\_le\_cardA}
\]
identifies \(C_{\mathrm{pred}}\) with the canonical joint-Casimir target
appearing in \(\texttt{bipartiteToyGroundStateSubspacePredicted}\).  Thus
\[
