...
shifted entry.  The remaining local case~(ii) work is now to prove those
dressed signs from \(\lambda\ge1\) and \(D\le0\), then feed them into
irreducibility and PF/min identification.
...
and the parity-gauged shifted-matrix structural layer.  The remaining
case~(ii) task is to prove sign non-negativity, irreducibility, and PF/min
identification for the shifted matrix on the two bare axis-swapped parity
blocks in the \(\lambda\ge1\), \(D\le0\) region.
