...
\item \textbf{Theorem~2.4 status}.  The spin-$1/2$ case-(i) target uniqueness
  ...
  are now packaged, reducing final uniqueness to the remaining
  balanced-sector/full-ground equality input.  Tasaki's case~(ii) route remains
  a separate future extension.
\end{itemize}
...
\texttt{spinHalf\_anisotropicHeisenbergS\_lambda\_one\_groundState\_zero\_magnetization\_of\_MLM\_casimir\_ladder\_t23\_pf}.
For general spin-\(S\), the remaining target input is balanced/full equality;
case~(ii) remains a separate task.
...
remove the explicit balanced-sector PF callback.  The remaining general
spin-\(S\) target input is balanced/full equality; case~(ii) is still a
separate route.
