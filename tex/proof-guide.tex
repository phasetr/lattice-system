...
\texttt{LatticeSystem/Quantum/SpinS/Theorem23SectorExistence.lean}.
The predicted
spin and admissible-sector interval API used by this definition are
factored into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23Sectors.lean}.  The
predicted-Casimir endpoint mismatch layer is factored into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedEndpoint.lean}.  The
predicted-GS and cross-ladder bridge layer is factored into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23Predicted.lean}, with the
predicted-GS ladder-closure and joint sublattice-Casimir layer split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedLadder.lean}, with the
lowered joint-magnetization extractor lemmas split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedLadderJointExtract.lean}, the
real-scalar and predicted-Casimir transfer suffix split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedLadderCasimirTransfer.lean},
and the source-weight bridge layer split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedSourceWeight.lean}, with the
re-embedded cross-ladder source-weight identities at a lowering predecessor
split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedSourceWeightCross.lean}.  The
local ladder and site-sum expansion layer is factored into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23Local.lean}, with the
raised-direction site-sum expansion theorems split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalRaisedSiteSum.lean}.  The single-site
lowering component layer is isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalLowering.lean}, with the
boundary-inclusive filtered local sign-sum layer split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalLoweringSignSum.lean} and the
single-site positivity/negativity theorems split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalLoweringPositivity.lean}.  The lowered
coefficient layer is isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalCoefficient.lean}, with the
signed-coefficient identity suffix split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalCoefficientSignedSum.lean}, with the
predecessor raising-source suffix isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalCoefficientRaisingSource.lean},
with the boundary-inclusive raising-source coefficient definition and the
raising-predecessor-source sum / dominance machinery split further into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalCoefficientRaisingSourceSum.lean}; the
predecessor-difference callback layer is split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifference.lean}, with the
unpacked predecessor-difference callback adapters split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceUnpacked.lean},
with the strict lowered site-sum positivity theorem and its callback abbrev
split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceUnpackedSiteSum.lean}; the
strict lowered site-sum dominance layer is split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceSiteSum.lean}; the
raising-direction component layer is split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceRaising.lean}, with the
single-site raising positivity group split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceRaisingPositivity.lean}
and the
raised site-sum contribution/decomposition suffix split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceRaisingSiteSum.lean}, and
the Marshall-positivity wrapper layer is split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceMarshall.lean}.  The
main \texttt{Theorem23.lean} module now keeps the site-sum and
Casimir-nonvanishing adjacent common-energy chain, while the predicted-Casimir
adjacent common-energy suffix is isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedCasimirEnergy.lean}.
The dominance-form common-energy wrappers are isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23Dominance.lean}, with the
successor predicted-Casimir transfer suffix isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23DominancePredictedCasimirTransfer.lean}
and the predicted-GS dominance suffix isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23DominancePredictedGS.lean}, with the
adjacent predicted-GS predecessor common-energy step split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23DominancePredictedGSPredecessor.lean}.  The local
adjacent-sector energy comparison suffix is isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23LocalDifferenceEnergy.lean}.
The final statement and sector-existence wrappers are isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23SectorExistence.lean}, with the
dominance-form sector-existence suffix isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23SectorExistenceDominance.lean},
with the predicted-GS dominance variants split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23SectorExistenceDominancePredictedGS.lean}; the
predicted-Casimir interval-chain suffix is isolated in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23IntervalCasimir.lean}, with the
lowered site-sum and lowered Marshall suffix split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23IntervalCasimirSiteSum.lean} and the
minimality bridge and named callback suffix split into
\texttt{LatticeSystem/Quantum/SpinS/Theorem23IntervalCasimirMinimality.lean}. The
hypothesis bundle and conclusion are aligned exactly with the per-sector
bundled Theorem 2.2
\texttt{marshallLiebMattis\_spinS\_heisenbergHamiltonianS\_groundState\_full}
(\#869). Given:
\begin{itemize}
...
\textbf{Predicted-Casimir ladder preservation}: the predicted
total-Casimir eigenspace is now packaged as stable under one
application of the total ladder operators.  The Lean theorems
\[
\begin{gathered}
\texttt{tasaki23\_totalSpinSOpMinus\_preserves\_predictedCasimirValue},\\
\texttt{tasaki23\_totalSpinSOpPlus\_preserves\_predictedCasimirValue}
\end{gathered}
\]
state that if a full spin-\(S\) vector \(\Psi\) satisfies
\[
  \hat S_{\mathrm{tot}}^2 \Psi
    =
  C_{\mathrm{pred}}\Psi,
\]
then \(\hat S^-_{\mathrm{tot}}\Psi\) and
\(\hat S^+_{\mathrm{tot}}\Psi\) satisfy the same total-Casimir
eigenvalue equation.  The proof is the standard SU(2) Casimir
commutation relation
\[
  [\hat S_{\mathrm{tot}}^2,\hat S^\pm_{\mathrm{tot}}]=0,
\]
formalised as
\texttt{totalSpinSSquared\_commute\_totalSpinSOpMinus} and
\texttt{totalSpinSSquared\_commute\_totalSpinSOpPlus}.  These
wrappers let the final Theorem~2.3 sector chain propagate the
predicted-Casimir hypothesis along adjacent ladder moves, while the
initial predicted-Casimir input and the local site-sum positivity
remain separate proof obligations.  The Lean module
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedLadderCasimirTransfer.lean}
contains these real-scalar and total-Casimir transfer wrappers.  Reference: H.~Tasaki,
\emph{Physics and Mathematics of Quantum Many-Body Systems}, Springer
2020, \S2.5 Theorem 2.3, p.~42.

\textbf{Sector-embedded predicted-Casimir ladder preservation}: the
full-space preservation wrappers have also been specialized to the
Marshall-signed sector vectors used in the adjacent-sector chain.  The Lean
theorems
\[
\begin{gathered}
\texttt{tasaki23\_totalSpinSOpMinus\_marshallSignedEmbedding}\\
\texttt{\_preserves\_predictedCasimirValue},\\
\texttt{tasaki23\_totalSpinSOpPlus\_marshallSignedEmbedding}\\
\texttt{\_preserves\_predictedCasimirValue}
\end{gathered}
\]
state that if the zero-extension of
\(\operatorname{sgn}_A(\tau)v(\tau)\), \(v(\tau)\in\mathbb R\), has
total-Casimir eigenvalue \(C_{\mathrm{pred}}\), then the lowered or raised
full-space image has the same total-Casimir eigenvalue.  This keeps the
Casimir propagation in the exact sector-embedded form needed before the
later Theorem~2.2 uniqueness comparison identifies a normalized adjacent
sector representative.

\textbf{Adjacent ladder-image predicted-Casimir package}: the sector
preservation facts are now combined with the adjacent common-energy
wrappers.  The Lean theorems
\[
\begin{gathered}
\texttt{tasaki23\_successor\_common\_energy}\\
\texttt{\_with\_lowered\_predictedCasimir},\\
\texttt{tasaki23\_predecessor\_common\_energy}\\
\texttt{\_with\_raised\_predictedCasimir}
\end{gathered}
\]
return the usual successor/predecessor common-energy data and, in the
same package, the identity
\[
  \hat S_{\mathrm{tot}}^2(\hat S^\pm_{\mathrm{tot}}\Phi)
    =
  C_{\mathrm{pred}}\,\hat S^\pm_{\mathrm{tot}}\Phi .
\]
Thus the non-zero ladder image used to enter the adjacent sector keeps
its predicted-Casimir information available before the later
sector-representative comparison.  The package is implemented in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedCasimirEnergy.lean},
which contains the predicted-Casimir adjacent common-energy suffix split from
\texttt{LatticeSystem/Quantum/SpinS/Theorem23.lean}.

\textbf{Sector-existence predicted-Casimir chain link}: the preceding
package is now connected to the per-sector Theorem~2.2 existence wrapper.
The Lean theorems
\[
\begin{gathered}
...
\end{gathered}
\]
first choose the Marshall-positive sector ground-state vector supplied
by \texttt{tasaki\_2\_5\_theorem\_2\_3\_sector\_existence}.  Assuming the
remaining predicted-Casimir identity and the appropriate site-sum
positivity for that chosen vector, they return the successor or
predecessor common-energy package and keep the identity
\[
  \hat S_{\mathrm{tot}}^2(\hat S^\pm_{\mathrm{tot}}\Phi)
    =
  C_{\mathrm{pred}}\,\hat S^\pm_{\mathrm{tot}}\Phi
\]
on the actual ladder image.  Thus the final Theorem~2.3 iteration can
work with sector-existence data rather than externally supplied
source-sector eigenvectors.

\textbf{Successor predicted-Casimir propagation}: the lowering-direction
sector-existence package is sharpened by
\[
\begin{gathered}
\texttt{totalSpinSOpMinus\_mulVec}\\
\texttt{\_marshallSignedEmbedding\_im\_zero},\\
\texttt{totalSpinSOpMinus}\\
\texttt{\_marshallSignedEmbedding\_eq\_smul\_successor\_of\_re},\\
\texttt{tasaki23\_totalSpinSSquared}\\
\texttt{\_predictedCasimirValue\_of\_real\_smul\_eq},\\
\texttt{tasaki23\_successor\_sector\_existence}\\
\texttt{\_with\_successor\_predictedCasimir}.
\end{gathered}
\]
The lowered image of a real Marshall-signed source-sector
representative is real-valued, because the entries of
\(\hat S^-_{\mathrm{tot}}\) are real.  Therefore the adjacent-sector
uniqueness relation, originally stated only for real parts, upgrades to
a full equality
\[
  \hat S^-_{\mathrm{tot}}\Phi_M = r\,\Phi_{M+1},
  \qquad r>0.
\]
The predicted-Casimir identity on the lowered image then cancels this
non-zero scalar and transfers to the successor representative
\(\Phi_{M+1}\).  This is the lowering-direction propagation step needed
to thread predicted-Casimir information along the admissible-sector
interval, rather than asking for a fresh source-sector callback at every
successor sector.  The scalar-cancellation theorem used here is kept in
\texttt{LatticeSystem/Quantum/SpinS/Theorem23PredictedLadderCasimirTransfer.lean}.

\textbf{Threaded predicted-Casimir interval chain}: the preceding scalar
transfer is now wired into the dominance-form common-energy step itself.
The Lean theorem
\[
\begin{gathered}
\texttt{tasaki23\_successor\_sector\_common\_energy}\\
\texttt{\_with\_successor\_predictedCasimir\_of\_onA\_neg\_lt\_offA}
\end{gathered}
\]
returns the usual successor common-energy package and, in the same
successor representative package, the identity
\[
  \hat S_{\mathrm{tot}}^2\Phi_{M+1}
    =
  C_{\mathrm{pred}}\Phi_{M+1}.
\]
Consequently
\[
\begin{gathered}
\texttt{tasaki23\_energy\_interval\_chain}\\
\texttt{\_with\_predictedCasimir\_of\_left\_endpoint}\\
\texttt{\_predictedCasimir\_of\_onA\_neg\_lt\_offA}
\end{gathered}
\]
strengthens the interval induction invariant from merely carrying a
common energy to carrying both the common energy and the predicted
Casimir.  The predicted-Casimir input is needed only for the left endpoint
representative selected by Theorem~2.2; each successor step obtains the
next Casimir identity from the induction hypothesis and the dominance-form
successor comparison.

\textbf{Left-endpoint predicted-Casimir final wrappers}: the threaded
chain is now connected to the existing final Theorem~2.3 packaging.  The
Lean theorems
\[
\begin{gathered}
\texttt{tasaki\_2\_5\_theorem\_2\_3\_of\_left\_endpoint}\\
\texttt{\_threaded\_predictedCasimir\_of\_onA\_neg\_lt\_offA},\\
\texttt{tasaki\_2\_5\_theorem\_2\_3\_of\_left\_endpoint}\\
\texttt{\_threaded\_predictedCasimir\_of\_onA\_neg\_lt\_offA}\\
...
\end{gathered}
\]
strip the propagated Casimir component from the strengthened interval
chain and pass the common-energy component to
\(\texttt{tasaki\_2\_5\_theorem\_2\_3\_of\_common\_energy\_chain}\).
The final conditional statement therefore no longer requires a
source-Casimir callback in every non-right-endpoint sector; it only
requires the predicted-Casimir identity for the left-endpoint
representative selected by Theorem~2.2.

\textbf{Left-endpoint predicted-GS final wrappers}: in the canonical
orientation \(|\neg A|\le |A|\), the remaining left-endpoint
predicted-Casimir input can itself be obtained from predicted
toy-ground-state membership.  The Lean theorems
\[
\begin{gathered}
\texttt{tasaki\_2\_5\_theorem\_2\_3\_of\_left\_endpoint}\\
\texttt{\_threaded\_predictedGS\_of\_onA\_neg\_lt\_offA},\\
\texttt{tasaki\_2\_5\_theorem\_2\_3\_of\_left\_endpoint}\\
\texttt{\_threaded\_predictedGS\_of\_onA\_neg\_lt\_offA}\\
\texttt{\_of\_sector\_minimality},\\
\texttt{tasaki\_2\_5\_theorem\_2\_3\_of\_left\_endpoint}\\
...
\end{gathered}
\]
to turn left-endpoint membership in
\(\texttt{bipartiteToyGroundStateSubspacePredicted}\,A\,N\) into the
Casimir callback required by the threaded interval-chain final wrappers.
Thus the final conditional statement now asks for predicted-GS membership
only at the left endpoint, rather than a direct Casimir identity.

\textbf{Lowered site-sum final wrappers}: the dominance inequality used
above is a sufficient way to prove the strict positivity of the lowered
Marshall-signed site-sum, but the adjacent-sector comparison only needs
that strict site-sum positivity itself.  Lean therefore also provides the
site-sum form
\[
\begin{gathered}
\texttt{tasaki23\_successor\_sector\_common\_energy}\\
\texttt{\_of\_site\_sum\_pos\_of\_mem}\\
\texttt{\_bipartiteToyGroundStateSubspacePredicted},
\\
\texttt{tasaki23\_energy\_interval\_chain\_of\_left\_endpoint}\\
\texttt{\_predictedGS\_of\_lowered\_site\_sum\_pos}.
\end{gathered}
\]
The first theorem combines predicted toy-ground-state membership with
the direct strict lowered site-sum input; the second iterates this step
through the admissible sector interval.  The final wrappers
\[
\begin{gathered}
\texttt{tasaki\_2\_5\_theorem\_2\_3}\\
\texttt{\_of\_predictedGS\_of\_lowered\_site\_sum\_pos},\\
\texttt{tasaki\_2\_5\_theorem\_2\_3}\\
\texttt{\_of\_predictedGS\_of\_lowered\_site\_sum\_pos}\\
\texttt{\_of\_sector\_minimality},\\
\texttt{tasaki\_2\_5\_theorem\_2\_3}\\
...
\end{gathered}
\]
then reuse the common-energy packaging and the existing sector and
real-sector minimality bridges.  The left-endpoint threaded wrapper
\[
\begin{gathered}
\texttt{tasaki\_2\_5\_theorem\_2\_3}\\
\texttt{\_of\_left\_endpoint\_threaded\_predictedGS}\\
\texttt{\_of\_lowered\_site\_sum\_pos}\\
\texttt{\_of\_sector\_minimality}
\end{gathered}
\]
uses the strengthened interval chain to require predicted-GS membership
only at the left endpoint while still consuming the direct site-sum
callback.  This moves the remaining local
positivity obligation from the stronger dominance statement to the
actual strict site-sum positivity needed for Marshall positivity of
\(S^-_{\mathrm{tot}}\Phi\).
...
\end{gathered}
\]
The predicted-data bridge layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23Predicted}\).  The local
site-sum and Casimir-nonvanishing adjacent common-energy layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23}\), with the predecessor
(raising-direction) adjacent common-energy steps split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23CommonEnergyPredecessor}\)
and the predicted-Casimir
adjacent common-energy suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23PredictedCasimirEnergy}\).  The local
lowering component layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalLowering}\), with the
boundary-inclusive local sign-sum layer in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalLoweringSignSum}\) and the
single-site positivity/negativity layer in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalLoweringPositivity}\).  The
local coefficient layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalCoefficient}\), with the
signed-coefficient identity suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalCoefficientSignedSum}\),
with the predecessor raising-source suffix in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalCoefficientRaisingSource}\),
with the boundary-inclusive raising-source coefficient definition and the
raising-predecessor-source sum / dominance machinery split further into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalCoefficientRaisingSourceSum}\).  The
local predecessor-difference layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifference}\), with the
unpacked predecessor-difference callback adapters in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceUnpacked}\), with
the strict lowered site-sum positivity theorem and its callback abbrev split
into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceUnpackedSiteSum}\).  The
strict lowered site-sum dominance layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceSiteSum}\).  The
raising-direction component layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceRaising}\), with the
single-site raising positivity group split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceRaisingPositivity}\)
and the
raised site-sum contribution/decomposition suffix isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceRaisingSiteSum}\), and
the Marshall-positivity wrapper layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceMarshall}\), with
the adjacent-sector energy comparison wrappers split further into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceEnergy}\), with the
Casimir-non-vanishing and site-sum-positivity adjacent-sector packages split
further into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23LocalDifferenceEnergyCasimir}\).  The
sector-existence and final-statement layer is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23SectorExistence}\), with the
dominance-form sector-existence suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23SectorExistenceDominance}\),
with the predicted-GS dominance variants split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23SectorExistenceDominancePredictedGS}\),
the predicted-GS sector-existence interval suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23SectorExistenceInterval}\), with
the lowered-vector-Marshall interval chain split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23SectorExistenceIntervalMarshall}\), and
the predecessor sector-existence chain link with the raised predicted-Casimir
variant split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23SectorExistencePredecessor}\).  The
named interval callback propositions are isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23IntervalCallbacks}\).  The
direct interval-chain wrappers are isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23Interval}\), with the
predicted-GS-aware local-comparison interval suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23IntervalPredictedGS}\), with the
joint-component and lowered-joint cross-ladder interval suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23IntervalJoint}\), with the
unpacked lowered-joint cross-ladder interval suffix split further into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23IntervalJointUnpacked}\), and with the
predicted-Casimir interval-chain suffix split further into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23IntervalCasimir}\), with the
lowered site-sum and lowered Marshall suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23IntervalCasimirSiteSum}\) and the
minimality bridge and named callback suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23IntervalCasimirMinimality}\).  The
outside-ground lower-bound callbacks and common final packaging wrappers
are isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGround}\), with the
predicted-Casimir final-wrapper suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredictedCasimir}\),
with the left-endpoint threaded predicted-Casimir final-wrapper suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredictedCasimirThreaded}\),
with the conditional left-endpoint predicted-GS final-wrapper suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundConditional}\),
and with the left-endpoint predicted-GS off-\(A\)/on-\(A\) dominance
final-wrapper suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredictedGS}\),
and with the left-endpoint threaded predicted-GS lowered-Marshall
final-wrapper suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredictedGSLoweredMarshall}\).
The cross-ladder final-wrapper tail is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundCrossLadder}\),
with the lowered-joint final-wrapper suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundCrossLadderLoweredJoint}\),
with the lowered-joint cross-ladder final wrappers split further into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundCrossLadderLoweredJointCross}\),
with the unpacked lowered-joint callback boundary split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundCrossLadderUnpacked}\),
with the re-embedded cross-ladder and source-weight suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundCrossLadderReembedded}\),
and with the unpacked re-embedded source-weight final-wrapper suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundCrossLadderReembeddedUnpacked}\),
and the predecessor-specialized final wrapper tail is isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredecessor}\),
with the lowerable and explicit lowerable positive-source suffix split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredecessorLowerable}\),
with the real source-weight and raising-source boundary split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredecessorRaising}\),
and with the predecessor-difference outside-sector boundary split further into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23OutsideGroundPredecessorDifference}\).
The
public named-callback aliases continue to live in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23Final}\), with the
direct lowered-site-sum final suffix isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23FinalLoweredSiteSum}\), with the
left-endpoint threaded lowered-site-sum wrapper and named-callback abbrevs
split into
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23FinalLoweredSiteSumLeft}\), and
with the lowered-vector-Marshall final suffix isolated in
\(\texttt{LatticeSystem.Quantum.SpinS.Theorem23FinalLoweredMarshall}\).
The first records the adjacent-sector output: one common Marshall-positive
energy on every admissible
\(\texttt{tasaki23GroundStateSectors}\) sector.  The second records the
remaining outside-sector estimate \(\mu\le\mu_M\) for the
Marshall-positive representatives supplied by the per-sector Theorem 2.2
wrapper, and the family callback records the same estimate after the
interval chain has produced the common energy \(\mu\).  The left-endpoint
predicted-GS callback records the lower endpoint membership input before
the uniform source callback feeds the same endpoint into the interval
chain.  The predecessor
callback records the local predecessor
raising-source difference positivity used by the direct lowered-site-sum
interval chain:
for each non-right-endpoint admissible sector, the re-embedded real
source-weight identity and lowered sublattice-Casimir/support data are
unpacked before proving that the off-\(A\) predecessor sum strictly
dominates the on-\(A\) predecessor sum.  The source predicted-GS callback
records the uniform input that each Marshall-positive source-sector
representative below the spectral shift belongs to the predicted toy
ground-state subspace.  This is only an API naming step;
the mathematical bridge remains
the Perron--Frobenius comparison of Tasaki, Springer 2020, Section 2.5,
Theorem 2.3, p.~42, with Seneta, \emph{Non-negative Matrices and Markov
Chains}, 3rd ed., Springer 2006, \S1.2, pp.~27--28.
...
\end{gathered}
\]
then feeds these non-zeroness callbacks through the full-reach bridge.
...
left and right Casimir-avoidance callbacks.
...
\end{gathered}
\]
Thus the reference-vector API can now be used directly at both public
named-callback final boundaries, not only at the intermediate saturated
joint source-vector callback.
...
and then reuse the corresponding saturated joint-reference final boundary.
...
ladder references and then reuse the corresponding saturated ladder-reference
final boundary.
...
\end{gathered}
\]
feed the same lower-family route into the threaded predicted-GS and
predecessor-difference final boundary.
...
wrappers feed the same lower-family theorem into the public
outside-sector-ground-energy-lower-bound boundary, so the discharged route can
also be stated directly from the explicit lowerable attach-sum dominance input.
...
into the non-empty discharged final boundary.  The real-coupling final API
wrapper
\[
\begin{gathered}
\texttt{tasaki\_2\_5\_theorem\_2\_3}\\
...
\end{gathered}
\]
specializes that explicit wrapper by supplying the saturated-energy witness
from
\(\texttt{saturatedFerromagnetEigenvalueS\_exists\_real}\), so the
real-coupling discharged route no longer exposes either the outside-sector
lower-family hypothesis or the saturated-energy real-scalar hypothesis.  The
...
source-vector callbacks.  The direct wrapper then reuses the saturated
joint-source final boundary, while the extracted wrapper reuses the route
through the scalar source-energy and saturated-Casimir extraction layer.
...
\end{gathered}
\]
therefore exposes the final proof boundary directly in terms of the two
directional outside-sector reach tasks.  The final named predecessor-difference
boundaries
\[
\begin{gathered}
...
hypotheses.  These final boundaries live in
...
family directly into the explicit source common-energy final boundary, with
the physical-range and discharged reach wrappers inheriting this route.
...
reuses the corresponding outside-sector ground-energy boundary.
