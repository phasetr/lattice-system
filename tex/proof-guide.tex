...
$M \neq m_{\max}$. The final step (subsequent PR) iterates this
chain from $M = -m_{\max}$ upward and sums the per-sector $\le 1$
bounds across the $(2m_{\max}+1)$ values of $M$ in the spectrum,
using the magnetisation direct-sum decomposition (\#889) and the
$H_{m_{\max}}$ base case (\#2759).
