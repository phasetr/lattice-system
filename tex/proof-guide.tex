...
consume the strict raw-support layer and the \(D=0\) bond-only boundary,
respectively.  The remaining endpoint input is the \(\lambda=1\) ion-only
support callback needed to close the pathwise PF/min hypotheses for every
\(t\in[0,1]\).
...
\(D=0\) variants, including raw-support wrappers at \(D=0\).  The parity-block
PF/min layer now transfers shifted case~(ii) parity-gauged non-negative
irreducible blocks to bare axis-swapped parity-block PF/min witnesses in the
strict raw-support region and at the \(D=0\) boundary.  The remaining
case~(ii) task is the \(\lambda=1\) ion-only support callback needed to close
the pathwise PF/min hypotheses at that endpoint.
