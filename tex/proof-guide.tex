...
\subsection*{Deferred: full CAR algebra}

The canonical anticommutation relations
$c_i^2 = 0$, $(c_i^\dagger)^2 = 0$,
$\{c_i, c_j^\dagger\} = \delta_{ij} \cdot I$,
$\{c_i, c_j\} = 0$ for $i \neq j$ are deferred to follow-up PRs.
They require careful tracking of the JW-string interactions across
sites; the on-site $c_i^2 = 0$ requires the JW string to commute past
the same-site $\sigma^+$ (which works since the string lives on
sites strictly less than $i$), while the cross-site
anticommutations involve sign cancellations from $\sigma^z$ acting
