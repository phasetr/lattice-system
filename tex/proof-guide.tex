...
\item \textbf{Theorem~2.4 status}.  The spin-$1/2$ case-(i) target uniqueness
  ...
  \(-1<\lambda<1\), \(D\ge0\).  Tasaki's case~(ii) and the general spin-$S$
  \(\lambda=1\) edge remain separate future work.
\end{itemize}
...
the SU(2)-global-uniqueness wrappers and the MLM/Casimir endpoint below;
case~(ii) and the general \(\lambda=1\) target wrapper remain separate tasks.
...
\(D\ge0\).  Tasaki case~(ii) and the general spin-\(S\) \(\lambda=1\) edge
