...
\texttt{spinHalf\_anisotropicHeisenbergS\_lambda\_one\_groundState\_zero\_magnetization\_of\_MLM\_casimir\_ladder\_t23\_pf}.
For general spin-\(S\), the strict-interior target endpoint is now obtained by
composing the SU(2)-global-uniqueness wrappers with the MLM/Casimir endpoint
below; case~(ii), the general \(D=0\) edge, and the general \(\lambda=1\) edge
