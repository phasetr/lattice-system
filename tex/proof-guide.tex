...
of Tasaki eq.~(2.4.7).  This composition is not formalised here; see
the remark below.
\end{theorem}
...
\begin{remark}[Toward $|\Xi_{\theta,\phi}\rangle$]
A single axis-$\alpha$ rotation here lies one composition short of the
two-axis rotation $\hat U^{(3)}_{\phi}\hat U^{(2)}_{\theta}$ used in
Tasaki eq.~(2.4.6).  The composition follows by applying the theorem
twice: first to pass $\hat H$ past $\hat U^{(2)}_{\theta}$, then past
$\hat U^{(3)}_{\phi}$, since both rotations commute with the
...
\subsection*{Deferred: cross-site CAR}

The cross-site relations
$\{c_i, c_j^\dagger\} = 0$ for $i \neq j$ and
$\{c_i, c_j\} = 0$ for $i \neq j$ are deferred to follow-up PRs.
They require careful tracking of sign cancellations from the JW
string $\sigma^z$ acting on intermediate sites between $i$ and $j$.
