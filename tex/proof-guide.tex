...
\subsection*{In progress: Tasaki §2.5 Theorem 2.4 (Mattis--Nishimori)}
...
(\texttt{magSubspaceS\_disjoint}), forcing each scalar to vanish.  Applied to
three nonzero vectors in three pairwise distinct sectors, the same argument
extends to a 3-element linearly independent family, contradicting Theorem 2.4
obligation (1)'s \(\dim\le2\) bound — the contradiction lever in the sector
decomposition + reflection argument.
...
This lets the explicit gauge unitary (a Wigner \(d\)-matrix \(\exp(-i\pi\hat
S^1/2)\) for general spin) be isolated behind an interface
(\texttt{AxisSwapUnitaryS}), and at spin-\(1/2\) it is instantiated concretely
by the \(\pi/2\) rotation about axis~1 (\texttt{axisSwapUnitarySpinHalf}, using
the existing \texttt{spinHalfRot1} conjugation lemmas), giving the spin-\(1/2\)
equal-degeneracy milestone.
