...
formal work is to identify each same-Hilbert-space graph-local star \(h_x\)
with a copy of the abstract single-cluster Hamiltonian on
\(\mathrm{Fin}(\deg(x)+1)\), so that the Problem~2.5.a minimum-eigenvalue
formula can be fed into the finite-sum lower-bound bridge.
