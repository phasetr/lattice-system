...
\medskip\noindent\textbf{Theorem A.12 (strong-coupling effective Hamiltonian, proved).}
...
$\psi\in H_0$, avoiding an explicit projection matrix), in the non-vacuous
finite-energy${}\Rightarrow{}$effective direction: a family $\Phi_v$ of $\hat H_v$-eigenstates
converging to a nonzero $\Phi$ with energy $E_v\to E$ has $\Phi\in H_0$ and $\Phi$ solving the
effective equation.  \textbf{Now proved (axiom-free)}
(\texttt{effectiveHamiltonian\_strongCoupling\_limit}, \texttt{Math/EffectiveLimit.lean}):
pairing the eigenvalue relation with $\Phi_v$ gives
$v\,\langle\Phi_v|\hat V|\Phi_v\rangle=E_v\langle\Phi_v|\Phi_v\rangle-\langle\Phi_v|\hat H_0|\Phi_v\rangle$,
whose right side converges; multiplying by $v^{-1}\to0$ shows the limiting quadratic form
$\langle\Phi|\hat V|\Phi\rangle$ vanishes, and Lemma A.11 yields $\hat V\Phi=0$.  Pairing instead
with a fixed $\psi\in\ker\hat V$ kills the $v\hat V$ term outright ($\hat V$ Hermitian), so
$\langle\psi|\hat H_0|\Phi_v\rangle=E_v\langle\psi|\Phi_v\rangle$ for every $v\ge0$, and the limit
is the weak effective equation (sesquilinear pairings are coordinatewise finite sums, hence
continuous).  This underlies the §11.5 effective-theory limits
...
fully formalized in book order (A.1--A.28), and \textbf{every item the book itself proves is now
proved axiom-free}: A.1--A.20 (including the Lie product / Trotter formula A.1 built from scratch,
Weyl eigenvalue monotonicity A.7 via Courant--Fischer, the trace-exponential monotonicity A.8 via
the spectral mapping, the strong-coupling effective Hamiltonian A.12, the
simultaneous-diagonalization step of A.17, Perron--Frobenius for a real symmetric matrix A.18, and
the polar / singular-value decompositions A.19/A.20).  The remaining items---the operator-algebraic
A.21--A.28 (Wigner, states, Banach--Alaoglu, ground states, GNS)---are recorded as faithful
documented axioms.  These
