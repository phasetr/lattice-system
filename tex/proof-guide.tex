...
turn split into \texttt{GeneralFlatBandCanonicalCoord.lean} for build speed.  The occupation-basis coordinate functional distributes over a single inner peel term as $\texttt{repr}(\text{peelTerm})\,g = (-1)^i\,[qs_i{=}s]\,\mu_{qs_i}(x)\,\texttt{repr}(\text{Slater}(qs.\texttt{eraseIdx}\,i))\,g$ (\texttt{generalFlatBandPeelTerm\_repr}), so the inner $j$-sum collects term-wise to the bridge coordinate of the double-erased rest state, whose list is itself the canonical creation list over the twice-erased index set $(I\setminus\{a\})\setminus\{b\}$ (\texttt{flatBandSpinConfigList\_eraseIdx\_eraseIdx}).  Since this twice-erased set contains neither $a$ nor $b$ and $\sigma$ agrees with the spin-swapped $\sigma_{a\leftrightarrow b}$ off $\{a,b\}$, the rest list is identical for the two configurations (\texttt{flatBandSpinConfigList\_congr}), so the shared rest coefficient --- nonzero by evaluating the bridge coordinate at the rest config itself (\texttt{generalFlatBandSlaterState\_repr\_self\_ne\_zero}) --- cancels in the comparison $D(\sigma)=D(\sigma_{a\leftrightarrow b})$.)
