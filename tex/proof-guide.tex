...
\subsection*{Deferred: cross-site CAR for \(0 < i < j\)}

The fully general cross-site relations
\(\{c_i, c_j^\dagger\} = 0\) and \(\{c_i, c_j\} = 0\) for arbitrary
\(i \neq j\) (with neither site fixed to \(0\)) are deferred to
follow-up PRs. The proof structure mirrors the \(i = 0\) case but
requires an additional factorisation of the JW string at site \(i\)
so that the anti-commutator of \(\sigma^+_i\) with
\texttt{jwString}\,\(j\) can be evaluated site-by-site.
