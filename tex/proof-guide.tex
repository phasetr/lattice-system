...
\item \textbf{Theorem~2.4 status}.  The spin-$1/2$ case-(i) target uniqueness
  ...
  the general spin-$S$ target conditional bridge reduces final uniqueness to
  balanced-sector PF plus balanced-sector/full-ground equality.  Tasaki's
  case~(ii) route and those remaining spin-$S$ target inputs remain separate
  future extensions.
\end{itemize}
...
\texttt{spinHalf\_anisotropicHeisenbergS\_lambda\_one\_groundState\_zero\_magnetization\_of\_MLM\_casimir\_ladder\_t23\_pf}.
Case~(ii) and final spin-\(S\) target uniqueness remain separate tasks.
...
remaining general spin-\(S\) work is to prove the balanced-sector PF and
balanced/full equality inputs; case~(ii) is still a separate route.
