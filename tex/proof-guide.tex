...
Since the maximal total spin at fixed electron number $N$ (with no double occupancy) is
$S_{\max}=N/2$, the statement requires the electron number to be \emph{fixed}; we introduce the
filling-restricted predicates \texttt{chargeSpinSector}, \texttt{sectorMinEnergyAtFilling}
($E_{\min}(N_e,S)$) and \texttt{exhibitsFerromagnetismAtFilling} (the counterpart of the §11.4
\texttt{exhibitsFerromagnetism} with the electron number constrained).  \textbf{Proposition
...
$2S_{\rm tot}+1=N_e+1$-fold $SU(2)$ multiplet.  Tasaki's proof is a Perron--Frobenius /
