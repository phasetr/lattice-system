...
\medskip
\noindent\textbf{Theorem 11.8 (axiomatized) and Lemma 11.9 (proved).}
...
development the predicates live in
\texttt{Fermion/JordanWigner/Hubbard/NagaokaBondGraph.lean}
(\texttt{nagaokaBondGraph}, \texttt{IsBiconnected}, \texttt{IsSimpleLoopGTFour},
\texttt{IsExchangeBond}, \texttt{ConnectedByExchangeBonds}), and the two
statements in \texttt{NagaokaConnectivityClassification.lean}. Theorem 11.8 is
recorded as an \textbf{\texttt{axiom}} (\texttt{nagaoka\_theorem\_11\_8}):
Tasaki \emph{explicitly leaves its proof to the original papers}
(Bobrow--Stubis--Li; Wilson's ``15-puzzle'' theorem, p.~387 refs [4], [81]) — a
cited external classification theorem. \textbf{Theorem 11.7 above is
\texttt{sorry}-free and does not depend on this axiom.}

\medskip
\noindent\textbf{Proof of Lemma 11.9 (\texttt{nagaoka\_lemma\_11\_9}, stated in
\texttt{NagaokaConnectivityClassification.lean}; machinery in
\texttt{NagaokaStateQuiver.lean}, graph predicates in
\texttt{NagaokaBondGraph.lean}).}
Lemma 11.9, initially axiomatized, is now \emph{proved}, following Tasaki's
hole-motion argument (1st ed., §11.2.2, pp.~387--388, Figs.~11.8--11.9,
footnotes 13--14) made precise as strong connectivity of the per-sector quiver
of \(-M\):
\begin{enumerate}
\item \emph{Edges.} \((-M)_{(y,\tau),(x,\sigma)} > 0\) iff \(x \ne y\),
  \(t_{xy} > 0\), and \(\tau\) is \(\sigma\) with the hole hopped \(x \to y\)
  (\texttt{neg\_tasakiEffReMatrix\_pos\_iff}); this generates the reachability
  relation \texttt{StateReach} (reflexive, transitive, and — by symmetry of
  \(M\) — symmetric).
\item \emph{Loop trips.} Circling a triangle of bonds transposes the two
  passed spins (\texttt{StateReach.transposition}, Fig.~11.8). Circling a
  length-4 loop 3-cycles the three passed spins; because spins are Boolean,
  one or two trips realize an exact transposition of the diagonal pair
  (\texttt{landing\_swap\_quad}, Fig.~11.9) \emph{or} of an adjacent pair
  (\texttt{landing\_swap\_quad\_adj}), per footnote 14.
\item \emph{Controlled transport.} The hole travels from any position \(p\) to
  the loop along a walk avoiding the exchanged pair \(\{x,y\}\) — such a route
  exists by E2 (\texttt{exists\_avoiding\_walk\_of\_induce\_connected}) — swaps
  in place, and returns along the reversed walk, restoring every other spin
  (\texttt{holeWalkTransport}, \texttt{swap\_via\_landing\_walk}). Hence every
  exchange bond yields a spin swap available from \emph{every} hole position
  (\texttt{reachSwap\_of\_isExchangeBond}, dispatching the length-4 cycle into
  its diagonal and adjacent cases).
\item \emph{Generation (footnote 13).} Swaps propagate along exchange-bond
  walks through the conjugation \((y\,z) = (y\,w)(w\,z)(y\,w)\), tracking the
  finite set of sites the hole must avoid (\texttt{ReachSwapOff.of\_walk}).
  Parking the hole at a \emph{farthest} vertex \(q\) of the exchange-bond graph
  makes every pair swappable: a geodesic from the root to any other vertex
  cannot pass through \(q\), so deleting \(q\) disconnects nothing
  (\texttt{exists\_vertex\_walks\_avoid}, the parking lemma — no cut-vertex
  analysis needed).
\item \emph{Assembly.} Two same-magnetization configurations at the parked
  hole differ on an even number of sites split into opposite mismatches
  (equal up-spin counts); swapping one opposite pair reduces the disagreement
  by two, and induction connects them
  (\texttt{StateReach.of\_swaps\_of\_holeSpinMag\_eq}). Hole mobility along the
  connected bond graph aligns arbitrary states
  (\texttt{StateReach.exists\_hole\_at}), an out-and-back hop supplies the
  positive-length diagonal of strong connectivity
  (\texttt{exists\_pos\_selfPath}), and sector reachability yields
  irreducibility (\texttt{nagaokaConnectivity\_of\_reach}). Finally, neither
  \(M\) nor the bond graph reads the diagonal of \(t\)
  (\texttt{tasakiEffReMatrix\_zeroDiag}, \texttt{nagaokaBondGraph\_zeroDiag}),
  so the auxiliary zero-diagonal hypothesis is removed and the lemma holds
  verbatim for symmetric \(t \ge 0\).
\end{enumerate}
...
\medskip
\noindent\textbf{Rotated-basis Fock-spanning infrastructure (Issue \#4346,
\texttt{TasakiFlatBandModeCreation.lean}, \texttt{TasakiFlatBandModeMonomial.lean},
\texttt{Math/ListProdMulVec.lean}).}  Towards that hard inclusion, the
single-particle change of basis is lifted to the many-body level.  The
\emph{mode creation/annihilation maps} $\hat C^\dagger_\sigma(w)=\sum_x w(x)\,\hat
c^\dagger_{x,\sigma}$ and $\hat C_\sigma(w)$ (\texttt{flatBandModeCreation},
\texttt{flatBandModeAnnihilation}) are $\mathbb C$-linear in the single-particle
vector $w$, with $\hat a^\dagger,\hat b^\dagger,\hat a,\hat b$ their values at
$\alpha,\beta$; since $\{\alpha\}\cup\{\beta\}$ is a basis (Lemma 11.10), every
site creation $\hat c^\dagger_{x,\sigma}=\hat C^\dagger_\sigma(\delta_x)$ is the
$\{\alpha\}\cup\{\beta\}$-expansion combination of the $\hat a^\dagger/\hat b^\dagger$
(\texttt{flatBandModeCreation\_eq\_repr\_sum}, \texttt{flatBandModeCreation\_single}).
The bilinear double sum of the spinful site CAR collapses on the diagonal to the
\emph{generic single-particle CAR} $\{\hat C_\sigma(w),\hat
C^\dagger_\tau(w')\}=(\sum_x w(x)w'(x))\,\delta_{\sigma\tau}\,\mathbf 1$
(\texttt{flatBandMode\_annihilation\_creation\_anticomm}), which specialises to
$\{\hat a,\hat a^\dagger\}$, the $\beta$-Gram $\{\hat b,\hat b^\dagger\}$, and
$\{\hat b,\hat a^\dagger\}=0$.  Writing \texttt{flatBandModeFockSubmodule} for the span
of the \emph{rotated Fock monomials} $\bigl(\prod \hat C^\dagger_\sigma(\text{basis
}i)\bigr)|\mathrm{vac}\rangle$, this span is invariant under every site creation
(it is a finite combination of the $\hat a^\dagger/\hat b^\dagger$, each of which
prepends one mode); since any computational basis vector $\mathtt{basisVec}\,c$ is
an ordered product of site creations on the vacuum
(\texttt{prod\_creation\_mulVec\_vacuum}) and a $\mathtt{mulVec}$-invariant
submodule absorbs such products (\texttt{listProd\_mulVec\_mem}), the rotated
monomials span the whole space: \texttt{flatBandModeFockSubmodule}~$=\top$
(\texttt{flatBandModeFockSubmodule\_eq\_top}).  Reindexing them by occupation
configs of the $4K{+}4$ rotated modes gives, since their count $2^{4K+4}$ equals
$\operatorname{finrank}V$, the \emph{rotated occupation basis}
\texttt{flatBandOccBasis} (\texttt{TasakiFlatBandOccBasis.lean}; the reorder uses
only $\{\hat C^\dagger,\hat C^\dagger\}=0$ via $\exists z$, no explicit sign).

\medskip
\noindent\textbf{The hard inclusion $\hat b$-kernel $\subseteq$ $\alpha$-Fock,
PROVED (\texttt{TasakiFlatBandUniqueness.lean}).}  The $\beta$-Gram matrix
$G_{uv}=\langle\beta_u,\beta_v\rangle$ is positive definite (Lemma 11.10), hence
invertible (\texttt{flatBandBetaGram}); its inverse defines the \emph{dual
annihilators} $\hat d_{u,\sigma}=\sum_v(G^{-1})_{uv}\hat b_{v,\sigma}$ with
$\{\hat d_{u,\sigma},\hat b^\dagger_{w,\tau}\}=\delta_{uw}\delta_{\sigma\tau}$,
$\{\hat d_{u,\sigma},\hat a^\dagger\}=0$, $\hat d_{u,\sigma}|\mathrm{vac}\rangle=0$
(\texttt{TasakiFlatBandDualAnnihilation.lean}).  Then
$\hat b^\dagger_{u,\sigma}\hat d_{u,\sigma}$ is the occupation projector of the
$\beta$-mode $(\mathrm{inr}\,u,\sigma)$ on the occupation basis (peel with
$\hat d$, glue back with $\hat b^\dagger$;
\texttt{flatBandBCreation\_dual\_mulVec\_occMonomial}).  A $\hat b$-kernel vector
$v$ has $\hat d_{u,\sigma}v=0$, so each projector kills it; expanding $v$ in the
occupation basis and reading off coordinates, every $\beta$-occupied coordinate
vanishes (basis independence).  Hence $v$ is a combination of $\beta$-free
occupation monomials, each an $\alpha$-Slater state, so $v\in\alpha$-Fock
(\texttt{flatBandBKernelSubmodule\_le\_alphaFockSubmodule}, axiom-clean).  Combined
with the easy inclusion this gives $\hat b$-kernel $=\alpha$-Fock.  A giant-type
\texttt{isDefEq} blow-up in the generic move-through lemmas (operator type
$\mathrm{Fin}(2(2K{+}1){+}2)$ vs.\ $\mathrm{Fin}(M{+}1)$) is dodged by re-proving
the needed pass-through by direct induction in the operators' native type.
...
Theorem 11.8, because a faithful proof needs the Fock-space
...
\medskip
\noindent\textbf{Dimension-route reduction of the classification axiom (Issue
\#4346, \texttt{TasakiFlatBandClassification.lean}).}  Toward discharging the axiom,
the uniqueness $\leq$ half is reduced to a sharp finite-dimensional linear-algebra
statement, following the dimension method of Theorem 11.26 rather than an explicit
symmetric-tensor construction. The multiplet has the expected dimension
$\operatorname{finrank}\texttt{flatBandFerromagneticMultipletSubmodule}=K+2$
(\texttt{flatBandFerromagneticMultipletSubmodule\_finrank}, from linear independence
of the $K+2$ lowered states). The total $\hat S^z_{\rm tot}$ commutes with $\hat
H_{\rm flat}$ (\texttt{fermionTotalSpinZ\_commute\_flatBandHamiltonian}): from
$[\hat S^\pm_{\rm tot},\hat H]=0$ and the $\mathfrak{su}(2)$ relation $\hat
S^+\hat S^- - \hat S^-\hat S^+ = 2\hat S^z$, the operator $2\hat S^z$ commutes with
$\hat H$, hence so does $\hat S^z$; together with $[\hat S^z,\hat N]=0$ this makes
$\hat S^z$ preserve the ground subspace $G$. Because $\hat S^z$ acts diagonally with
half-integer weights $a-(K+1)/2$ ($a$ the up-count), and a ground vector lives in
the $\hat N=K+1$ shell where only the $K+2$ weights $a-(K+1)/2$ ($a\in\{0,\dots,K+1\}$)
occur, $G$ decomposes as the finite supremum
\[
  G = \bigsqcup_{a\in\mathrm{Fin}(K+2)}
        G \sqcap \mathrm{eigenspace}\bigl(\hat S^z,\ a-\tfrac{K+1}{2}\bigr)
\]
(\texttt{flatBandHalfFilledGroundSubmodule\_eq\_iSup\_weight}; off-weight blocks are
$\bot$ by the diagonal support argument). Since distinct eigenspaces are independent,
the blocks form an internal direct sum, so $\operatorname{finrank}G=\sum_a
\operatorname{finrank}(\text{block}_a)$. Hence \emph{if} each $\hat S^z$-weight block
is at most one-dimensional, then $\operatorname{finrank}G\leq K+2$
(\texttt{Math/EigenspaceWeightFinrank.lean},
\texttt{finrank\_le\_of\_weight\_blocks}); combined with the proven inclusion
$\texttt{multiplet}\leq G$
(\texttt{flatBandFerromagneticMultipletSubmodule\_le\_groundSubmodule}) and
$\operatorname{finrank}\texttt{multiplet}=K+2$, the dimension comparison
\texttt{Submodule.eq\_of\_le\_of\_finrank\_le} forces $G=\texttt{multiplet}$
(\texttt{flatBand\_groundSubmodule\_eq\_multipletSpan\_of\_blocks}). This isolates the
entire remaining content of the classification axiom into the single sharp statement
``each $\hat S^z$-weight block of $G$ is $\leq 1$-dimensional'' (the symmetric /
maximal-spin core).

\smallskip\noindent\textbf{Discharge of the block bound and removal of the axiom
(Issue \#4346, \texttt{TasakiFlatBandSwapCoeff.lean}).}  The block bound
\texttt{flatBand\_block\_finrank\_le\_one} is proved by a Marshall-sign / swap-invariance
argument on the occupation coordinates, in the rotated single-particle basis
$\{\alpha_p\}\cup\{\beta_u\}$ (\texttt{flatBandOccBasis}, the full Fock basis indexed by
occupation configurations). For a ground vector $v$ the local frustration-free conditions
give $\hat b_{u,\sigma}v=0$ (no $\beta$-occupation) and $\hat c_{x\downarrow}\hat
c_{x\uparrow}v=0$ at every physical site $x$ (no double occupancy, the Gram identity
$\hat n_{x\uparrow}\hat n_{x\downarrow}=(\hat c_{x\downarrow}\hat
c_{x\uparrow})^{\mathsf H}(\hat c_{x\downarrow}\hat c_{x\uparrow})$). Reading the
two-hole coordinate of $0=\hat c_{\mathrm{int}(p)\downarrow}\hat
c_{\mathrm{int}(p)\uparrow}v$ on the orbital-ordered canonical list (where the
overlapping pair $p,p+1$ sits at adjacent positions, so moving it to the front carries
the explicit Koszul sign $+1$) isolates exactly the two configurations $\alpha_s$ and its
pair-spin-swap, giving $\operatorname{repr}(v,\alpha_s)=0\Leftrightarrow
\operatorname{repr}(v,\alpha_{\mathrm{swap}})=0$
(\texttt{flatBand\_ground\_repr\_alphaSpinOcc\_swap\_iff}). This coordinate-vanishing
then propagates across every configuration of equal up-count (induction on the inversion
weight, each adjacent $(1,0)\to(0,1)$ transposition preserving the up-count), so the
coordinate functional at the sorted representative is injective on the block and
$\operatorname{finrank}(\text{block})\leq 1$. Feeding this into
\texttt{flatBand\_groundSubmodule\_eq\_multipletSpan\_of\_blocks} yields the
\emph{axiom-free} equality \texttt{flatBand\_theorem\_11\_11\_groundSubmodule\_eq\_multipletSpan}
(and the maximal-spin corollary), depending only on
\texttt{propext}/\texttt{Classical.choice}/\texttt{Quot.sound}; the former classification
axiom \texttt{flatBand\_zeroEnergy\_halfFilled\_mem\_ferromagneticMultipletSpan} is
removed. Tasaki Theorem 11.11 is thus fully proved.
...
$\mu_z\sim\mu_{z'}$ when they share a nonzero component).

\textbf{Lemma 11.16 is now discharged (axiom-free)} by elementary linear algebra
(\texttt{generalFlatBand\_lemma\_11\_16}).  The construction: the restricted coordinate
functionals $\mathrm{fc}_x = (\texttt{EuclideanSpace.proj}_x)\!\restriction_{\ker T}$
\emph{separate points} of the finite-dimensional $h_0=\ker T$ (a flat-band vector with all
coordinates zero is zero), so they span the dual $(\ker T)^\ast$; a $D_0$-element subset $I$
extracted from them (\texttt{generalFlatBand\_exists\_special\_index}) is a basis of
$(\ker T)^\ast$.  Its dual basis lives in $(\ker T)^{\ast\ast}$, and reflexivity of the
finite-dimensional $\ker T$ (\texttt{Module.evalEquiv}) transports it back to a
\emph{predual} basis $\{\mu_z\}_{z\in I}$ of $\ker T$ itself.  The biorthogonality
$\mathrm{fc}_x(\mu_z)=\delta_{xz}$ (\texttt{Module.Basis.dualBasis\_apply\_self} after
\texttt{Module.apply\_evalEquiv\_symm\_apply}) reads off as $\mu_z(x)=\delta_{xz}$, giving both
site-localisation ($\mu_z(z)=1\ne0$, $\mu_z(z')=0$) and linear independence; membership of each
$\mu_z$ in $\ker T$ gives $T\,\mu_z=0$.  This does not even use $T\ge0$.

The remaining two, \texttt{tasaki\_theorem\_11\_15} and \texttt{generalFlatBand\_theorem\_11\_17}
(the basis connectivity is \texttt{generalFlatBandBasisGraph}/%
\texttt{generalFlatBandBasisConnected}), are still recorded as documented axioms, matching the
policy for Theorems 11.8 / 11.13; their proofs are deferred to later work.

\medskip\noindent\textbf{Theorem 11.17 discharge, operator-algebraic foundation (Tasaki
§11.3.4, Issue \#4363, PR1, \texttt{GeneralFlatBandOperators.lean}).}  Tasaki's proof of
Theorem 11.17 mirrors the fully-proved Theorem 11.11 (the $\nu$-chain flat band), generalised
from the concrete delta-chain modes to an \emph{arbitrary} special basis $\{\mu_z\}_{z\in I}$ of
$\ker T$ (Lemma 11.16).  The first PR builds the operator-algebraic premise side.  The
\emph{general flat-band creation/annihilation operators} are the smeared operators
$\hat a^\dagger_{z,\sigma} := \hat C^\dagger_\sigma(\mu_z)$,
$\hat a_{z,\sigma} := \hat C_\sigma(\mu_z)$
(\texttt{generalFlatBandCreation}/\texttt{Annihilation}, with
$\hat C^\dagger_\sigma(\varphi)=\sum_x \varphi(x)\,\hat c^\dagger_{x,\sigma}$).  Their adjoints
$(\hat C^\dagger_\sigma(\varphi))^{\mathsf H}=\hat C_\sigma(\bar\varphi)$ and full
$\mathbb C$-linearity are established, together with the canonical-anticommutation algebra: the
unified site form $\{\hat c_{x,\sigma},\hat c^\dagger_{y,\tau}\}=[\,x{=}y\wedge
\sigma{=}\tau\,]$, the \emph{bilinear} smeared form $\{\hat C_\sigma(\varphi),
\hat C^\dagger_\tau(\psi)\}=\delta_{\sigma\tau}\bigl(\sum_x\varphi(x)\psi(x)\bigr)\cdot\mathbf 1$,
and the site-dual form $\{\hat c_{z,\sigma},\hat a^\dagger_{z',\tau}\}=\delta_{\sigma\tau}\,
\mu_{z'}(z)\cdot\mathbf 1$.  From these one obtains the $\hat a^\dagger$-monomial Slater states and
their span (the right-hand side of eq.~(11.3.46)), the single- and double-site annihilation
\emph{peel} formulae (the eq.~(11.3.48) engine), the doubled annihilator
$\hat c_{x,\downarrow}\hat c_{x,\uparrow}$ together with the double-occupancy Gram identity
$\hat n_{x,\uparrow}\hat n_{x,\downarrow}=(\hat c_{x,\downarrow}\hat c_{x,\uparrow})^{\mathsf H}
(\hat c_{x,\downarrow}\hat c_{x,\uparrow})$, and the kinetic-operator positivity: writing
$T=C^{\mathsf H}C$ gives the Gram-sum factorisation
$\hat H_{\rm hop}=\sum_\sigma\sum_k (\hat C_\sigma(C_k))^{\mathsf H}\hat C_\sigma(C_k)\ge0$.
Consequently a flat-band ground state at filling $N=D_0$ (zero Rayleigh quotient, $U>0$) has
\emph{no double occupancy} ($\hat n_{x,\uparrow}\hat n_{x,\downarrow}\Phi=0$,
$\hat c_{x,\downarrow}\hat c_{x,\uparrow}\Phi=0$) and is annihilated by $\hat C_\sigma(\bar w)$ for
\emph{every} $w\in\operatorname{range}T$.  (The smeared annihilator is $\mathbb C$-\emph{linear},
so the exact killing space is the row span of the positive square root $C$ of $T=C^{\mathsf H}C$,
which equals $\overline{\operatorname{range}T}$: writing $b=Cw$, the identity
$\overline{(Tw)_x}=\sum_k C_{kx}\overline{b_k}$ exhibits $\overline{Tw}$ as a row combination, giving
$\hat C_\sigma(\overline{Tw})\Phi=0$; for an \emph{orthonormal} eigenbasis the operator detecting
occupation of a nonzero-eigenvalue mode $u$ is exactly $\hat C_\sigma(\bar u)$, so this is the form
the Fock-spanning step consumes.)  The single-particle space is split by
the Hermitian spectral theorem (\texttt{IsHermitian.eigenvectorBasis}): zero-eigenvalue
eigenvectors lie in the flat band $\ker T$, nonzero-eigenvalue eigenvectors lie in
$\operatorname{range}T$, and every $v$ decomposes as $v_{\ker}+v_{\rm range}$
(\texttt{exists\_ker\_add\_range\_decomp}).  (These ground-state-annihilation and spectral results
were later split into \texttt{GeneralFlatBandGroundAnnihilation.lean} in a build-speed refactor.)
This is exactly the operator-algebraic premise of the
Fock-spanning eq.~(11.3.46); the remaining Fock-space spanning, spin representation
(eq.~(11.3.47)), sign relation (eq.~(11.3.49)) and connectivity capstone are the subsequent PRs.

\medskip\noindent\textbf{Theorem 11.17 PR2, the general occupation basis (Tasaki §11.3.4,
Issue \#4363, \texttt{GeneralFlatBandModeMonomial.lean} + \texttt{GeneralFlatBandOccBasis.lean}).}
The right-hand side of eq.~(11.3.46) is a span of Fock monomials, so the proof needs a Fock
\emph{basis} adapted to a single-particle basis.  Tasaki's argument reuses the fully-proved
Theorem 11.11 occupation-basis machinery, which is in fact \emph{generic in the single-particle
basis}; PR2 re-develops it for an arbitrary basis $e$ of the $M{+}1$ modes (to be instantiated at the
spectral eigenbasis of $T$).  First the smeared creators anticommute,
$\{\hat C^\dagger_\sigma(\varphi),\hat C^\dagger_\tau(\psi)\}=0$ (bilinear double sum of the site
anticommutations), whence $(\hat C^\dagger_\sigma(\varphi))^2=0$.  The \emph{general-basis Fock
monomials} are ordered products $\prod_{(i,\sigma)\in qs}\hat C^\dagger_\sigma(e_i)$ on the vacuum;
their span is all of Fock space, because it is invariant under each mode creation, hence (writing a
site creation $\hat c^\dagger_{x,\sigma}=\sum_i (e.\mathrm{repr}\,\delta_x)_i\,\hat
C^\dagger_\sigma(e_i)$ via the basis expansion) under every \emph{site} creation, and every
computational basis vector is an ordered product of site creations on the vacuum.  Reordering the
creation list scales a monomial by a nonzero sign (induction on the permutation; a transposition
flips the sign through the anticommutation), and a repeated mode kills it ($(\hat C^\dagger)^2=0$);
so the \emph{occupation monomials} (products over the sorted occupied modes of a configuration
$f$) still span $\top$, and there are $2^{2(M+1)}=\dim$ of them, giving the \textbf{general
occupation basis} (\texttt{generalOccBasis}).  This is the Fock-space scaffolding for the
eq.~(11.3.46) spanning step of the next PR.

\medskip\noindent\textbf{Theorem 11.17 PR3, the spectral-eigenbasis annihilation peel (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandEigenbasis.lean}).}  To detect which modes a ground
state occupies, the occupation basis is built on the \emph{orthonormal eigenbasis} of the hopping
matrix.  The eigenbasis $T.\mathtt{IsHermitian.eigenvectorBasis}$ (on
$\mathrm{EuclideanSpace}\,\mathbb C\,(\mathrm{Fin}(M{+}1))$) transports to a \texttt{Module.Basis}
of the plain coordinate space $\mathrm{Fin}(M{+}1)\to\mathbb C$ (\texttt{eigenbasisAsBasis}, via
\texttt{OrthonormalBasis.toBasis} and \texttt{WithLp.linearEquiv}).  Its coordinate orthonormality
$\sum_x \overline{e_j(x)}\,e_k(x)=\delta_{jk}$ collapses the bilinear CAR coefficient, giving the
\emph{dual canonical anticommutation relation}
$\{\hat C_\sigma(\bar e_j),\hat C^\dagger_\tau(e_k)\}=\delta_{jk}\delta_{\sigma\tau}\cdot\mathbf 1$.
Together with the smeared-annihilator vacuum kill $\hat C_\sigma(\varphi)|\mathrm{vac}\rangle=0$,
moving $\hat C_\sigma(\bar e_j)$ rightward through a mode monomial yields the
\textbf{eigenbasis-annihilation peel}
$\hat C_\sigma(\bar e_j)\,|qs\rangle = \sum_i (-1)^i\,\delta_{j,(qs_i)_1}\delta_{\sigma,(qs_i)_2}\,
|qs\setminus i\rangle$ — the operator removes the mode $(j,\sigma)$ wherever it occurs (Koszul
sign), and annihilates a monomial not containing it.  This is the occupation-detection engine for
the eq.~(11.3.46) hard direction (a flat-band ground state, annihilated by every nonzero-eigenvalue
mode $\hat C_\sigma(\bar e_j)$, has occupation only on the flat band), completed in the next PR.

\medskip\noindent\textbf{Theorem 11.17 PR4, eq.~(11.3.46) Fock spanning, hard direction (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandSpanning.lean}).}  The flat-band ground state at
filling is now shown to be a superposition of flat-band occupation monomials.  The transported
eigenbasis satisfies $T\,e_j=\lambda_j e_j$, so the precise range-$T$ annihilation (PR1) gives
$\hat C_\sigma(\bar e_j)\Phi=0$ for every nonzero-eigenvalue mode $j$.  Form the \emph{eigenmode
number operator} $\hat n_{j,\sigma}=\hat C^\dagger_\sigma(e_j)\hat C_\sigma(\bar e_j)$; from the dual
CAR and the creation anticommutation it obeys
$\hat n_{j,\sigma}\hat C^\dagger_\tau(e_k)=\delta_{jk}\delta_{\sigma\tau}\hat C^\dagger_\tau(e_k)
+\hat C^\dagger_\tau(e_k)\hat n_{j,\sigma}$, so by induction on a mode monomial it is
\emph{diagonal} in the occupation basis, $\hat n_{j,\sigma}\,\mathrm{occMon}(g)=g(j,\sigma)\,
\mathrm{occMon}(g)$.  Since $\hat n_{j,\sigma}\Phi=\hat C^\dagger_\sigma(e_j)(\hat C_\sigma(\bar
e_j)\Phi)=0$ for nonzero-eigenvalue $j$, expanding $\Phi=\sum_g c_g\,\mathrm{occMon}(g)$ in the
occupation basis and reading off the coefficient of $g$ gives $c_g\,g(j,\sigma)=0$; hence the
\textbf{coefficient vanishes}, $c_g=0$, whenever $g$ occupies a nonzero-eigenvalue mode.  Therefore
$\Phi$ lies in the span of the \emph{flat-supported} occupation monomials (those occupying only
zero-eigenvalue modes) --- this is \texttt{flatBand\_groundState\_mem\_flatFockSpan}, eq.~(11.3.46).

\medskip\noindent\textbf{Theorem 11.17 PR5, filling refinement (Tasaki §11.3.4, Issue \#4363,
\texttt{GeneralFlatBandFilling.lean}).}  The same diagonalisation argument applies to the
\emph{total} fermion number $\hat N$: from the basis-independent raising relation
$[\hat N,\hat C^\dagger_\sigma(w)]=\hat C^\dagger_\sigma(w)$, induction on a mode monomial gives
$\hat N\,\mathrm{occMon}(g)=|\mathrm{occupied}(g)|\,\mathrm{occMon}(g)$, so $\hat N$ is diagonal in
the occupation basis with eigenvalue the particle count.  A $D_0$-electron ground state
($\hat N\Phi=D_0\Phi$) therefore has occupation-basis coefficients supported on the $D_0$-electron
configurations; combined with eq.~(11.3.46) this yields
\texttt{flatBand\_groundState\_atFilling\_mem\_flatFockSpan}: $\Phi$ lies in the span of the
flat-supported occupation monomials with \emph{exactly} $D_0$ occupied modes.

\medskip\noindent\textbf{Theorem 11.17 PR6, transport to the special basis (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandMuTransport.lean}).}  Eq.~(11.3.46) was proved over the spectral
eigenbasis, but the spin representation (eq.~(11.3.47)) is stated over the \emph{special basis}
$\{\mu_z\}$ (Lemma 11.16).  Since both are bases of the same flat band, the spanning transports.
Concretely: $\ker T = \operatorname{span}\{\mu_z\}$ at the coordinate level (each $\mu_z\in\ker T$,
they are independent and number $D_0=\dim\ker T$; the $\mathrm{WithLp}$ identification transports
the finrank of $\ker T$ between $\mathrm{EuclideanSpace}$ and the plain coordinate space).  The
$\mu$-Slater Fock submodule $\mathtt{generalFlatBandFockSubmodule}\ \mu$ is invariant under
$\hat C^\dagger_\sigma(w)$ for every $w\in\operatorname{span}\{\mu_z\}$ (a combination of the mode
creators $\hat a^\dagger_{\mu_z,\sigma}$, each of which prepends a mode to a Slater list), and
contains the vacuum.  A flat-supported eigenbasis occupation monomial uses only flat eigenvectors,
each lying in $\ker T=\operatorname{span}\{\mu_z\}$, so it lies in the $\mu$-Slater submodule; hence
so does the ground state --- \texttt{flatBand\_groundState\_mem\_generalFlatBandFockSubmodule}.

\medskip\noindent\textbf{Theorem 11.17 PR7, index-mode number operators toward eq.~(11.3.47)
(Tasaki §11.3.4, Issue \#4363, \texttt{GeneralFlatBandSpinConfig.lean}).}  On the index set $I$ the
special basis is localised ($\mu_{z'}(z)=\delta_{zz'}\mu_z(z)$), so the site annihilator $\hat
c_{z,\sigma}$ ($z\in I$) is dual to the mode creator $\hat a^\dagger_{\mu_z,\sigma}$:
$\{\hat c_{z,\sigma},\hat a^\dagger_{\mu_{z'},\tau}\}=\delta_{\sigma\tau}\delta_{zz'}\mu_z(z)$.  The
index-mode number operator $\hat n^\mu_{z,\sigma}=\hat a^\dagger_{\mu_z,\sigma}\hat c_{z,\sigma}$
obeys the creation commutation $[\hat n^\mu_{z,\sigma},\hat a^\dagger_{\mu_{z'},\tau}]
=\delta_{\sigma\tau}\delta_{zz'}\mu_z(z)\,\hat a^\dagger_{\mu_{z'},\tau}$, so by induction it is
diagonal in the $\mu$-Slater states, $\hat n^\mu_{z,\sigma}|qs\rangle=\mu_z(z)\,(\#(z,\sigma)\text{
in }qs)\,|qs\rangle$.  The operator identity $\hat n^\mu_{z,\uparrow}\hat n^\mu_{z,\downarrow}
=\hat a^\dagger_{\mu_z,\uparrow}\hat a^\dagger_{\mu_z,\downarrow}\,(\hat c_{z,\downarrow}\hat
c_{z,\uparrow})$ together with the zero-energy condition $\hat c_{z,\downarrow}\hat
c_{z,\uparrow}\Phi=0$ ($\hat H_{\rm int}\Phi=0$) gives \textbf{no double occupancy of the index
modes}, $\hat n^\mu_{z,\uparrow}\hat n^\mu_{z,\downarrow}\Phi=0$.

\medskip\noindent\textbf{Theorem 11.17 PR8, the tight $I$-mode transport (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandSpinRep.lean}).}  To extract the occupation coefficients we need a
basis adapted to the index modes.  The special basis $\{\mu_z\}_{z\in I}$ is linearly independent, so
it extends to a basis $e_\mu$ of the whole single-particle space (\texttt{exists\_extended\_special\_%
basis}, via \texttt{Module.Basis.extend}); each $\mu_z$ is some $e_\mu(\mathrm{idx}\,z)$, so an
$I$-mode $\mu$-Slater state equals a mode monomial over $e_\mu$ (\texttt{generalFlatBandSlaterState\_%
eq\_generalModeMonomial}), carrying it into the general occupation basis $\texttt{generalOccBasis}\
e_\mu$ and giving its linear independence.  Since PR6 builds a ground state only from $\ker T=%
\operatorname{span}\{\mu_z\}$ creations, it actually lives in the \emph{tight} $I$-mode $\mu$-Slater
submodule \texttt{generalFlatBandIModeFockSubmodule} (the span of $\mu$-Slater states whose modes all
lie in $I$), which is invariant under index-mode creation and contains the ground state
(\texttt{flatBand\_groundState\_mem\_imode}) --- resolving the subtlety that $\mu_z$ is unconstrained
for $z\notin I$.

\medskip\noindent\textbf{Theorem 11.17 PR9, the spin-configuration capstone, eq.~(11.3.47) (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandSpinConfigRep.lean}).}  The coefficient extraction is
carried out in the general occupation basis $\texttt{generalOccBasis}\ e_\mu$.  A left inverse
$\mathrm{idxInv}$ of the index map on $I$ (\texttt{flatBandSpecialIdxInv}, with $\mathrm{idx}$
injective on $I$ since distinct $\mu_z$ are distinct) identifies the $\mathrm{idx}(I)$-supported
occupation monomials with $I$-mode $\mu$-Slater states:
$\texttt{occMon}_{e_\mu}(g)=\prod_{z\in I}\hat a^\dagger_{\mu_z,\sigma}|vac\rangle$
(\texttt{generalOccMonomial\_eq\_generalFlatBandSlaterState\_of\_idxSupported}).  On these the double
index-mode number operator is diagonal with eigenvalue
$\mu_z(z)^2\,[g(\mathrm{idx}\,z,\uparrow){=}1]\,[g(\mathrm{idx}\,z,\downarrow){=}1]$.  Expanding
$\Phi$ in the basis, three coefficient-vanishing facts pin it down: (a) \emph{support} --- the
coefficient vanishes off the $\mathrm{idx}(I)$-supported configs (PR8 tight membership +
\texttt{Basis.repr\_support\_subset\_of\_mem\_span}); (b) \emph{no double occupancy} --- it vanishes
on configs with a doubly occupied index mode, since $\mu_z(z)^2\ne0$ and $\hat n^\mu_{z,\uparrow}\hat
n^\mu_{z,\downarrow}\Phi=0$ (PR7); (c) \emph{filling} --- it vanishes off the $D_0$-electron sector
($\hat N$ diagonal).  As $D_0=|I|$, the surviving configs occupy each index mode by exactly one spin,
so $\Phi$ lies in the span of the one-spin-per-index $\mu$-Slater states
$\Phi=\sum_\sigma C(\sigma)\prod_{z\in I}\hat a^\dagger_{\mu_z,\sigma_z}|vac\rangle$ --- this is
eq.~(11.3.47), \texttt{flatBand\_groundState\_mem\_spinConfigSpan} (sorry-free, axiom-clean).

\medskip\noindent\textbf{Theorem 11.17 PR10, eq.~(11.3.47) explicit one-spin-per-index (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandSpinConfigRep.lean}).}  The surviving spin-config
occupations occupy each index mode by exactly one spin.  Pigeonhole: the first-coordinate projection
$q\mapsto q.1$ is injective on the occupied finset (no double occupancy forbids two spins at the same
$\mathrm{idx}\,z$), so its image has $|{\rm occupied}|=D_0=|I|$ elements inside $\mathrm{idx}(I)$
(cardinality $|I|$), hence equals $\mathrm{idx}(I)$; every index mode is therefore occupied
(\texttt{flatBandSpinConfig\_occupied\_per\_index}), and with no double occupancy \emph{exactly} one
spin (\texttt{flatBandSpinConfig\_exactlyOne\_per\_index}).

\medskip\noindent\textbf{Theorem 11.17 PR11, eq.~(11.3.47) $\sigma$-parametrised form (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandSpinConfigRep.lean}).}  Eq.~(11.3.47) is restated with
the surviving states indexed by spin configurations $\sigma:\Lambda\to\{\uparrow,\downarrow\}$: the
one-spin-per-index occupation \texttt{flatBandSpinConfigOcc} (occupying exactly $(\mathrm{idx}\,z,
\sigma_z)$ for $z\in I$) and its occupation monomial \texttt{flatBandSpinConfigState}
$=\prod_{z\in I}\hat a^\dagger_{\mu_z,\sigma_z}|vac\rangle$.  Every surviving spin-config occupation
equals \texttt{flatBandSpinConfigOcc} of its spin function $\sigma_g(z)=[g(\mathrm{idx}\,z,\uparrow)
\ne1]$ (\texttt{flatBandSpinConfig\_eq\_spinConfigOcc}, from the one-spin-per-index property), so the
spin-config span of PR9 is the span of the $\sigma$-states:
$\Phi\in\operatorname{span}\{\,\texttt{flatBandSpinConfigState}\,\sigma\,\}_\sigma$
(\texttt{flatBand\_groundState\_mem\_spinConfigStateSpan}) --- the $C(\sigma)$-coefficient foundation
for the sign relation.

\medskip\noindent\textbf{Theorem 11.17 PR12, spin-config state linear independence (Tasaki §11.3.4,
Issue \#4363, \texttt{GeneralFlatBandSignPropagation.lean}).}  For the coefficients $C(\sigma)$ to be
well-defined, the spin-configuration states must be linearly independent.  Distinct spin
configurations $s:I\to\{\uparrow,\downarrow\}$ give distinct occupation configs
\texttt{flatBandSpinConfigOcc} (evaluate the config equality at $(\mathrm{idx}\,z, s_z)$ and use the
injectivity of $\mathrm{idx}$ on $I$), hence distinct elements of the basis
$\texttt{generalOccBasis}\ e_\mu$; \texttt{flatBandSpinConfigState\_linearIndependent} then follows
from \texttt{Basis.linearIndependent.comp}.

\medskip\noindent\textbf{Theorem 11.17 PR13, explicit $C(\sigma)$ sum and Slater form (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandSignPropagation.lean}).}  Two foundations for the
sign argument.  First, each spin-config state is a $\mu$-Slater state of its preimage list
(\texttt{flatBandSpinConfigState\_eq\_slaterState}, since \texttt{flatBandSpinConfigOcc} is
$\mathrm{idx}(I)$-supported), so the proved site double-annihilation peel engine
\texttt{generalFlatBand\_double\_siteAnnihilation\_peel} applies to them.  Second, since the spin
configurations on the index set $I$ form a finite type, the span membership of PR11 (narrowed to
$I$-configs, since \texttt{flatBandSpinConfigOcc} depends only on $\sigma|_I$ ---
\texttt{flatBandSpinConfigOcc\_congr}) gives an explicit finite combination
$\Phi=\sum_{s:I\to\{\uparrow,\downarrow\}} C(s)\,\texttt{flatBandSpinConfigState}\,(\mathrm{extend}\,s)$
(\texttt{flatBand\_groundState\_eq\_spinConfigStateSum}, via
\texttt{Submodule.mem\_span\_range\_iff\_exists\_fun}); indexing by $I\to\{\uparrow,\downarrow\}$
(not the redundant $\Lambda\to\{\uparrow,\downarrow\}$) makes $C(s)$ the genuine, comparison-ready
coefficients matching the linear-independence index type.

\medskip\noindent\textbf{Theorem 11.17 PR14, eq.~(11.3.48) raw form (Tasaki §11.3.4, Issue \#4363,
\texttt{GeneralFlatBandSignPropagation.lean}).}  The left-hand side of eq.~(11.3.48) is assembled in
$C(\sigma)$ form.  The site double-annihilation of a spin-config state is the double annihilation of
its $\mu$-Slater preimage list (\texttt{flatBandSpinConfigState\_cDownUp\_eq\_slaterDoubleAnnih}),
on which the proved peel engine \texttt{generalFlatBand\_double\_siteAnnihilation\_peel} expands.
Since $\hat c_{x,\downarrow}\hat c_{x,\uparrow}\Phi=0$ (the zero-energy condition) and
$\Phi=\sum_s C(s)\,\texttt{flatBandSpinConfigState}$, the $C(\sigma)$-weighted sum of
double-annihilated spin-config states vanishes for \emph{every} site $x$
(\texttt{flatBand\_cDownUp\_spinConfigSum\_eq\_zero}); for $x\in\Lambda\setminus I$ the connectivity
data $\mu_z(x)$ enters through the peel.  Collecting the $(D_0{-}2)$-occupied terms and using the
linear independence (PR12) then yields the sign relation $C(\sigma)=C(\sigma_{z_1\leftrightarrow z_2})$
(eq.~(11.3.49)) --- the subsequent PRs.

\medskip\noindent\textbf{Theorem 11.17 PR15, spin-aware peel vanishing (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandSignPropagation.lean}).}  The non-contributing peel terms are killed
up front.  If every mode of a $\mu$-Slater list has zero amplitude $\mu_{q.1}(x)=0$ at the site $x$
or the wrong spin $q.2\ne\sigma$, then $\hat c_{x,\sigma}$ annihilates it
(\texttt{generalFlatBand\_siteAnnihilation\_eq\_zero}, every peel term vanishing --- the general
analogue of the proved Theorem 11.11 \texttt{flatBand\_siteAnnihilation\_eq\_zero}).  In particular,
when $x$ connects to no index mode ($\mu_z(x)=0$ for all $z\in I$) the double annihilation kills the
spin-config state (\texttt{flatBandSpinConfigState\_cDownUp\_eq\_zero\_of\_disconnected}) --- the
trivial branch of eq.~(11.3.48), contributing no sign relation.

\medskip\noindent\textbf{Theorem 11.17 PR16, $\mu$-Slater permutation scaling (Tasaki §11.3.4,
Issue \#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  To compare list orderings (the opaque
preimage list vs.\ a canonical order) when collecting peel terms, a $\mu$-Slater state changes only
by a tracked sign under reordering: swapping the first two creations negates the state
(\texttt{generalFlatBandSlaterState\_swap}, the two leading $\hat a^\dagger$ anticommute), and a
general permutation of the creation list scales it by a nonzero $\pm1$
(\texttt{generalFlatBandSlaterState\_perm}, the \texttt{generalFlatBandSlaterState} analogue of
\texttt{generalModeMonomial\_perm}).

\medskip\noindent\textbf{Theorem 11.17 PR17, the canonical spin-config list (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  The orbital-ordered creation list of a spin
configuration is \texttt{flatBandSpinConfigList}~$=$ the sorted $(z,\sigma_z)$ for $z\in I$ (the
general-basis analogue of the Theorem 11.11 \texttt{flatBandAlphaSpinList}).  Its occupied finset is
$\{(\mathrm{idx}\,z,\sigma_z):z\in I\}$ (\texttt{flatBandSpinConfigOcc\_occFinset}); it is nodup with
the same \texttt{toFinset} as the opaque $\mu$-Slater preimage list, hence a permutation of it
(\texttt{flatBandSpinConfigList\_perm\_preimageList}).  Combined with the permutation scaling, the
spin-config state is a nonzero-sign multiple of the canonical-list Slater state
(\texttt{flatBandSpinConfigState\_eq\_smul\_canonical}) --- the orbital-ordered form on which the
eq.~(11.3.48) double peel has explicit positions and Koszul signs.

\medskip\noindent\textbf{Theorem 11.17 PR18, the cDownUp two-head extraction (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  On the orbital-ordered form, site
annihilation removes a leading matching-spin head: $\hat c_{x,\sigma}$ applied to
$(z,\sigma)\!::\!\text{rest}$ (with \text{rest} carrying no matching-spin mode connected to $x$)
returns $\mu_z(x)\cdot\text{Slater}(\text{rest})$ (\texttt{generalFlatBand\_siteAnnihilation\_head},
the general analogue of \texttt{flatBand\_siteAnnihilation\_head}).  Composing the two annihilations,
$\hat c_{x,\downarrow}\hat c_{x,\uparrow}$ removes the leading up head $(a,\uparrow)$ and down head
$(b,\downarrow)$, leaving $\mu_a(x)\,\mu_b(x)\,\text{Slater}(\text{rest})$
(\texttt{generalFlatBand\_cDownUp\_two\_head}, the analogue of \texttt{flatBand\_cDownUp\_two\_head})
--- the seed of the eq.~(11.3.48) sign relation.

\medskip\noindent\textbf{Theorem 11.17 PR19, the cDownUp swapped two-head (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  On the swapped down--up head pair, the
extraction carries the opposite sign: $\hat c_{x,\downarrow}\hat c_{x,\uparrow}$ on
$(a,\downarrow)\!::\!(b,\uparrow)\!::\!\text{rest}$ gives
$-\mu_a(x)\,\mu_b(x)\,\text{Slater}(\text{rest})$ (\texttt{generalFlatBand\_cDownUp\_two\_head\_swap},
via \texttt{generalFlatBandSlaterState\_swap} composed with the two-head extraction).  This relative
$-1$ versus the canonical up--down assignment (one extra Koszul transposition) is exactly the seed of
the eq.~(11.3.49) sign relation $C(\sigma)=C(\sigma_{z_1\leftrightarrow z_2})$ (the general-basis
analogue of the Theorem 11.11 \texttt{flatBand\_cDownUp\_swap}).

\medskip\noindent\textbf{Theorem 11.17 PR20, the Slater move-pair-front (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  To extract an \emph{arbitrary} occupied pair
(not just a leading one), bring it to the head.  Moving the head past the next two creations is sign
$+1$ (two adjacent transpositions, \texttt{generalFlatBandSlaterState\_move\_one\_past\_two}); by
induction, pushing a contiguous pair $a,b$ leftward past a block $l_1$ is $2|l_1|$ transpositions,
hence $\text{Slater}(l_1 \mathbin{+\!+} a\!::\!b\!::\!l_2) = \text{Slater}(a\!::\!b\!::\!(l_1
\mathbin{+\!+} l_2))$ (\texttt{generalFlatBandSlaterState\_move\_pair\_front}, the general-basis
analogue of \texttt{flatBandModeMonomial\_move\_pair\_front}).  Combined with the two-head/swapped
extraction, this gives the $\pm\mu_a(x)\mu_b(x)$ contribution of an arbitrary pair.

\medskip\noindent\textbf{Theorem 11.17 PR21, extracting a pair at an arbitrary position (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  Composing the move-pair-front
with the two-head extraction, $\hat c_{x,\downarrow}\hat c_{x,\uparrow}$ on
$l_1 \mathbin{+\!+} (a,\uparrow)\!::\!(b,\downarrow)\!::\!l_2$ (with $l_1,l_2$ disconnected from $x$)
removes the pair with amplitude $\mu_a(x)\mu_b(x)$, leaving $\text{Slater}(l_1 \mathbin{+\!+} l_2)$
(\texttt{generalFlatBand\_cDownUp\_extract\_pair}); the swapped assignment $(a,\downarrow),(b,\uparrow)$
extracts with the opposite sign $-\mu_a(x)\mu_b(x)$
(\texttt{generalFlatBand\_cDownUp\_extract\_pair\_swap}) --- the per-pair eq.~(11.3.49) relative
$-1$.

\medskip\noindent\textbf{Theorem 11.17 PR22, the canonical-Slater $D$-coefficient expansion (Tasaki
§11.3.4, Issue \#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  The sign-relation comparison
of $\sigma$ and $\sigma_{z_1\leftrightarrow z_2}$ must be run in a single, order-fixed coefficient
system.  The \texttt{flatBandSpinConfigState} coefficients carry an existential $\pm1$ canonical sign
(\texttt{flatBandSpinConfigState\_eq\_smul\_canonical}); instead, since the canonical-list Slater
states span the same space, $\Phi$ has an explicit expansion in them,
$\Phi=\sum_s D(s)\,\text{Slater}(\texttt{flatBandSpinConfigList}\,\sigma_s)$
(\texttt{flatBand\_groundState\_eq\_canonicalSlaterSum}, indexed by $s:I\to\{\uparrow,\downarrow\}$).
The $D(s)$ are the order-fixed coefficients of eqs.~(11.3.47)--(11.3.49): in the canonical sorted
order an index pair $a,b$ sits at the same positions $\mathrm{rank}(a),\mathrm{rank}(b)$ for both
$\sigma$ and $\sigma_{a\leftrightarrow b}$, so the move-to-front permutation sign is shared and the
relative $-1$ survives cleanly.  Reindexing the full double peel by removed pair and collecting via
the occupation-basis coordinate functional and linear independence is the subsequent PR.  (The
reorder/extraction machinery above --- the Slater swap/permutation, the canonical list, the
head/two-head/swap extraction, the move-pair-front, the per-pair extraction, and the
canonical-Slater $D$-coefficient expansion --- lives in \texttt{GeneralFlatBandSlaterReorder.lean},
split from \texttt{GeneralFlatBandSignPropagation.lean} for build speed.  The canonical-list
coordinate/config machinery built on top of it (the list length and per-mode shape, the
occupation-basis coordinate functional and its distribution over the positional double-peel sum,
the index-configuration \texttt{idxConfigOf} and its erase-tracking, the position$\leftrightarrow$index
correspondence, and the erase-to-canonical bridge \texttt{flatBandSpinConfigList\_eraseIdx}) was in
turn split into \texttt{GeneralFlatBandCanonicalCoord.lean} for build speed.  The occupation-basis coordinate functional distributes over a single inner peel term as $\texttt{repr}(\text{peelTerm})\,g = (-1)^i\,[qs_i{=}s]\,\mu_{qs_i}(x)\,\texttt{repr}(\text{Slater}(qs.\texttt{eraseIdx}\,i))\,g$ (\texttt{generalFlatBandPeelTerm\_repr}), so the inner $j$-sum collects term-wise to the bridge coordinate of the double-erased rest state, whose list is itself the canonical creation list over the twice-erased index set $(I\setminus\{a\})\setminus\{b\}$ (\texttt{flatBandSpinConfigList\_eraseIdx\_eraseIdx}).  Since this twice-erased set contains neither $a$ nor $b$ and $\sigma$ agrees with the spin-swapped $\sigma_{a\leftrightarrow b}$ off $\{a,b\}$, the rest list is identical for the two configurations (\texttt{flatBandSpinConfigList\_congr}), so the shared rest coefficient --- nonzero by evaluating the bridge coordinate at the rest config itself (\texttt{generalFlatBandSlaterState\_repr\_self\_ne\_zero}) --- cancels in the comparison $D(\sigma)=D(\sigma_{a\leftrightarrow b})$.  The "exactly one $(i,j)$" claim rests on the fact that the index config of a canonical creation list determines its index set (\texttt{idxConfigOf\_flatBandSpinConfigList\_inj}, by evaluating the config at the mode $(\mathrm{idx}\,w,\sigma w)$), so distinct double-peels empty distinct index pairs and land on distinct target configs; matching the rest set to $(I\setminus\{a\})\setminus\{b\}$ then pins the unordered removed pair to $\{a,b\}$ (\texttt{Finset.eq\_or\_eq\_of\_erase\_erase\_eq}); the opposite spin guards then disambiguate the outer index as $a$ and the inner index as $b$ (\texttt{flatBandSpinConfig\_doublePeel\_index\_eq}), the "exactly one $(i,j)$" engine.  The inner sum is a single peel, for which the removed index is pinned by the config alone (\texttt{flatBandSpinConfig\_singlePeel\_index\_eq}, via $S\setminus\{d\}=S\setminus\{b\}\Rightarrow d=b$).  Hence the inner $j$-sum collapses by \texttt{Finset.sum\_eq\_single} to a single term at the position of $b$ (off-positions give vanishing coordinates, the $b$-position carries the down-guard and rest list $\mathrm{canonical}(S\setminus\{b\})$), with Koszul sign $(-1)^{\mathrm{pos}_b}$ (\texttt{flatBandSpinConfig\_inner\_sum\_collapse}).  For an outer index $c\notin\{a,b\}$ the inner sum instead vanishes identically (\texttt{flatBandSpinConfig\_inner\_sum\_other\_outer\_zero}), the off-diagonal case of the outer collapse.  Together these give the single-term collapse $\texttt{repr}(\hat c_{x,\downarrow}\hat c_{x,\uparrow}\,\mathrm{Slater}(\sigma))\,g_{ab} = (-1)^{\mathrm{pos}_a}(-1)^{\mathrm{pos}_b}\,\mu_a(x)\,\mu_b(x)\,R$, where $R$ is the shared rest coordinate (\texttt{cDownUp\_canonical\_repr\_twoHole}).  The index at a canonical position equals the corresponding sorted-finset element (\texttt{flatBandSpinConfigList\_get\_fst\_eq\_sort}), so the position of an index is $\sigma$-independent (\texttt{flatBandSpinConfigList\_choose\_eq}), and (for $p\neq q$) the position of $b$ in $I\setminus\{a\}$ is $\mathrm{pos}_I(b)-[\mathrm{pos}_I(b)>\mathrm{pos}_I(a)]$ (\texttt{flatBandSpinConfigList\_choose\_erase\_shift}, via \texttt{List.getElem\_eraseIdx} on $I.\mathrm{sort}$), and the two single-transposition-shifted exponents $p+(q-[q>p])$ and $q+(p-[p>q])$ differ by exactly one, so the corresponding signs are negatives (\texttt{neg\_one\_pow\_two\_erase\_shift}); letting the Koszul signs of $\sigma$ and the spin-swapped $\sigma_{a\leftrightarrow b}$ be compared on the common $I.\mathrm{sort}$ positions.  Consequently the two-hole coordinate flips sign under the $a\leftrightarrow b$ spin swap, $\texttt{coord}_\sigma=-\texttt{coord}_{\sigma\circ(a\,b)}$ (\texttt{cDownUp\_canonical\_repr\_twoHole\_swap\_eq\_neg}) --- the eq.~(11.3.49) heart.  The collapse and sign machinery lives in \texttt{GeneralFlatBandTwoHoleCollapse.lean}, split from \texttt{GeneralFlatBandCanonicalCoord.lean} for build speed; the connectivity induction (from the eq.~(11.3.49) relation \texttt{flatBand\_groundState\_D\_swap\_eq} through the dimension bound \texttt{generalFlatBandGround\_finrank\_le\_of\_connected}) is in turn split into \texttt{GeneralFlatBandConnectivity.lean}, isolating its heavier \texttt{finrank} elaboration from the collapse core.  Evaluating the index config at every mode $(\mathrm{idx}\,w,s)$ refines the injectivity to a $\sigma$-varying form: equal configs force equal index sets \emph{and} matching spins (\texttt{idxConfigOf\_flatBandSpinConfigList\_inj\_gen}), the engine for the eq.~(11.3.49) sum collapse to the two configurations $\sigma$ and $\sigma_{a\leftrightarrow b}$: a contributing double peel of $\mathrm{Slater}(\tau)$ at the $(a,b)$-emptied target forces $\tau=\sigma$ or $\tau=\sigma\circ(a\,b)$ on $I$ (\texttt{flatBandSpinConfig\_doublePeel\_config\_eq}); hence a Slater coordinate at the $(a,b)$-emptied target vanishes whenever $\tau\notin\{\sigma,\sigma\circ(a\,b)\}$ (\texttt{cDownUp\_canonical\_repr\_eq\_zero\_of\_ne}); summing over all spin configs therefore collapses to the two terms $\sigma|_I$ and $(\sigma\circ(a\,b))|_I$ (\texttt{cDownUp\_canonicalSum\_eq\_two\_terms}).  Since acting with $\hat c_{x,\downarrow}\hat c_{x,\uparrow}$ kills the ground state and the two surviving coordinates are negatives of each other and nonzero (for a connected pair $\mu_a(x),\mu_b(x)\neq0$), the coefficients of $\sigma$ and $\sigma_{a\leftrightarrow b}$ coincide: $D(\sigma)=D(\sigma_{a\leftrightarrow b})$ (\texttt{flatBand\_groundState\_D\_swap\_eq}), completing the eq.~(11.3.49) Marshall-sign relation.  On the special-basis connectivity graph (edge $z\sim z'$ iff some site $x$ has $\mu_z(x),\mu_{z'}(x)\neq0$) this gives the per-edge step $D(\sigma)=D(\sigma_{z\leftrightarrow z'})$ (\texttt{flatBand\_groundState\_D\_swap\_eq\_of\_adj}), and, since a same-spin swap is the identity on the configuration, the edge-swap invariance $D(\sigma)=D(\sigma\circ(z\,z'))$ holds unconditionally (\texttt{flatBand\_groundState\_D\_edgeSwap\_eq}); $D$ is thus invariant under every edge-transposition of the basis graph, the input to the connectivity induction toward Theorem 11.17.  Viewed as an action of $\mathrm{Perm}(I)$, $D(s)=D(s\circ(z\,z'))$ for every edge $z\sim z'$ (\texttt{flatBand\_groundState\_D\_permSwap\_eq}), so $D$ is invariant under the subgroup generated by the edge-transpositions.  By induction on a walk $z\rightsquigarrow z'$, decomposing $\mathrm{swap}(z,z')$ as the conjugate $\mathrm{swap}(z,w)\,\mathrm{swap}(w,z')\,\mathrm{swap}(z,w)$, this extends to $D(s\circ\mathrm{swap}(z,z'))=D(s)$ for all reachable $z,z'$ (\texttt{flatBand\_groundState\_D\_swap\_eq\_of\_walk}).  For a connected basis every transposition is therefore $D$-invariant, and since transpositions generate $\mathrm{Perm}(I)$, $D(s\circ\pi)=D(s)$ for all $\pi$ (\texttt{flatBand\_groundState\_D\_perm\_eq}); i.e. $D$ depends only on the number of up-spins: equal-weight configurations differ by a permutation (\texttt{exists\_perm\_comp\_of\_card\_eq}), so $D(s)=D(s')$ whenever $s,s'$ have the same up-count (\texttt{flatBand\_groundState\_D\_const\_of\_weight\_eq}).  Grouping the canonical-Slater decomposition of any ground state by up-count and factoring out the constant coefficient, every ground state lies in the span of the $D_0+1$ symmetric weight-states $W_0,\dots,W_{D_0}$, so the ground subspace has $\mathrm{finrank}\le D_0+1$ (\texttt{generalFlatBandGround\_finrank\_le\_of\_connected}).  For the matching lower bound, the all-up $\mu$-Slater state is a highest-weight vector: $\hat S^+_{\rm tot}$ annihilates it since each $\hat c_{i,\downarrow}$ does (every creation mode is spin-$\uparrow$, \texttt{generalFlatBand\_totalSpinPlus\_mulVec\_allUpSlater}), so \texttt{highestWeight\_spinMultiplet\_general} will generate the $D_0+1$-fold SU(2) tower.  The all-up Slater has $\hat N_{\rm tot}$ eigenvalue $D_0$ (the total number operator is diagonal on the $\mu$-Slater states, $\hat N_{\rm tot}|\mathrm{Slater}(qs)\rangle=|qs|\,|\mathrm{Slater}\rangle$, \texttt{fermionTotalNumber\_mulVec\_generalFlatBandSlaterState}), the filling input toward the weight $\hat S^z_{\rm tot}=D_0/2$.  Splitting the $2M{+}2$ modes into sites and spins via \texttt{sum\_spinful\_reindex} gives the charge decomposition $\hat N_{\rm tot}=\hat N_\uparrow+\hat N_\downarrow$ (\texttt{fermionTotalNumber\_eq\_up\_add\_down}); since $\hat c_{i,\downarrow}$ annihilates the all-up Slater, $\hat N_\downarrow|\mathrm{Slater}\rangle=0$ (\texttt{fermionTotalDownNumber\_mulVec\_allUpSlater}), whence $\hat N_\uparrow|\mathrm{Slater}\rangle=\hat N_{\rm tot}|\mathrm{Slater}\rangle=D_0|\mathrm{Slater}\rangle$ and $\hat S^z_{\rm tot}=(\hat N_\uparrow-\hat N_\downarrow)/2$ gives the highest weight $\hat S^z_{\rm tot}|\mathrm{Slater}\rangle=(D_0/2)|\mathrm{Slater}\rangle$ (\texttt{generalFlatBand\_totalSpinZ\_mulVec\_allUpSlater}), the second SU(2) highest-weight input for \texttt{highestWeight\_spinMultiplet\_general}.  Its occupation-basis self-coordinate is nonzero, so the all-up Slater is itself nonzero (\texttt{generalFlatBandSlaterState\_allUp\_ne\_zero}), the $\mathbf{v}\neq0$ nontriviality input.  For the remaining input $\mathbf{v}\in$ ground subspace, the kinetic part of $\hat H=\hat K+U\,\hat n_\uparrow\hat n_\downarrow$ factors as $\hat K=\sum_{\sigma,k}\hat C_\sigma(C_k)^\dagger\hat C_\sigma(C_k)$ for $T=C^\dagger C$, and the smeared annihilation $\hat C_\sigma(\varphi)=\sum_x\varphi(x)\hat c_{x,\sigma}$ peels to $\sum_i(-1)^i[\sigma{=}q_{i,2}]\big(\sum_x\varphi(x)\mu_{q_{i,1}}(x)\big)|\mathrm{Slater}(\mathrm{erase}\,i)\rangle$, so $\hat C_\sigma(\varphi)$ annihilates a $\mu$-Slater whenever every occupied mode is $\varphi$-orthogonal, $\sum_x\varphi(x)\mu_{q,1}(x)=0$ (\texttt{spinfulAnnihilationFromVector\_mulVec\_generalFlatBandSlaterState\_eq\_zero\_of\_orthogonal}); since $\mu_z\in\ker T=\ker C$ gives $C\mu_z=0$, i.e. $\sum_x C_k(x)\mu_z(x)=0$, each $\hat C_\sigma(C_k)$ kills the all-up Slater and hence $\hat K$ does.  Thus $\hat K|\mathrm{Slater}\rangle=0$ (\texttt{hubbardKinetic\_mulVec\_allUpSlater\_eq\_zero}); the interaction $U\sum_i\hat n_{i\uparrow}\hat n_{i\downarrow}$ also annihilates it (no double occupancy, \texttt{hubbardOnSiteInteraction\_mulVec\_allUpSlater\_eq\_zero}), so $\hat H|\mathrm{Slater}\rangle=0$; with $\hat N_{\rm tot}|\mathrm{Slater}\rangle=D_0|\mathrm{Slater}\rangle$ this places the all-up $\mu$-Slater in the ground submodule $\ker\hat H\cap\{N{=}D_0\}$ (\texttt{generalFlatBandSlaterState\_allUp\_mem\_groundSubmodule}), exhibiting the SU(2) highest-weight vector inside the ground subspace.  The lowering operator $\hat S^-_{\rm tot}$ preserves this ground submodule: it commutes with $\hat H$ (SU(2) symmetry, \texttt{fermionTotalSpinMinus\_commute\_hubbardHamiltonian}) and with $\hat N_{\rm tot}$ (\texttt{fermionTotalSpinMinus\_commute\_fermionTotalNumber}), so $\hat S^-_{\rm tot}$ maps $\ker\hat H\cap\{N{=}D_0\}$ into itself (\texttt{fermionTotalSpinMinus\_mulVec\_mem\_generalFlatBandGroundSubmodule}); hence the entire SU(2) tower $(\hat S^-_{\rm tot})^k|\mathrm{Slater}\rangle$ lies inside the ground subspace, the $D_0{+}1$ independent ground states for the lower bound.  Lifting this independent family into the ground submodule and counting (\texttt{LinearIndependent.of\_comp} then \texttt{fintype\_card\_le\_finrank}, with the extended single-particle basis $e_\mu$ and index map supplied by \texttt{exists\_extended\_special\_basis}) gives the \emph{unconditional} dimension lower bound $\mathrm{finrank}\,(\text{ground})\ge D_0+1$ (\texttt{generalFlatBandGround\_finrank\_ge}), valid for any special basis; combined with the connected upper bound \texttt{generalFlatBandGround\_finrank\_le\_of\_connected} it pins $\mathrm{finrank}=D_0+1$ on a connected basis.)

\medskip\noindent\textbf{Theorem 11.17 PR23, the canonical double peel (Tasaki §11.3.4, Issue
\#4363, \texttt{GeneralFlatBandSlaterReorder.lean}).}  The reindexing starts from the explicit
expansion: $\hat c_{x,\downarrow}\hat c_{x,\uparrow}$ on $\text{Slater}(\texttt{flatBandSpinConfigList}\,
\sigma)$ equals the position double-sum over the orbital-ordered canonical list
(\texttt{cDownUp\_canonical\_eq\_doublePeel}, via the proved engine
\texttt{generalFlatBand\_double\_siteAnnihilation\_peel}).  Its $(i,j)$ terms are to be collected by
the removed index pair, then projected by the occupation-basis coordinate functional, in the
subsequent PRs.  (Structural support: the canonical list has length $|I|$ and each of its modes is
$(z,\sigma_z)$ --- so the double-peel spin guard $[q].2=\uparrow$ reads as a condition on $\sigma$
of the index: \texttt{flatBandSpinConfigList\_length}, \texttt{flatBandSpinConfigList\_mem\_snd\_eq}.)  The projection itself is the Kronecker delta $\langle\texttt{generalOccBasis}\,e_\mu\rangle.\mathrm{repr}\,(\texttt{occMon}_{e_\mu}\,h)\,g=[h=g]$ (\texttt{generalOccMonomial\_repr}, since $\texttt{occMon}_{e_\mu}\,h$ is the basis vector).  And a one-erased canonical list keeps its modes of the form $(z,\sigma_z)$ and stays nodup (\texttt{flatBandSpinConfigList\_eraseIdx\_mem\_snd\_eq}, \texttt{flatBandSpinConfigList\_eraseIdx\_nodup}), so the $(D_0{-}1)$/$(D_0{-}2)$-electron states from the double peel are spin-config lists over a smaller index set.  Each canonical-list position is pinned to its index --- the mode at position $i$ is $(z_i,\sigma_{z_i})$ for the $i$-th smallest index $z_i\in I$ (\texttt{flatBandSpinConfigList\_getElem}).  Applying the occupation-basis coordinate functional $\langle\texttt{generalOccBasis}\,e_\mu\rangle.\mathrm{repr}\,\cdot\,g$ to the canonical double peel distributes by linearity over the position double-sum, leaving the coordinates of the doubly-erased $(D_0{-}2)$-Slater states weighted by the peel amplitudes and Koszul signs (\texttt{cDownUp\_canonical\_repr\_eq\_sum}); the removed-pair identification then picks out, for a fixed $(D_0{-}2)$-target $g$, the unique contributing $(i,j)$.  Each such coordinate is computed by \texttt{generalFlatBandSlaterState\_over\_I\_repr}: for a nodup list of index modes, $\langle\texttt{generalOccBasis}\,e_\mu\rangle.\mathrm{repr}\,(\text{Slater}\,qs)\,g=z\cdot[\mathrm{config}(qs)=g]$ for a nonzero sign $z$ (via the $\mu$-Slater$\leftrightarrow$mode-monomial bridge, permutation scaling, and the occupation-monomial coordinate).  The supporting generic fact $(\,l.\mathrm{eraseIdx}\,i).\mathrm{toFinset}=l.\mathrm{toFinset}.\mathrm{erase}\,l[i]$ for nodup $l$ (\texttt{List.toFinset\_eraseIdx\_of\_nodup}) tracks which mode each double-peel \texttt{eraseIdx} removes: the $\mathrm{idx}$-image occupation config \texttt{idxConfigOf} of a one-erased list zeroes the entry at the removed mode $(\mathrm{idx}\,qs[i].1,qs[i].2)$ (\texttt{idxConfigOf\_eraseIdx}); a double erase zeroes the config at both removed modes (\texttt{idxConfigOf\_eraseIdx\_eraseIdx}), giving the config of every $(D_0{-}2)$-Slater state from the double peel.  The canonical list\textquotesingle s own \texttt{idxConfigOf} is the spin-config occupation \texttt{flatBandSpinConfigOcc} (\texttt{idxConfigOf\_flatBandSpinConfigList}), connecting back to the established occupation machinery.  Distinct positions carry distinct indices (\texttt{flatBandSpinConfigList\_get\_fst\_inj}) --- the injectivity behind \textquotedblleft exactly one $(i,j)$ per removed pair\textquotedblright; each index of $I$ occurs at a unique position (\texttt{flatBandSpinConfigList\_mem}, \texttt{flatBandSpinConfigList\_existsUnique\_pos}).  Erasing a position of the canonical creation list yields the canonical list of the smaller index set (\texttt{Finset.sort\_eraseIdx\_eq\_sort\_erase}), so the $(D_0-2)$-electron rest states reuse the same canonical machinery; indeed erasing a canonical-list position returns the canonical list of the erased index set (\texttt{flatBandSpinConfigList\_eraseIdx}).  The double-annihilation expansion and sign relation
(eqs.~(11.3.48)/(11.3.49)) toward the connectivity capstone are the subsequent PRs.
...
Theorem 11.20, proved modulo Lemmas 11.21/11.22).}  The culmination of §11.4 is a Hubbard model with a
...
\textbf{Theorem 11.20} (\texttt{tasaki\_theorem\_11\_20}, \emph{now a proved theorem}, $d=1$,
$1\le K$): for every $\nu>0$ there are thresholds, uniform in the system size, such that once $t/s$
and $U/s$ are large enough the model satisfies \texttt{exhibitsFerromagnetism} (saturated
ferromagnetism, $S_{\max}=N/2$).  The discharge (in \texttt{NonsingularFerromagnetism.lean},
downstream of the $\hat h_p$ lemmas) is the book-faithful reduction: Lemma 11.22 fixes parameters
$\mathtt{lam}=c_\lambda s$, $\kappa=c_\kappa$ making every $\hat h_p\ge0$ once $t/s\ge T$, $U/s\ge V$,
and Lemma 11.21 turns that frustration-freeness into saturated ferromagnetism.  Thus Theorem 11.20
rests on the analytic Lemma 11.22 and the Theorem 11.11 classification axiom
(\texttt{flatBand\_zeroEnergy\_halfFilled\_mem\_ferromagneticMultipletSpan}); both are documented
axioms.  The proof-internal numbered lemmas are recorded:
\texttt{nonsingularLocalHamiltonian}~$K\,\nu\,s\,t\,U\,\mathtt{lam}\,\kappa\,i$ is the local
Hamiltonian $\hat h_p$ (eq.~(11.4.48), $d=1$, on $p=i$ and its neighbours,
\texttt{NonsingularLocalHamiltonian.lean}), and
\textbf{Lemma 11.21} (\texttt{nonsingular\_exhibitsFerromagnetism}, \emph{now proved}:
$\hat h_p\ge0\ \forall p \Rightarrow \texttt{exhibitsFerromagnetism}$, via Theorem 11.11) is
discharged in \texttt{NonsingularFerromagnetism.lean}, while \textbf{Lemma 11.22}
...
positivity below $S_{\max}$) remain documented axioms (genuine eigenvalue-continuity perturbation
theory mathlib lacks); Theorem 11.20 is proved from 11.21 (proved) and 11.22.
Towards discharging Lemma 11.21, the \emph{frustration-free decomposition} eq.~(11.4.46) is now
proved (\texttt{NonsingularFrustrationFree.lean}): \texttt{tasakiNonsingular\_eq\_sum\_localHamiltonian}
shows, for every $\mathtt{lam},\kappa$,
$\hat H = \bigl(\sum_i\hat h_{p}^{(i)}\bigr) - (K{+}1)(1{+}2\nu^2)s\cdot\mathbf 1
+ \mathtt{lam}\cdot\bigl(\sum_u\hat N^\beta_u + \sum_x\hat n_{x\uparrow}\hat n_{x\downarrow}\bigr)$.
The per-site multiplicities collapse via the cyclic reindexings $\sum_p f(p\mp1)=\sum_p f(p)$
(\texttt{sum\_shift\_sub\_one}/\texttt{sum\_shift\_add\_one}) and the even/odd site split
(\texttt{sum\_deltaSite\_split}); the $\kappa$-dependence cancels by the external-site incidence
$(1-\kappa)+\kappa=1$.  Axiom-free.  This yields the operator-positivity half of Lemma 11.21
(\texttt{NonsingularFrustrationFreePos.lean}): the remainder $\sum_u\hat N^\beta_u+\sum_x\hat
n_{x\uparrow}\hat n_{x\downarrow}$ equals $\texttt{flatBandHamiltonian}~K\,\nu\,1\,1$
(\texttt{nonsingularRemainder\_eq\_flatBand}), hence is positive-semidefinite, and so when every
$\hat h_p\ge0$ and $\mathtt{lam}\ge0$, \texttt{tasakiNonsingular\_add\_const\_posSemidef} gives
$\hat H + (K{+}1)(1{+}2\nu^2)s\cdot\mathbf 1\ge 0$ (a sum of the positive-semidefinite $\hat h_p$ and
the positive $\mathtt{lam}$-remainder).  So the ground energy is $\ge -(K{+}1)(1{+}2\nu^2)s =: -C$,
achieved by the max-spin tower (\texttt{tasakiNonsingular\_sectorMinEnergy\_maxSpin\_le}: the all-up
flat-band state lies in the $S_{\max}$ sector and is an eigenvector at $-C$, needing $1\le K$).  The
\emph{strict gap} below $S_{\max}$ is then closed by an attainment argument
(\texttt{NonsingularFerromagnetism.lean}): the fixed-filling total-spin sector is the unit sphere
intersected with two matrix-eigenvalue conditions, hence \emph{compact}
(\texttt{isCompact\_eigenSphere}/\texttt{isCompact\_spinSector}), and
$\varphi\mapsto\texttt{rayleighOnVec}~\hat H~\varphi$ is \emph{continuous}
(\texttt{continuous\_rayleighOnVec}); so a nonempty lower sector attains its infimum at some
$\varphi_0$ (\texttt{lt\_iInf\_rayleigh\_of\_eigenSphere}).  There $\texttt{rayleighOnVec}\ge -C$
(frustration-free lower bound) and $\ne -C$: a state achieving $-C$ would lie in
$\ker\hat H_{\rm flat}$ (the $\mathtt{lam}$-term vanishes), hence by Theorem 11.11 carry
$S_{\max}$, contradicting $S<S_{\max}$.  An empty sector gives the junk infimum $0>-C$ (as $C>0$).
Thus $-C<E_{\min}(S)$ for every $S<S_{\max}$ while $E_{\min}(S_{\max})\le-C$, i.e.\
\texttt{exhibitsFerromagnetism} (Lemma 11.21).  Modulo Theorem 11.11.
...
\emph{(xliii) The raising operator preserves the hard-core subspace} (\texttt{TJRaiseHardcore.lean},
E3b PR5c): $\texttt{fermionTotalSpinPlus\_mulVec\_mem\_hardcore}$ ($\hat S^+_{\rm tot}$ maps the
hard-core subspace into itself, mirror of the $\hat S^-_{\rm tot}$ version via
$\texttt{fermionTotalSpinPlus\_commute\_hubbardHardcoreProjection}$) and
$\texttt{fermionTotalSpinPlus\_pow\_mulVec\_mem\_hardcore}$ (the tower $(\hat S^+)^k v$ stays
hard-core).  This keeps every tower state $(\hat S^+)^k\Phi_0$ hard-core --- the prerequisite for the
coefficient-sum recursion along the tower.  Fully axiom-free.

\emph{(xliv) The raised vector is nonzero --- the Marshall positivity crux}
(\texttt{TJRaisePositivity.lean}, E3b PR5d): $\texttt{coeffSum\_tJExpansion}$
($\texttt{coeffSum}(\texttt{tJExpansion}\,v)=\sum_s v_s$) and
$\texttt{spinPlusPow\_ne\_zero\_of\_coeffSum\_ne\_zero}$ --- a hard-core $\hat N=N_e$ vector $\Phi_0$
with $\hat N_\downarrow\Phi_0=m\Phi_0$ and nonzero coefficient sum has $(\hat S^+_{\rm tot})^m\Phi_0
\neq 0$.  Each raising step multiplies the coefficient sum by the down-count eigenvalue $m-k$
(\texttt{coeffSum\_fermionTotalSpinPlus\_eq\_of\_downEigen} with the merged hard-core/$\hat N$/$\hat
N_\downarrow$ tower lemmas), nonzero for $k<m$, so $\texttt{coeffSum}$ stays nonzero up to $k=m$.
Applied to $\Phi_0=\texttt{tJExpansion}(\mathbb C\circ v)$ ($\texttt{coeffSum}=\sum_q v_q>0$,
$m=(N_e-1)/2$) this is the Marshall non-vanishing $\Omega\neq 0$.  Fully axiom-free.

\emph{(xlv) A maximal-spin highest-weight ground state} (\texttt{TJMaximalSpinGroundState.lean}, E4
PR6a): $\texttt{tJ\_exists\_maximalSpin\_highestWeight\_groundState}$ assembles the whole E3b chain ---
for odd $N_e<N+1$ and $\tau,J>0$ there is a nonzero $\Omega$ with $\hat S^+\Omega=0$, $\hat
S^3\Omega=(N_e/2)\Omega$, $\hat H_{\rm tJ}\Omega=\mu\Omega$ at $\mu=\texttt{groundEnergyAtFilling}$.
Here $\Omega=(\hat S^+)^{(N_e-1)/2}(\texttt{tJExpansion}(\mathbb C\circ v))$ for the strictly-positive
Perron--Frobenius eigenvector $v$: non-vanishing from
$\texttt{spinPlusPow\_ne\_zero\_of\_coeffSum\_ne\_zero}$ ($\texttt{coeffSum}=\sum_q v_q>0$),
highest-weight from $\texttt{tJ\_raised\_highestWeight}$, and
$\texttt{groundEnergyAtFilling}=\mu$ from the E2 bounds.  This $\Omega$ feeds
$\texttt{highestWeight\_spinMultiplet\_general}$ for the $N_e+1$ multiplet.  Rests only on A.17's
documented axiom.

\emph{(xlvi) Ground degeneracy lower bound} (\texttt{TJGroundDegeneracyLower.lean}, E4):
$\texttt{tJ\_groundSubmodule\_finrank\_ge}$ --- the d=1 ferromagnetic t-J ground subspace at odd
filling $N_e<N+1$ has $N_e+1\le\dim$.  The maximal-spin highest weight $\Omega$ generates via
$\texttt{highestWeight\_spinMultiplet\_general}$ the $N_e+1$ linearly independent tower $(\hat
S^-)^k\Omega$; since $\hat S^-$ commutes with $\hat H_{\rm tJ}$ and $\hat N$ and preserves the
hard-core subspace, every tower member is again a ground state in
$\texttt{groundSubmoduleAtFilling}$, so $\texttt{LinearIndependent.of\_comp}$ +
$\texttt{fintype\_card\_le\_finrank}$ give the bound.  Rests only on A.17's documented axiom.

\emph{(xlvii) SU(2) ladder injectivity on weight spaces} (\texttt{TJLadderInjective.lean}, E5):
$\texttt{fermionTotalSpin\_ladder\_norm}$ --- the norm identity $\|\hat S^- v\|^2=\|\hat S^+ v\|^2+2sz
\|v\|^2$ for a weight vector $\hat S^3 v=sz\cdot v$ (from $[\hat S^+,\hat S^-]=2\hat S^3$ and $(\hat
S^-)^{\mathsf H}=\hat S^+$, via $\texttt{star}\,w\cdot w=\overline{\sum\|w_i\|^2}$); consequences
$\texttt{fermionTotalSpinMinus\_mulVec\_ne\_zero\_of\_spinZ\_pos}$ ($\hat S^-$ injective for $sz>0$)
and $\texttt{fermionTotalSpinPlus\_mulVec\_ne\_zero\_of\_spinZ\_neg}$ ($\hat S^+$ injective for
$sz<0$).  No representation theory --- these ladder injections embed each $\hat S^3$-weight space of
the ground subspace into $\hat S^3=\pm\tfrac12$, giving (via E3a) the upper bound $\dim\le N_e+1$.
Fully axiom-free.

\emph{(xlviii) Weight-space finrank non-increasing toward $\tfrac12$} (\texttt{TJGroundWeightFinrank.lean},
E5b): $\texttt{fermionTotalSpinMinus\_mulVec\_mem\_groundSubmodule}$ ($\hat S^-$ preserves $G$),
$\texttt{fermionTotalSpinZ\_mulVec\_fermionTotalSpinMinus\_mulVec}$ ($\hat S^-$ lowers the $\hat S^3$
weight by one), and $\texttt{tJ\_ground\_weight\_finrank\_le\_succ}$: for $sz\ge 0$, $\dim(G\sqcap\hat
S^3{=}sz{+}1)\le\dim(G\sqcap\hat S^3{=}sz)$ ($\hat S^-$ injects the $sz{+}1$ weight space into the $sz$
one, by the ladder injectivity at $sz{+}1>0$).  Iterating to $\tfrac12$ (and up from below via $\hat
S^+$) bounds every weight space by the $\hat S^3=\tfrac12$ block ($\le 1$, E3a) --- the route to
$\dim G\le N_e+1$.  Rests only on A.17's documented axiom.

\emph{(xlix) Weight-space finrank from below via $\hat S^+$} (\texttt{TJGroundWeightFinrankRaise.lean},
E5b mirror): $\texttt{fermionTotalSpinPlus\_mulVec\_mem\_groundSubmodule}$ ($\hat S^+$ preserves $G$),
$\texttt{fermionTotalSpinZ\_mulVec\_fermionTotalSpinPlus\_mulVec}$ ($\hat S^+$ raises the $\hat S^3$
weight by one), and $\texttt{tJ\_ground\_weight\_finrank\_le\_of\_spinZ\_neg}$: for $sz<0$,
$\dim(G\sqcap\hat S^3{=}sz)\le\dim(G\sqcap\hat S^3{=}sz{+}1)$ ($\hat S^+$ injects the $sz$ weight space
into $sz{+}1$, by the ladder injectivity at $sz<0$).  With the $\hat S^-$ step this bounds every weight
space by the $\hat S^3=\tfrac12$ block ($\le 1$, E3a); the remaining input is the $\hat S^3$ direct-sum
to assemble $\dim G\le N_e+1$.  Rests only on A.17's documented axiom.

\emph{(l) Every $\hat S^3$ weight block of the ground subspace is $\le 1$-dim}
(\texttt{TJGroundWeightOne.lean}, E5b): $\texttt{tJ\_ground\_weight\_finrank\_le\_one\_pos}$
($\dim(G\sqcap\hat S^3{=}\tfrac12{+}k)\le 1$, $k:\mathbb N$) and
$\texttt{tJ\_ground\_weight\_finrank\_le\_one\_neg}$ ($\dim(G\sqcap\hat S^3{=}{-}\tfrac12{-}k)\le 1$),
by iterating the two weight-finrank steps ($\hat S^-$ from above, $\hat S^+$ from below) down to the
$\hat S^3=\tfrac12$ block ($\le 1$, E3a); the half-integers $\pm\tfrac12,\pm\tfrac32,\dots$ exhaust the
$\hat S^3$ spectrum at odd filling.  The remaining input to $\dim G\le N_e+1$ is the $\hat S^3$
direct-sum.  Rests only on A.17's documented axiom.

\emph{(li) $\hat S^3$ weight decomposition of the ground subspace}
(\texttt{TJGroundWeightDirectSum.lean}, E5b):
$\texttt{fermionTotalSpinZ\_mulVec\_mem\_groundSubmodule}$ ($\hat S^3$ preserves $G$, so $G$ is an
invariant submodule of $\hat S^3$),
$\texttt{fermionTotalSpinZ\_iSup\_eigenspace\_eq\_top}$ ($\hat S^3$ is diagonal on the computational
basis, so every standard basis vector is an eigenvector and the eigenspaces span $\top$), and
$\texttt{tJ\_groundSubmodule\_eq\_iSup\_inf\_eigenspace}$: $G=\bigsqcup_\mu (G\sqcap\ker(\hat
S^3-\mu))$ (\texttt{Submodule.eq\_iSup\_inf\_genEigenspace} for the invariant $G$).  This is the
$\hat S^3$ direct-sum input; reindexing it to the finite $N_e+1$ half-integer weights and applying
the internal-direct-sum finrank with the per-block bound ($\le 1$, item (l)) yields
$\dim G\le N_e+1$.  Rests only on A.17's documented axiom.

\emph{(lii) Finite $\hat S^3$ weight reindexing of the ground subspace}
(\texttt{TJGroundWeightReindex.lean}, E5b):
$\texttt{tJ\_groundSubmodule\_inf\_eigenspace\_eq\_bot}$ (off-weight blocks vanish: $G\sqcap\ker(\hat
S^3-\mu)=\{0\}$ for $\mu$ outside $\{a-N_e/2 : a\in\mathrm{Fin}(N_e{+}1)\}$, since a ground vector is
an $N_e$-electron and $\hat S^3{=}\mu$ eigenstate, forcing every supported configuration to satisfy
$\mu=\#{\uparrow}-N_e/2$) and $\texttt{tJ\_groundSubmodule\_eq\_iSup\_weight}$:
$G=\bigsqcup_{a\in\mathrm{Fin}(N_e+1)}\bigl(G\sqcap\ker(\hat S^3-(a-N_e/2))\bigr)$ (the all-$\mathbb
C$ supremum collapses to the $N_e+1$ occurring half-integer weights).  This is the finite weight
decomposition; the internal-direct-sum finrank with the per-block bound ($\le 1$, item (l)) then
gives $\dim G\le N_e+1$.  Rests only on A.17's documented axiom.

\emph{(liii) Degeneracy upper bound $\dim G\le N_e+1$}
(\texttt{TJGroundDegeneracyUpper.lean}, E5b): $\texttt{tJ\_groundSubmodule\_finrank\_le}$.
The $N_e+1$ half-integer weight blocks $G\sqcap\ker(\hat S^3-(a-N_e/2))$ are independent
(\texttt{Module.End.eigenspaces\_iSupIndep}), so they form an internal direct sum of $G$
($\texttt{DirectSum.coeLinearMap}$ injective with range $G$, then
$\texttt{Module.finrank\_directSum}$); with the per-block bound ($\le 1$, item (l)) this gives
$\dim G=\sum_a\dim(\text{block})\le (N_e+1)\cdot 1$.  Paired with the SU(2)-tower lower bound
$N_e+1\le\dim G$ ($\texttt{tJ\_groundSubmodule\_finrank\_ge}$) this pins $\dim G=N_e+1$, the input to
the maximal-spin multiplet capstone.  Rests only on A.17's documented axiom.

\emph{(liv) Proposition 11.24 capstone --- PROVED} (\texttt{TJProposition1124.lean}, E6):
$\texttt{proposition\_11\_24}$ discharges the former axiom.  On $\texttt{cycleGraph}(N+1)$ at odd
filling $N_e<N+1$, the ground subspace is the maximal-spin $(N_e+1)$-fold multiplet
($\texttt{IsMaximalSpinMultipletSubmodule}\ N\ G\ N_e$): $\dim G=N_e+1$ ($\texttt{le\_antisymm}$ of
the lower bound (l$'$) $\texttt{tJ\_groundSubmodule\_finrank\_ge}$ and the upper bound (liii)), and
every $v\in G$ is an $(\hat S_{\mathrm{tot}})^2$ eigenvector at $(N_e/2)(N_e/2+1)$ --- the $N_e+1$
linearly independent tower states $(\hat S^-)^k\Omega$ span $G$ (card $=$ finrank), each a maximal-spin
$(\hat S_{\mathrm{tot}})^2$ eigenvector ($\texttt{highestWeight\_spinMultiplet\_general}$), so every
linear combination is.  This completes Tasaki's d=1 ferromagnetic t-J ferromagnetism result.
...
\emph{Discharge in progress (Issue \#4314).}  With Proposition 11.24 now proved, Theorem 11.26
is being discharged from Lemma 11.25 (kept as the documented strong-coupling-limit axiom) plus the
$t$-$J$ side for all $N_e\le K+1$: the metallic range $N_e<K+1$ is supplied by
\texttt{proposition\_11\_24}, and the half-filling boundary $N_e=K+1$ reduces to the ferromagnetic
Heisenberg model because the kinetic term vanishes there.  The first step
(\texttt{TJHalfFillingKinetic.lean}) proves
$\texttt{tJ\_kinetic\_sandwich\_mulVec\_tJConfigOf\_eq\_zero\_of\_full}$: for a fully occupied site
state ($\forall k,\,s_k\neq 0$, i.e.\ $N_e=N+1$), $\hat P_{\rm hc}\hat K\hat P_{\rm hc}|\Phi_s\rangle=0$
--- every hop lands on an occupied site, producing a doubly occupied (non-hard-core) configuration
killed by the projection.  Axiom-free.  PR2 (\texttt{TJHalfFillingExchange.lean}) then reduces the
full Hamiltonian: $\texttt{tJHamiltonian\_mulVec\_tJConfigOf\_eq\_of\_full}$ gives
$\hat H_{tJ}|\Phi_s\rangle=J\cdot\texttt{tJExchange}\,|\Phi_s\rangle$ for a fully occupied $s$, where
$\texttt{tJExchange}=\sum_{x,y}[G.\mathrm{Adj}]\,(\hat n_x\hat n_y/4-\hat S_x\cdot\hat S_y)$ --- i.e.\
at half-filling $\hat H_{tJ}$ is the ferromagnetic Heisenberg model.  Axiom-free.  PR3a
(\texttt{TJAllUpSpinDot.lean}) computes the spin dot product on the maximal-spin reference:
$\texttt{fermionSpinDot\_mulVec\_allUpState}$ gives $\hat S_i\cdot\hat S_j|{\uparrow\cdots\uparrow}
\rangle=\tfrac14|{\uparrow\cdots\uparrow}\rangle$ for $i\neq j$ (the raising/lowering terms annihilate
$|{\uparrow\cdots\uparrow}\rangle$ --- one directly via $\hat S^+_j|{\uparrow\cdots\uparrow}\rangle=0$,
the other because $\hat c_{i\downarrow}$ commutes through the different-site $\hat S^-_j$ --- leaving the
diagonal $\hat S^z_i\hat S^z_j=\tfrac14$), so $\hat n_x\hat n_y/4-\hat S_x\cdot\hat S_y$ vanishes on it
and $|{\uparrow\cdots\uparrow}\rangle$ has exchange energy $0$.  Axiom-free.  PR3b
(\texttt{TJAllUpGround.lean}) concludes $\texttt{tJHamiltonian\_mulVec\_allUpState\_eq\_zero}$:
$\hat H_{tJ}|{\uparrow\cdots\uparrow}\rangle=0$ at half-filling (kinetic vanishes, exchange vanishes),
so the maximal-spin $|{\uparrow\cdots\uparrow}\rangle$ is a zero-energy eigenstate of the
half-filling ferromagnetic Heisenberg model.  Axiom-free.  Towards positive-semidefiniteness
(PR3c-prep, \texttt{TJSingletAnnihilation.lean}) we introduce the \emph{singlet annihilation}
operator $\Delta_{xy}=\hat c_{y\downarrow}\hat c_{x\uparrow}-\hat c_{y\uparrow}\hat c_{x\downarrow}$,
with $\Delta_{xy}^\dagger=\hat c^\dagger_{x\uparrow}\hat c^\dagger_{y\downarrow}-\hat
c^\dagger_{x\downarrow}\hat c^\dagger_{y\uparrow}$ and $\Delta_{xy}|{\uparrow\cdots\uparrow}\rangle=0$
(it kills the fully aligned state).  The Heisenberg bond is $\hat n_x\hat n_y/4-\hat S_x\cdot\hat
S_y=\tfrac12\,\Delta_{xy}^\dagger\Delta_{xy}$ (a global operator identity, follow-up), hence
positive-semidefinite.  Axiom-free.  PR3c (\texttt{TJExchangeBondPSD.lean}) proves the underlying CAR
identity $\texttt{tJSingletAnnihilation\_conjTranspose\_mul\_self}$: for $x\neq y$,
$\Delta_{xy}^\dagger\Delta_{xy}=\hat n_{x\uparrow}\hat n_{y\downarrow}+\hat n_{x\downarrow}\hat
n_{y\uparrow}-\hat S^+_x\hat S^-_y-\hat S^-_x\hat S^+_y$, by expanding the four products and reordering
with the cross-site (anti)commutators (\texttt{TJCrossSiteSpinCommute.lean}: $[\hat c_{x\uparrow},\hat
S^+_y]=[\hat c_{x\downarrow},\hat S^-_y]=0$).  This is the operator content behind $\hat n_x\hat
n_y/4-\hat S_x\cdot\hat S_y=\tfrac12\Delta_{xy}^\dagger\Delta_{xy}$ (and hence the bond's
positive-semidefiniteness).  Axiom-free.  PR3d (\texttt{TJExchangeBondHalf.lean}) completes this:
$\texttt{tJExchangeBond\_eq\_half\_singletNormSq}$ ($\hat n_x\hat n_y/4-\hat S_x\cdot\hat
S_y=\tfrac12\Delta_{xy}^\dagger\Delta_{xy}$, $x\neq y$) and
$\texttt{tJExchangeBond\_posSemidef}$ (the bond is positive-semidefinite, via
$\texttt{Matrix.posSemidef\_conjTranspose\_mul\_self}$).  Axiom-free.  PR3e
(\texttt{TJExchangePSD.lean}) sums over the graph: $\texttt{tJExchange\_posSemidef}$ proves
$(\texttt{tJExchange}\ N\ G)$ is positive-semidefinite (a $\texttt{Finset.sum\_induction}$ over the
per-bond PSD terms), the operator nonnegativity behind the half-filling ground energy $=0$.
Axiom-free.  PR3f (\texttt{TJHalfFillingReduction.lean}) extends the per-basis results to the whole
sector: $\texttt{tJFillingSector\_full}$ ($N_e=N{+}1\Rightarrow$ fully occupied),
$\texttt{tJ\_kinetic\_sandwich\_mulVec\_eq\_zero\_of\_filling}$, and
$\texttt{tJHamiltonian\_mulVec\_eq\_smul\_tJExchange\_of\_filling}$ ($\hat H_{tJ}v=J\cdot
\texttt{tJExchange}\,v$ for any hard-core $\hat N{=}N{+}1$ vector $v$) --- so on the half-filling
sector $\hat H_{tJ}=J\cdot\texttt{tJExchange}$.  Axiom-free.  PR3g computes the ground energy:
$\texttt{TJAllUpProperties.lean}$ supplies the all-up filling facts, and
$\texttt{tJ\_groundEnergyAtFilling\_eq\_zero}$ proves $\texttt{groundEnergyAtFilling}\ \hat H_{tJ}\
(N{+}1)=0$ ($\le 0$ from the all-up eigenvector at $0$; $\ge 0$ from $\texttt{tJExchange}\ge 0$ and
$J>0$).  Axiom-free.  PR3h ($\texttt{TJHalfFillingDegeneracyLower.lean}$) proves the lower bound
$\texttt{tJ\_halfFilling\_groundSubmodule\_finrank\_ge}$: $N{+}2\le\dim$ of the half-filling ground
subspace.  The all-up state is a highest-weight ground state ($\hat S^+_{\mathrm{tot}}=0$, $\hat
S^3_{\mathrm{tot}}=\tfrac{N+1}{2}$, $\hat H_{tJ}|{\uparrow\cdots\uparrow}\rangle=0=
\texttt{groundEnergy}\cdot|{\uparrow\cdots\uparrow}\rangle$, $\hat N=N{+}1$, hard-core), so the
SU(2) lowering tower $\texttt{highestWeight\_spinMultiplet\_general}$ yields $N{+}2$ linearly
independent ground states $(\hat S^-_{\mathrm{tot}})^k|{\uparrow\cdots\uparrow}\rangle$ (the
maximal-spin multiplet; mirrors the $N_e<N{+}1$ lower bound).  Axiom-free.  PR3i
($\texttt{TJHalfFillingBondAction.lean}$) proves the per-bond action
$\texttt{tJExchangeBond\_mulVec\_tJConfigOf\_full}$: on a filled $|\Phi_s\rangle$ ($x\ne y$),
$(\tfrac14\hat n_x\hat n_y-\hat S_x\!\cdot\!\hat S_y)|\Phi_s\rangle =
\tfrac12(|\Phi_s\rangle-|\Phi_{\texttt{tJSpinSwap}\,s\,x\,y}\rangle)$ --- the singlet projector
$\tfrac12(1-\mathrm{SWAP}_{xy})$ on the configuration basis.  The diagonal
$\tfrac14\hat n_x\hat n_y-\hat S^3_x\hat S^3_y$ and ladder
$\tfrac12(\hat S^+_x\hat S^-_y+\hat S^-_x\hat S^+_y)$ combine over the four spin cases, using the
sign-free spin-swap action ($\texttt{fermionSiteSpinPlus\_mul\_Minus\_mulVec\_tJConfigOf}$, the two
same-site creation/annihilation pairs squaring to $1$).  Hence a ground state of $\texttt{tJExchange}$
has spin-swap-invariant amplitudes on every bond --- the input to the degeneracy upper bound (a
connected graph's edge swaps generate $S_{N+1}$, so the amplitudes depend only on the up-count, a
$(N{+}2)$-dimensional symmetric space).  Axiom-free.  PR3i-2
($\texttt{TJHalfFillingBondGround.lean}$) discharges the first half of that programme:
$\texttt{tJ\_ground\_bond\_mulVec\_eq\_zero}$ shows a half-filling ground state $v$ is annihilated by
\emph{every} adjacent bond, $(\tfrac14\hat n_x\hat n_y-\hat S_x\!\cdot\!\hat S_y)v=0$.  Indeed
$\texttt{tJ\_ground\_tJExchange\_mulVec\_eq\_zero}$ gives $\texttt{tJExchange}\,v=0$ (ground energy
$0$, $\hat H_{tJ}=J\cdot\texttt{tJExchange}$, $J>0$), so $\langle v,\texttt{tJExchange}\,v\rangle=0$;
each bond term $\langle v,\text{bond}_{xy}v\rangle\ge 0$ (positive-semidefinite), so by
$\texttt{Finset.single\_le\_sum}$ every bond term vanishes, whence $\Delta_{xy}v=0$ (using
$\text{bond}=\tfrac12\Delta^\dagger\Delta$ and $\texttt{dotProduct\_star\_self\_eq\_zero}$) and
$\text{bond}_{xy}v=0$.  With PR3i-1 this makes the ground amplitudes spin-swap invariant on every
bond.  Axiom-free.  PR3i-3a ($\texttt{TJHalfFillingAmplitude.lean}$) reads this off in the basis:
$\texttt{tJ\_ground\_amplitude\_swap\_invariant}$ shows
$v(\texttt{tJConfigOf}\,s)=v(\texttt{tJConfigOf}\,(\texttt{tJSpinSwap}\,s\,x\,y))$ for an adjacent
bond.  Expanding $v$ in the filling basis ($\texttt{tJ\_filling\_completeness}$), applying the bond
(which is $0$ on $v$, and a half spin-swap on each basis state), and reading off the coefficient at
the index $\texttt{tJConfigOf}\,s$ (basis orthonormality, $\texttt{tJConfigOf\_injective}$, and the
involution $\texttt{tJSpinSwap\_involutive\_site}$) gives
$\tfrac12\big(v(\texttt{tJConfigOf}\,s)-v(\texttt{tJConfigOf}\,(\texttt{tJSpinSwap}\,s\,x\,y))\big)=0$.
The sector permutation $\texttt{tJFillingSwap}$ packages the swap on the filling sector.  Axiom-free.
PR3i-3b ($\texttt{TJHalfFillingUpCount.lean}$) propagates this along the connected chain:
$\texttt{tJ\_ground\_amplitude\_eq\_of\_same\_upCount}$ shows the ground amplitudes are constant on
each up-count class --- for sector configurations $t,t'$ with the same number of up-spins,
$v(\texttt{tJConfigOf}\,t)=v(\texttt{tJConfigOf}\,t')$.  Equal up-counts give equal value-counts
(both are full, so $\#\!\downarrow = (N{+}1)-\#\!\uparrow$), hence $t,t'$ are connected by adjacent
value-swaps ($\texttt{adjacentSwapReachable\_of\_same\_counts}$, reused from the Prop 11.24 route),
and each adjacent swap preserves the amplitude (PR3i-3a) by induction on
$\texttt{AdjacentSwapReachable}$ (the reflexive--transitive closure of adjacent bond swaps; the empty
count is preserved so every intermediate configuration is full).  There are $N{+}2$ up-counts
$\{0,\dots,N{+}1\}$, so this is the last combinatorial input to the upper bound
$\dim\le N{+}2$.  Axiom-free.  PR3i-3c ($\texttt{TJHalfFillingDegeneracyUpper.lean}$) closes the upper
bound: $\texttt{tJ\_halfFilling\_groundSubmodule\_finrank\_le}$ proves $\dim G\le N{+}2$.  A ground
state is determined by its amplitudes ($\texttt{tJ\_filling\_completeness}$), which are constant on
each up-count class, so evaluating at one representative per class ($\texttt{tJUpCountRep}\,j$ = the
config with the first $j$ sites up) is an injective linear map $G\hookrightarrow(\mathrm{Fin}\,(N{+}2)\to\mathbb C)$
($\texttt{LinearMap.finrank\_le\_finrank\_of\_injective}$, codomain dimension $N{+}2$).  Paired with the
SU(2)-tower lower bound (\#4326) this pins $\dim G=N{+}2$ --- the dimension half of the half-filling
case of Theorem 11.26.  Axiom-free.  PR3-cap ($\texttt{TJHalfFillingMaximalSpin.lean}$) assembles the
half-filling result: $\texttt{tJ\_halfFilling\_isMaximalSpinMultiplet}$ proves
$\texttt{IsMaximalSpinMultipletSubmodule}\,N\,G\,(N{+}1)$ --- the half-filling t-J ground subspace is
the maximal-spin $(N{+}2)$-fold multiplet (ground $S_{\mathrm{tot}}=(N{+}1)/2$).  With $\dim G=N{+}2$
(\#4331 $\wedge$ \#4326), the all-up SU(2) tower's $N{+}2$ linearly independent ground states span $G$
($\texttt{span\_eq\_top\_of\_card\_eq\_finrank}$), and each is an $(\hat S_{\mathrm{tot}})^2$
eigenvector at $S_{\max}(S_{\max}{+}1)$, so every ground state is.  This is the boundary
($N_e=N{+}1$, ferromagnetic-Heisenberg) case of the t-J side of Theorem 11.26, the half-filling
counterpart of the metallic Proposition 11.24; axiom-free (no A.17 is needed at half filling).
PR-unify ($\texttt{TJMaximalSpinUnified.lean}$) combines the two cases:
$\texttt{tJ\_isMaximalSpinMultiplet\_of\_le}$ proves that for odd $N_e\le K{+}1$ and $\tau,J>0$ the
d=1 ferromagnetic t-J ground subspace on $\texttt{cycleGraph}\,(K{+}1)$ is the maximal-spin
$(N_e{+}1)$-fold multiplet --- the metallic case $N_e<K{+}1$ from Proposition 11.24 and the
half-filling case $N_e=K{+}1$ from the result above.  (The half-filling chain was generalized to drop
the $0<N$ hypothesis --- $\texttt{adjacentSwapReachable\_of\_same\_counts\_general}$ handles the
single-site $N=0$ case by reflexivity --- so the $K=0$ boundary is covered.)  This is the t-J input to
Theorem 11.26; it rests on the A.17 axiom through Proposition 11.24.

\medskip\noindent\textbf{Theorem 11.26 (metallic ferromagnetism in the d=1 Hubbard model) is now a
proved theorem.}  PR-final discharges the former axiom in $\texttt{MetallicFerroModel.lean}$:
$\texttt{theorem\_11\_26}$ states that for the d=1 decorated Hubbard model
$\texttt{decHubbardHamiltonian}\,K\,\nu\,s\,t\,U$ with odd $N_e\le K{+}1$, there are thresholds
$T,V>0$ such that for $t\ge T$, $U\ge V$ the ground subspace at filling $N_e$ is the maximal-spin
$(N_e{+}1)$-fold multiplet.  The proof takes the thresholds $T,V,W$ from Lemma 11.25 (the documented
strong-coupling Hubbard$\leftrightarrow$t-J equivalence axiom), applies its biconditional at hopping
$\tau=(1+4\nu^2)s$ and $J=W$ to rewrite the Hubbard ground-multiplet claim into the t-J one, and
closes with $\texttt{tJ\_isMaximalSpinMultiplet\_of\_le}$.  Thus
$\texttt{\#print axioms theorem\_11\_26}$ is
$[\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound},\texttt{lemma\_11\_25},
\texttt{exists\_joint\_su2\_energy\_eigenstate}]$: the result rests only on the two documented
Tasaki-level axioms (the strong-coupling equivalence and A.17).
...
\medskip\noindent\textbf{Theorem A.1 (Lie product formula, proved).}  For finite complex
...
non-commuting Lie/Trotter formula is \textbf{now proved from scratch} (axiom-free,
\texttt{lieProductFormula}, \texttt{Math/MatrixAnalysis/LieProduct.lean}), in a general complete
normed $\mathbb R$-algebra (\texttt{trotterProductFormula}) instantiated for matrices under the
scoped operator norm.  The proof is the classical four-step argument: \emph{(i)} exponential-series
tail bounds $\|e^X-1-X\|\le\|X\|^2e^{\|X\|}$, $\|e^X-1\|\le\|X\|e^{\|X\|}$,
$\|e^X\|\le e^{\|X\|}$, obtained by splitting the series
(\texttt{Summable.tsum\_eq\_zero\_add}) and comparing termwise with the real exponential series;
\emph{(ii)} the telescoping power estimate $\|C^n-D^n\|\le n\,M^{n-1}\|C-D\|$ for
$\|C\|,\|D\|\le M$; \emph{(iii)} the product comparison
$\|e^{sA}e^{sB}-e^{s(A+B)}\|\le 4s^2(\|A\|+\|B\|)^2e^{\|A\|+\|B\|}$ for $0\le s\le1$
(both sides equal $1+s(A+B)$ to first order; the four remainders are bounded by \emph{(i)});
\emph{(iv)} since $e^{A+B}$ is \emph{exactly} the $n$-th power of its $1/n$-step
(\texttt{exp\_nsmul}), the telescope with $M=e^{(\|A\|+\|B\|)/n}$ (so
$M^{n-1}\le e^{\|A\|+\|B\|}$) converts the per-step $O(1/n^2)$ distance into an $O(1/n)$ bound,
which squeezes to $0$.

\medskip\noindent\textbf{Lemmas A.4--A.6 (positive-semidefinite basics, proved).}  These are
available in mathlib and are \emph{proved} (axiom-free,
\texttt{Math/PosSemidef/Basics.lean}).  \textbf{Lemma A.4}
...
\medskip\noindent\textbf{Theorem A.7 (eigenvalue monotonicity, proved).}  If self-adjoint
...
operator-monotonicity content of the min--max principle (eq.~(A.2.30)).  \textbf{Now proved
(axiom-free)} by the Courant--Fischer block/pigeonhole argument
(\texttt{hermitian\_eigenvalues}$_0$\texttt{\_monotone}, via
\texttt{LatticeSystem.Quantum.hermitian\_eigenvalues}$_0$\texttt{\_mono} in
\texttt{Math/MatrixAnalysis/CourantFischer.lean}): the spectral Rayleigh sum $\mathrm{re}\,\langle
x,Tx\rangle=\sum_i\lambda_i\,\lVert\langle b_i,x\rangle\rVert^2$ (from
\texttt{eigenvectorBasis\_apply\_self\_apply}) gives, with Parseval, block bounds
$c\lVert x\rVert^2\le\mathrm{re}\langle x,Tx\rangle$ for $x$ supported on eigenvalues $\ge c$ (and
dually $\le c$); the top-$(i{+}1)$ eigenspace of $\hat A$ and the bottom-$(\mathrm{card}\,n-i)$
eigenspace of $\hat B$ have dimensions summing to $\mathrm{card}\,n+1$, so by the finrank pigeonhole
(\texttt{finrank\_add\_finrank\_le\_of\_disjoint}) they meet in a nonzero $x$, and
$\lambda_i(\hat A)\lVert x\rVert^2\le\mathrm{re}\langle x,\hat Ax\rangle\le\mathrm{re}\langle x,\hat
Bx\rangle\le\lambda_i(\hat B)\lVert x\rVert^2$ (Loewner monotonicity of the energy).  The
constructive min--max machinery beyond mathlib's extreme-eigenvalue variational principle is built
in \texttt{CourantFischer.lean}.

\medskip\noindent\textbf{Corollary A.8 (trace of the exponential, proved).}  An
...
$\mathrm{Tr}[f(\hat A)]\le\mathrm{Tr}[f(\hat B)]$ for non-decreasing $f$ (eq.~(A.2.32)).
\textbf{Now proved (axiom-free)}, \texttt{hermitian\_trace\_exp\_monotone}
(\texttt{Math/MatrixAnalysis/TraceExpMonotone.lean}): the spectral mapping
$\mathrm{Tr}[e^{\hat A}]=\sum_j e^{a_j}$ (\texttt{trace\_exp\_eq\_sum\_exp\_eigenvalues}) follows
from the spectral theorem $\hat A=U\,\mathrm{diag}(a)\,U^\dagger$ by passing $e^{(\cdot)}$ through
the conjugation (\texttt{Matrix.exp\_conj}, with $U^\dagger=U^{-1}$ for the eigenvector unitary),
exponentiating the diagonal entrywise (\texttt{Matrix.exp\_diagonal}), and cancelling the
conjugation under the trace (\texttt{Matrix.trace\_mul\_cycle}); the termwise comparison
$e^{a_j}\le e^{b_j}$ is Theorem A.7 (now proved via Courant--Fischer) composed with the
monotonicity of $e^x$.
...
(axiom-free, \texttt{Math/FrustrationFree.lean}).
...
\texttt{Math/PosSemidef/Kernel.lean}).
...
(\texttt{effectiveHamiltonian\_strongCoupling\_limit}, \texttt{Math/EffectiveLimit.lean})
...
Lemma A.14.  Proved (axiom-free, \texttt{Math/AngularMomentum/Ladder.lean}).
...
\texttt{Math/AngularMomentum/Quantization.lean}).
...
\texttt{Math/AngularMomentum/Multiplet.lean}).

\medskip\noindent\textbf{Theorem A.17 (spin-0/spin-1/2 sector suffices, proved).}  For an
...
in the $E$-eigenspace (\texttt{exists\_joint\_su2\_energy\_eigenstate})---is \textbf{now proved
(axiom-free)}.  The Casimir $\hat{\boldsymbol J}^2=J_1^2+J_2^2+J_3^2$ is positive semidefinite (a sum
$\sum_\alpha J_\alpha^{\mathsf H}J_\alpha$) and commutes with both $\hat J^{(3)}$ (from the su(2)
relations, $[\hat{\boldsymbol J}^2,\hat J^{(3)}]=0$) and $\hat H$; so $\hat{\boldsymbol J}^2$ and
$\hat J^{(3)}$ are two \emph{commuting Hermitian} operators preserving the $\hat H=E$ eigenspace $W$
(nonzero, witnessed by $v$).  A generic finite-dimensional lemma---two commuting Hermitian operators on
a nonzero invariant subspace have a common eigenvector there, with real eigenvalues, via
\texttt{Module.End.exists\_eigenvalue} on the restriction, eigenvalue reality
(\texttt{IsSymmetric.conj\_eigenvalue\_eq\_self}) and invariant-subspace restriction
(\texttt{Math/CommutingHermitianEigenvector.lean}: \texttt{exists\_common\_eigenvector\_of\_isHermitian\_commute})---yields
the common eigenvector $\Phi\in W$, with the $\hat{\boldsymbol J}^2$-eigenvalue $a\ge0$ (positive
operator) written $a=J(J+1)$ for $J=(\sqrt{1+4a}-1)/2\ge0$.  Proved (axiom-free,
\texttt{Math/AngularMomentum/SpinHalfSector.lean}).  As a consequence the §11.5 Proposition~11.24,
which rested only on this A.17 ingredient, becomes \emph{fully axiom-free}; dependents such as
Theorem~11.26 lose their A.17 dependency but still rest on their other documented axioms (e.g.\
\texttt{lemma\_11\_25}).
...
\texttt{Math/PerronFrobeniusSymmetric.lean}).

\medskip\noindent\textbf{Theorems A.19 (polar) and A.20 (SVD), proved.}  Any square complex matrix
...
the singular \emph{values} of a linear map but not the matrix factorizations, so these are
\textbf{constructed here (axiom-free)}.  The SVD is built from the spectral theorem of
$M=A^\dagger A$ (\texttt{Matrix.IsHermitian.eigenvectorUnitary} gives $V$, eigenvalues
$\lambda_i\ge0$ by \texttt{PosSemidef.eigenvalues\_nonneg}): the images $w_i=A v_i$ are orthogonal
with $\lVert w_i\rVert^2=\lambda_i$ (proved via the matrix adjoint of \texttt{toEuclideanLin}), so the
normalised $u_i=d_i^{-1}w_i$ ($d_i=\sqrt{\lambda_i}$, $\lambda_i\ne0$) are orthonormal and extend to a
full orthonormal basis (\texttt{Orthonormal.exists\_orthonormalBasis\_extension\_of\_card\_eq})
forming $U$; then $A V=U D$ columnwise (for $\lambda_i=0$, $w_i=0$ and $d_i=0$), hence $A=UDV^\dagger$.
The polar decomposition follows with $W=UV^\dagger$ and $C=VDV^\dagger$ (positive-semidefinite by
\texttt{PosSemidef.mul\_mul\_conjTranspose\_same}).  In
\texttt{Math/MatrixAnalysis/Decomposition.lean}.
...
(\texttt{Math/CStarAlgebra/State.lean}).
...
\texttt{Math/CStarAlgebra/GroundState.lean}).
...
\texttt{GelfandNaimarkSegal}; \texttt{Math/CStarAlgebra/GNS.lean}).
...
\texttt{mathlib} supports is proved axiom-free: A.4--A.7, A.9--A.11, A.13--A.20 (including Weyl
eigenvalue monotonicity A.7 via Courant--Fischer, the simultaneous-diagonalization step of A.17,
Perron--Frobenius for a real symmetric matrix A.18, and the polar / singular-value decompositions
A.19/A.20).  The remaining items---A.1, A.8, A.12, and the operator-algebraic A.21--A.28 (Wigner,
states, Banach--Alaoglu, ground states, GNS)---are recorded as faithful documented axioms (A.8
trace-exp pending the spectral mapping for \texttt{exp}).  These
...
\begin{itemize}
  ...
  \item Problem 2.5.b finite-sum lower-bound bridge.  The declaration
        ...
        The auxiliary fact that a finite sum of Hermitian matrices is
        Hermitian is the generic helper \texttt{Matrix.isHermitian\_sum}
        (\texttt{Math/MatrixAnalysis/HermitianSum.lean}), shared across the
        physics files that previously each carried a private copy.
