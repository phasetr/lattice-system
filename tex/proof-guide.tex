...
\textbf{Theorem 11.20} (\texttt{tasaki\_theorem\_11\_20}, \emph{now a proved theorem}, $d=1$): for
...
$S_{\max}=N/2$).  The discharge (in \texttt{NonsingularLocalHamiltonian.lean}, downstream of the
$\hat h_p$ lemmas) is the book-faithful reduction: Lemma 11.22 fixes parameters
...
rests only on Lemmas 11.21/11.22 (themselves documented axioms, deferred).  The proof-internal
numbered lemmas are recorded (\texttt{NonsingularLocalHamiltonian.lean}):
...
positivity below $S_{\max}$) remain documented axioms; Theorem 11.20 is proved from 11.21/11.22.
...
by the max-spin tower; the strict gap below $S_{\max}$ (via Theorem 11.11) remains.  Axiom-free.
