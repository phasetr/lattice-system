...
\texttt{LatticeSystem/Quantum/SpinS/Theorem23.lean}. Given a
sublattice indicator $A : V \to \mathrm{Bool}$ with $|A| \ge 1$,
$|\neg A| \ge 1$, a real symmetric coupling $J$ supported and
positive on the bipartite complete graph
$\mathrm{bipartiteCompleteGraphOf}\,A$, the statement asserts the
existence of a common ground-state energy $\mu$ such that:
\begin{itemize}
  \item (existence) for every magnetisation sector $M$ in
    $\mathrm{tasaki23GroundStateSectors}\,A\,N \;=\;
    [\min(|A|, |\neg A|)\cdot N,\, \max(|A|, |\neg A|)\cdot N]$
    (in $\mathrm{magSumS}$ units), the spin-$S$ Heisenberg
    Hamiltonian $\hat H = \sum_{x,y} J(x,y)\,\hat
    {\boldsymbol S}_x \cdot \hat{\boldsymbol S}_y$ admits a
    non-zero eigenvector $\Psi$ of eigenvalue $\mu$ supported on
    sector $M$;
  \item (minimality) no eigenvalue of $\hat H$ is strictly less
    than $\mu$.
\end{itemize}
...
\bigr|\cdot N/2$. The proof iterates the sector-level Theorem 2.2
result \texttt{marshallLiebMattis\_spinS\_heisenbergHamiltonianS\_groundState\_full}
(\#869) across the admissible magnetisation range. Reference:
H.~Tasaki, \emph{Physics and Mathematics of Quantum Many-Body
