...
mathlib's \texttt{SimpleGraph.lineGraph}. The two main results --- Theorem 11.12
...
$N=D$, the ground states are the maximal-spin multiplet) --- are stated by Tasaki
with their proofs deferred to §11.3.3 (Theorem 11.12) or omitted entirely
(Theorem 11.13), so they are introduced as documented axioms
(\texttt{MielkeTheorems.lean}), matching the policy for Theorem 11.8 / Lemma 11.9.
...
states that, for a \emph{connected} base lattice, the single-electron operator
...
\]
(\texttt{mielke\_lineGraph\_ker\_finrank\_eq}, for $t\ge 0$). The next step identifies
$\operatorname{rank} S = |\tilde\Lambda| - \dim\ker(SS^{\mathsf H})$ with
$\dim\ker(SS^{\mathsf H})=(\text{bipartite}\,?\,1:0)$ for a connected base, yielding
$D(\tilde\Lambda,B)$.
