...
\noindent\textbf{Status / scope.} The single-spin case is closed
(\texttt{LatticeSystem/Quantum/TimeReversalSpinHalf.lean}). The
multi-spin extension \(\hat{\Theta}_{\text{tot}} =
\bigotimes_{x \in \Lambda} \hat{\Theta}_x\) (Tasaki §2.3 lattice
discussion) and the integer-spin case
(\(\hat{\Theta}^{2} = +\hat{1}\)) are deferred.
