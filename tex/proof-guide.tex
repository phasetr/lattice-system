...
\item \textbf{Theorem~2.4 status}.  The spin-$1/2$ case-(i) target uniqueness
  ...
  are now packaged, the target wrappers from SU(2)-endpoint global uniqueness
  supply the strict gap over non-balanced non-empty sectors, and the new
  MLM/Casimir endpoint wrappers construct that SU(2) uniqueness input from the
  general Theorem~2.3 Perron--Frobenius endpoint.  Tasaki's case~(ii), the
  general spin-$S$ \(D=0\) edge, and the general spin-$S$ \(\lambda=1\) edge
...
longer exposes an abstract SU(2)-endpoint uniqueness hypothesis.  Tasaki
case~(ii), the general spin-\(S\) \(D=0\) edge, and the general spin-\(S\)
\(\lambda=1\) edge remain separate tasks.
