...
$S_{\rm tot} = \bigl||A| - |\neg A|\bigr|\,S$. Subsequent γ-4 PRs
...
operator-level form that the upcoming γ-4 spectrum analysis will
chain through.
...
γ-4 milestone, not yet established.
...
\textbf{Step 15 (PR~\#2786)}: §2.4 $\leftrightarrow$ γ-4 bridge
...
reverse bridge via \texttt{le\_antisymm}. Identifies the γ-4
...
Theorem 2.3 (γ-4): the N\'eel state matches the predicted GS
...
to \texttt{onSiteS x ((2:ℂ) • spinSOp3 N)}), then applies linearity
...
identity. Foundation for Tasaki Problem 2.5.c (γ-7).
