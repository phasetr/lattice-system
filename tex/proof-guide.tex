...
decomposition; (b) proving the global lower-bound callback, equivalently
first using the single-cluster magnetization projection bridge to pass to a
non-zero weight component, then showing the projected spectrum is exhausted by
joint total/leaf Casimir sectors and applying the coupled leaf/center
sector-energy bridge. The older raw target $\operatorname{Re}\alpha\ge\alpha_{\rm pred}$
is too strong as an all-sector design goal; the lower-bound route should
use the actual leaf spin parameter supplied by the sector decomposition.
Both
