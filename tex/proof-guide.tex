...
feeds those two callbacks into the full-ground-energy block transfer.
...
and feeds them to the full-ground-energy block transfer.  The remaining
pathwise case~(ii) input is the appropriate strict or zero-coefficient
reachability hypotheses for both parity blocks along the path, together with
the explicit \((\lambda,D)=(1,0)\) corner PF/min callbacks.
