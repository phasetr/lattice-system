...
Remaining: the full eigenvector lift $\hat H_{\rm tJ}(\texttt{tJExpansion}\,v)=\mu\cdot\texttt{tJExpansion}\,v$
(the sector matrix $M=\texttt{tJEffReMatrixOnSector}$ acting on expansions, lifting the
Perron--Frobenius eigenvector to an $\hat H_{\rm tJ}$-eigenvector), the total-spin
identification ($S_{\mathrm{tot}}=N_e/2$) by
Marshall-positive overlap, the SU(2)-tower lower bound, the A.17 upper bound, and the capstone that
replaces the axiom.
