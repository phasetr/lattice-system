...
for f in claims.tsv source-items.tsv item-claims.tsv claim-vocabulary-review.tsv claim-vocabulary.tsv source-invariants.tsv bindings.tsv axioms.tsv claim-axioms.tsv pages.tsv sources.tsv tracks.tsv; do
  [[ -f "$BASE_REG/$f" && -f "$CURRENT_REG/$f" ]] || fail "missing registry/$f"
done
for f in correction-events.tsv claim-normalization-reviews.tsv claim-corrections.tsv claim-successors.tsv source-item-additions.tsv; do [[ -f "$CURRENT_REG/$f" ]] || fail "missing registry/$f"; done
...
if [[ -f "$BASE_REG/source-item-additions.tsv" ]] &&
   ! awk -F '\t' 'NR==FNR{if(FNR>1)o[$1]=$0;next}FNR>1{c[$1]=$0}END{for(i in o)if(c[i]!=o[i])exit 1}' \
     "$BASE_REG/source-item-additions.tsv" "$CURRENT_REG/source-item-additions.tsv"; then
  fail "unmanifested source item addition"
fi
...
EVENT_SOURCE=$(awk -F '\t' -v e="$EVENT" 'FNR>1&&$1==e{print $3}' "$CURRENT_REG/correction-events.tsv")
[[ $PRODUCTION -eq 0 || "$RECORDED_BASE" == "$BASE_COMMIT" ]] || fail "correction event base commit mismatch"
awk -F '\t' -v s="$EVENT_SOURCE" 'FNR>1&&$1==s{n++}END{exit n!=1}' "$BASE_REG/sources.tsv" || fail "correction event source mismatch"
awk -F '\t' -v s="$EVENT_SOURCE" 'FNR>1&&$1==s{n++}END{exit n!=1}' "$CURRENT_REG/source-invariants.tsv" || fail "correction event source mismatch"
...
freeze correction-events.tsv 0; freeze claim-normalization-reviews.tsv 2; freeze claim-corrections.tsv 2; freeze claim-successors.tsv 2; freeze source-item-additions.tsv 2
...
awk -F '\t' -v e="$EVENT" -v source="$EVENT_SOURCE" '
  FILENAME==ARGV[1]{if(FNR>1)s[$1]=$2;next}
  FNR>1&&$2==e&&s[$5]!=source{bad=1}
  END{exit bad}
' "$BASE_REG/claims.tsv" "$CURRENT_REG/claim-corrections.tsv" || fail "correction predecessor source mismatch"

review_check=$(awk -F '\t' -v e="$EVENT" -v source="$EVENT_SOURCE" '
  FILENAME==ARGV[1]{if(FNR>1){base[$1]=1;bs[$1]=$2}next}
  FILENAME==ARGV[2]{if(FNR>1)current[$1]=1;next}
  FNR>1&&$2==e {
    if(!($4 in base)||!($4 in current)){print "unknown";exit}
    if(bs[$4]!=source){print "source";exit}
  }
' "$BASE_REG/claims.tsv" "$CURRENT_REG/claims.tsv" "$CURRENT_REG/claim-normalization-reviews.tsv")
[[ "$review_check" != unknown ]] || fail "normalization review targets an unknown claim"
[[ "$review_check" != source ]] || fail "normalization review source mismatch"
...
for p in "$BASE_REG"/*.tsv; do f=${p##*/}; case "$f" in claims.tsv|source-items.tsv|item-claims.tsv|claim-vocabulary-review.tsv|claim-vocabulary.tsv|source-invariants.tsv|correction-events.tsv|claim-normalization-reviews.tsv|claim-corrections.tsv|claim-successors.tsv|source-item-additions.tsv|bindings.tsv|axioms.tsv|claim-axioms.tsv)continue;;esac; [[ -f "$CURRENT_REG/$f" ]]&&cmp -s "$p" "$CURRENT_REG/$f" || fail "unrelated frozen registry drift"; done
...
successor_check=$(awk -F '\t' -v e="$EVENT" -v source="$EVENT_SOURCE" '
  FILENAME==ARGV[1]{if(FNR>1){base[$1]=$0;bs[$1]=$2;bt[$1]=$12;bd[$1]=$6}next}
  FILENAME==ARGV[2]{if(FNR>1){current[$1]=$0;cs[$1]=$2;ct[$1]=$12;cd[$1]=$6}next}
  FILENAME==ARGV[3]{if(FNR>1&&$2==e)pred[$1]=$5;next}
  FNR>1&&$2==e {
    if($6=="new_successor"&&($7 in current)&&cs[$7]!=source){print "new-source";exit}
    if($6=="existing_duplicate"){
      if(bs[$7]!=source){print "successor-source";exit}
      if($7==pred[$3]){print "predecessor";exit}
      if(bt[$7]!="false"||ct[$7]!="false"){print "inactive";exit}
      if(bd[$7]=="out_of_scope"||cd[$7]=="out_of_scope"){print "out-of-scope";exit}
      if(base[$7]!=current[$7]){print "drift";exit}
    }
  }
' "$BASE_REG/claims.tsv" "$CURRENT_REG/claims.tsv" "$CURRENT_REG/claim-corrections.tsv" "$CURRENT_REG/claim-successors.tsv")
case "$successor_check" in
  new-source) fail "new successor source mismatch" ;;
  successor-source) fail "correction successor source mismatch" ;;
  predecessor) fail "existing duplicate successor is the correction predecessor" ;;
  inactive) fail "existing duplicate successor is not active" ;;
  out-of-scope) fail "existing duplicate successor is not a formalization target" ;;
  drift) fail "existing duplicate successor drift" ;;
esac
...
item_order_check=$(LC_ALL=C awk -F '\t' '
  function valid(s,n,a,i){n=split(s,a,".");if(n<2||a[1]!~/^[0-9][0-9][0-9][0-9][0-9][0-9]$/)return 0;for(i=2;i<=n;i++)if(a[i]!~/^[0-9][0-9][0-9][0-9]$/)return 0;return 1}
  FILENAME==ARGV[1]{if(FNR>1)t[$1]=$2+0;next}
  FILENAME==ARGV[2]{if(FNR>1){st[$1]=$2;sp[$1]=$3+0}next}
  FNR>1{
    if(!valid($3)){print "bad";exit}
    tuple=sprintf("%09d.%09d.%s",t[st[$2]],sp[$2],$3)
    if(rows&&tuple<=previous){print "order";exit}
    previous=tuple;rows++
  }
' "$CURRENT_REG/tracks.tsv" "$CURRENT_REG/sources.tsv" "$CURRENT_REG/source-items.tsv")
[[ "$item_order_check" != bad ]] || fail "bad nested source item order key"
[[ "$item_order_check" != order ]] || fail "source items are not in global source order"

item_drift=$(awk -F '\t' -v e="$EVENT" '
  FILENAME==ARGV[1]{if(FNR>1&&$2==e)pred[$5]=1;next}
  FILENAME==ARGV[2]{if(FNR>1&&($3 in pred))related[$1]=1;next}
  FILENAME==ARGV[3]{if(FNR>1){old[$1]=$0;known[$1]=1}next}
  FILENAME==ARGV[4]{if(FNR>1)cur[$1]=$0;next}
  END{for(id in known)if(cur[id]!=old[id]){if(id in related)r=1;else u=1}if(r)print "related";else if(u)print "unrelated"}
' "$CURRENT_REG/claim-corrections.tsv" "$BASE_REG/item-claims.tsv" "$BASE_REG/source-items.tsv" "$CURRENT_REG/source-items.tsv")
[[ "$item_drift" != related ]] || fail "historical source item drift"
[[ "$item_drift" != unrelated ]] || fail "unrelated frozen registry drift"

relation_drift=$(awk -F '\t' -v e="$EVENT" '
  FILENAME==ARGV[1]{if(FNR>1&&$2==e)pred[$5]=1;next}
  FILENAME==ARGV[2]{if(FNR>1){old[$0]=1;if($3 in pred)related[$0]=1}next}
  FILENAME==ARGV[3]{if(FNR>1)cur[$0]=1;next}
  END{for(row in old)if(!(row in cur)){if(row in related){print "related";exit}else u=1}if(u)print "unrelated"}
' "$CURRENT_REG/claim-corrections.tsv" "$BASE_REG/item-claims.tsv" "$CURRENT_REG/item-claims.tsv")
[[ "$relation_drift" != related ]] || fail "historical source item relation drift"
[[ "$relation_drift" != unrelated ]] || fail "unrelated item-claim drift"

item_addition_check=$(awk -F '\t' -v e="$EVENT" -v source="$EVENT_SOURCE" -v review="$EVENT_REVIEW" '
  FILENAME==ARGV[1]{if(FNR>1)baseItem[$1]=1;next}
  FILENAME==ARGV[2]{if(FNR>1){item[$1]=1;itemSource[$1]=$2}next}
  FILENAME==ARGV[3]{if(FNR>1&&$2==e){corr[$1]=1;corrClaim[$1]=$5}next}
  FILENAME==ARGV[4]{if(FNR>1&&$2==e){if($6=="new_successor"){newCorr[$7]=$3;new[$7]=1;hasNewCorr[$3]=1}else if($6=="existing_duplicate")existing[$7]=1}next}
  FILENAME==ARGV[5]{if(FNR>1)baseClaim[$1]=1;next}
  FILENAME==ARGV[6]{if(FNR>1){relItem[$3]=$1;claimUse[$3]++;pos[$1]++;rel[$1 SUBSEP pos[$1]]=$3}next}
  FNR>1&&$2!=e {if(!($5 in baseItem)){print "owner";exit};next}
  FNR>1&&$2==e {
    if(!($3 in corr)||!hasNewCorr[$3]||$6!=review){print "owner";exit}
    if($4!=++additionPos[$3]){print "owner";exit}
    if($5 in baseItem){print "reuse";exit}
    if(!($5 in item)||itemSource[$5]!=source){print "source";exit}
    added[$5]=$3
  }
  END{
    for(id in item)if(!(id in baseItem)&&!(id in added)){print "unmanifested";exit}
    for(id in added){
      if(pos[id]==0){print "unmanifested";exit}
      for(i=1;i<=pos[id];i++){
        claim=rel[id SUBSEP i]
        if(claim in baseClaim){if(claim in existing)print "existing";else print "old";exit}
        if(!(claim in new)){print "old";exit}
        if(newCorr[claim]!=added[id])mismatch=1
        else {matched=1;if(claimUse[claim]>1){print "duplicate";exit}}
      }
    }
    if(mismatch){if(matched)print "different";else print "owner";exit}
  }
' "$BASE_REG/source-items.tsv" "$CURRENT_REG/source-items.tsv" "$CURRENT_REG/claim-corrections.tsv" "$CURRENT_REG/claim-successors.tsv" "$BASE_REG/claims.tsv" "$CURRENT_REG/item-claims.tsv" "$CURRENT_REG/source-item-additions.tsv")
case "$item_addition_check" in
  owner*) fail "source item addition event or correction mismatch" ;;
  reuse*) fail "source item addition reuses a frozen item ID" ;;
  source*) fail "source item addition source mismatch" ;;
  unmanifested*) fail "unmanifested source item addition" ;;
  duplicate*) fail "correction successor occurs in more than one source item" ;;
  existing*) fail "new source item contains an existing-duplicate successor" ;;
  old*) fail "new source item contains a frozen predecessor claim" ;;
  different*) fail "new source item contains a successor from a different correction" ;;
esac
...
awk -F '\t' -v e="$EVENT" 'FILENAME==ARGV[1]{if(FNR>1&&$2==e){a[$5]=$6;r[$5]=$4}next}FILENAME==ARGV[2]{if(FNR>1)rr[$1]=$9;next}FILENAME==ARGV[3]{if(FNR>1&&$2==e&&$6=="new_successor")ad[$7]=1;next}FILENAME==ARGV[4]{if(FNR>1)c[$1]=1;next}FILENAME==ARGV[5]{if(FNR>1)iu[$3]++;next}FILENAME==ARGV[6]{if(FNR>1){vr[$1]=$3;vu[$1]++}next}FILENAME==ARGV[7]{if(FNR>1)vv[$1]++;next}END{for(i in a)if(a[i]=="reclassify"){if(vu[i]!=1||vr[i]!=rr[r[i]])bad("reclassified claim lacks correction vocabulary review")}else{if(vu[i])bad("excluded predecessor retains vocabulary review");if(vv[i])bad("excluded predecessor retains vocabulary use")}for(i in ad){if(!(i in c))continue;if(iu[i]==0)bad("active successor lacks the predecessor item");if(iu[i]>1)bad("active successor has duplicate item provenance");if(vu[i]!=1)bad("active successor lacks vocabulary review")};exit failed}function bad(s){print s>"/dev/stderr";failed=1}' "$CURRENT_REG/claim-corrections.tsv" "$CURRENT_REG/claim-normalization-reviews.tsv" "$CURRENT_REG/claim-successors.tsv" "$CURRENT_REG/claims.tsv" "$CURRENT_REG/item-claims.tsv" "$CURRENT_REG/claim-vocabulary-review.tsv" "$CURRENT_REG/claim-vocabulary.tsv" || fail "correction relation propagation failed"
...
awk -F '\t' -v s="$EVENT_SOURCE" '
  NR==FNR{if(FNR>1&&$1!=s)old[$1]=$0;next}
  FNR>1&&$1!=s{cur[$1]=$0}
  END{for(id in old)if(cur[id]!=old[id])bad=1;for(id in cur)if(!(id in old))bad=1;exit bad}
' "$BASE_REG/source-invariants.tsv" "$CURRENT_REG/source-invariants.tsv" || fail "unrelated source invariant drift"
IFS=$'\t' read -r source pages ra rt re rem ro rr < <(awk -F '\t' -v s="$EVENT_SOURCE" 'FNR>1&&$1==s{print}' "$CURRENT_REG/source-invariants.tsv")
[[ "$source" == "$EVENT_SOURCE" ]] || fail "correction event source mismatch"
aa=$(awk -F '\t' -v s="$EVENT_SOURCE" 'FNR>1&&$2==s&&$12=="false"{n++}END{print n+0}' "$CURRENT_REG/claims.tsv"); at=$(awk -F '\t' -v s="$EVENT_SOURCE" 'FNR>1&&$2==s&&$12=="false"&&$6!="out_of_scope"{n++}END{print n+0}' "$CURRENT_REG/claims.tsv"); ae=$(awk -F '\t' -v s="$EVENT_SOURCE" 'FNR>1&&$2==s&&$12=="false"&&$7=="equation"{n++}END{print n+0}' "$CURRENT_REG/claims.tsv"); am=$(awk -F '\t' -v s="$EVENT_SOURCE" 'NR==FNR{if(FNR>1&&$2==s)p[$1]=1;next}FNR>1&&$2==s&&$12=="false"{u[$4]=1}END{for(i in p)if(!(i in u))n++;print n+0}' "$CURRENT_REG/pages.tsv" "$CURRENT_REG/claims.tsv")
[[ "$ra" == "$aa" && "$re" == "$ae" && "$rem" == "$am" ]] || fail "census invariant count is stale"
[[ "$rt" == "$at" ]] || fail "formalization target count is stale"
ao=$(LC_ALL=C awk -F '\t' -v s="$EVENT_SOURCE" 'FNR>1&&$2==s{print}' "$CURRENT_REG/pages.tsv" "$CURRENT_REG/claims.tsv"|git hash-object --stdin); [[ "$ro" == "$ao" ]] || fail "census OID is stale"
