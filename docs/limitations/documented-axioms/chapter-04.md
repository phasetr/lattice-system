...
- **This discharges nothing.** `no_long_range_order_1d`
  (`LatticeSystem/Quantum/SpinS/NoLongRangeOrder1D.lean`) states eq. (4.1.11)
  for `∀ α : Fin 3` (Lean's index is Tasaki's `α` minus one), reading off
  Tasaki's own fourth sentence on p. 77: "The same bound holds for `α = 1` or
  `2` because the unique ground state `|Φ_GS⟩` is SU(2) invariant." For
  `N ≥ 1`, the `α = 3` instance is `no_long_range_order_1d_of_theorem_4_2` —
  the contraposition, run at a single volume through eq. (3.4.16) and the
  per-volume eq. (3.4.21) bound — applied to Theorem 4.2
  (`shastry_no_symmetry_breaking_1d`); the other two axes are transported from
  ...
  `N = 0` is unconditional (the staggered order operator vanishes on every
  axis there).
