...
- **Re-check condition:** the disposition would change only when all three of the following exist in reviewed form in this repository (or are usable from mathlib): (a) a general, reviewed cluster/polymer-expansion (or equivalent quantum-perturbation) framework with volume-uniform convergence estimates, strong enough to prove a spectral gap and exponential clustering at large D rather than assume them; (b) a math-before-code transcription of the large-D expansion for (8.1.1) — from Kennedy-Tasaki [49] or an equivalent source — that derives an explicit threshold D₀, an L-independent ΔE₀(D), and L-independent C and ξ; and (c) for the uniqueness conjunct specifically, a Lean proof of Tasaki Theorem 2.4 (pp. 43-44) applicable at S = 1 to (8.1.1) in the λ = 1, D ≥ 0 case on the even ring — the repository's Theorem 2.4 development reaches an unconditional closure for obligation (1), and its general spin-S obligation-(2) wrappers no longer need the SU(2)-endpoint global-uniqueness callback (removed per `docs/formalization/legacy/27-spin-marshall-lieb-mattis-on-the-magnetization-sector-tasa-part-03.md` row at line 62), but they carry their own complete-bipartite positivity premise `hJpos : ∀ x y, (bipartiteCompleteGraphOf A).Adj x y → 0 < (J x y).re` (`AnisotropicHeisenbergSpinSMLMEndpoint.lean:65`; identically on `AnisotropicHeisenbergStructuralGeneralN.lean:40`), which the nearest-neighbour even ring fails for L ≥ 6 — this `hJpos` premise, not the `hT23 : tasaki_2_5_theorem_2_3` hypothesis (a `def`, `Theorem23StructuralBipartiteToy.lean:40`, which itself takes the same complete-bipartite positivity as its own antecedent and so is vacuously satisfiable on the ring), is the actual gating obstruction. A connected-coupling data route already exists — `tasaki_2_5_theorem_2_3_data_of_connected` (`ConnectedTheorem23.lean:208`), whose doc comment records that it "drops the complete-bipartite `hJ_pos` premise" — but propagating it through the Theorem 2.4 general-spin-S obligation-(2) endpoints above (which still require `hJpos`) is not yet wired; that wiring is the actual remaining work item for (c). Partial progress on (c) alone does not reopen this entry, since the gap and decay conjuncts — precisely what the book attributes to [49] — would remain unproved.
