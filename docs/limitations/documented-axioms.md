...
**Tasaki §8.3.1, eq. (8.3.3)** (p. 252) is a **documented axiom** carried by two declarations in `LatticeSystem/Quantum/SpinS/SPTPhase.lean`: `vbsStringOrderParameterS` (doc comment lines 53-55, declaration line 56, an uninterpreted marker) and `tasaki_oshikawa_8_3_3` (doc comment lines 58-63, declaration lines 64-65). Unlike Theorem 8.3 above, this is not a numbered theorem of the book but a displayed equation Tasaki reports from the literature.
...
- **Consumers:** freshly re-verified (`grep -rn` over `LatticeSystem/`): `vbsStringOrderParameterS` occurs only inside `SPTPhase.lean` (its doc comment line 53, its declaration line 56, and the axiom statement at line 65) and `tasaki_oshikawa_8_3_3` only at its own declaration (line 64). No proved result depends on either. (Repository-wide the pair also appears in the legacy catalogue `docs/formalization/legacy/19-the-aklt-model-tasaki-7-1.md` and in this ledger entry, neither of which is a Lean consumer; `tex/proof-guide.tex` mentions neither name.)
...
- **Everything built on the markers is a real definition, not an axiom:** `HamiltonianPath` (structure, line 91), `ContinuouslyConnected` (line 102), `SymmetryConnected` (line 108), `IsTrivialPhase` (line 116) and `IsSPTPhase` (line 123) are `def`s/`structure`s, as the module doc comment states explicitly ("the SPT-phase notions … are honest `def`s (never axioms — the SPT phase is a definition, not a theorem)"). The axiomatization is confined to the two markers, and to them the definitions delegate all the deep content: `HamiltonianPath.gapped_unique` (line 97) requires `IsShortRangeGappedUniqueGS` along the path, `IsTrivialPhase` (line 117) requires a product-state endpoint via `IsProductStateHamiltonian`, and `IsSPTPhase` (line 124) requires `IsShortRangeGappedUniqueGS H`.
...
**Tasaki §8.3.3** (`S_LR = −Σ_j p_j log p_j`, defined just after eqs. (8.3.7)-(8.3.8), p. 262) contributes one **documented axiom**, `entanglementEntropyS` (`LatticeSystem/Quantum/SpinS/SPTTopologicalIndex.lean`, doc comment lines 66-69, declaration line 70). This is a contentless marker, not a deferred theorem: no book theorem in this repository depends on it.
...
- **Tracking:** master tracker #4718 (strict book-order axiom discharge). No dedicated discharge issue exists for `entanglementEntropyS`, and none is to be opened while the re-check condition above is unmet. Catalogue row: `docs/formalization/legacy/19-the-aklt-model-tasaki-7-1.md`, the `IsTimeReversalInvariant` / `IsBondInversionInvariant` / `vbsInversionParityS` row.
