...
**Tasaki §8.3.3** (`S_LR = −Σ_j p_j log p_j`, defined just after eqs. (8.3.7)-(8.3.8), p. 262) contributes one **documented axiom**, `entanglementEntropyS` (`LatticeSystem/Quantum/SpinS/SPTTopologicalIndex.lean`, doc comment lines 64-67, declaration line 68). This is a contentless marker, not a deferred theorem: no book theorem in this repository depends on it.
...
- **Consumers:** freshly re-verified (`grep -rn` over `LatticeSystem/`): `entanglementEntropyS` occurs only at its own declaration (`SPTTopologicalIndex.lean`, line 68) and in that module's header doc comment (line 27). There are **zero** consumers: no definition, theorem or axiom anywhere in the repository mentions it, the module header being prose only. (`tex/proof-guide.tex` mentions the name once in prose and the legacy catalogue row lists it; neither is a Lean consumer.)
