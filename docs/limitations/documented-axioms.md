...
- **Consumers:** freshly re-verified (`grep -rn` over `LatticeSystem/`), all four axioms (`neelOrderParameterS`, `stringOrderParameterS`, `tasaki_theorem_8_3`, `tasaki_neel_transverse_eq_nonneg`) occur in the repository **only** at their own declarations in `LambdaDModel.lean` (lines 57, 63, 73-74, 80-81) — zero consumers elsewhere.
- **No path-integral / quantum-to-classical infrastructure exists in this repository:** the proof Tasaki cites uses the path-integral representation of §6.3, i.e. a positive quantum-to-classical mapping. The repository's existing §6.3 development, `tasaki_prop_6_5_hhaf_spin_one` (`LatticeSystem/Quantum/SpinS/HiddenAntiferromagneticOrderUniqueness.lean`), is a Perron-Frobenius / kink-path argument on the restricted `H_HAF` subspace, not a path-integral representation, so it does not supply the needed machinery.
- **Axiom reason (documented):** Tasaki states Theorem 8.3 without proof: "See [49] for the proof (which makes use of the path integral representation as in Sect. 6.3), and [47] for an extension." (p. 240, re-verified against the rendered page), where [49] = T. Kennedy, H. Tasaki, *Hidden symmetry breaking and the Haldane phase in S = 1 quantum spin chains*, Commun. Math. Phys. **147**, 431-484 (1992) (verified against the Chapter 8 bibliography, `.self-local/refs/Hal.Tasaki.txt` line 15913, which prints the working link `https://projecteuclid.org/euclid.cmp/1104250747`), and [47] = T. Kennedy, *Non-positive matrix elements for Hamiltonians of spin-1 chains*, J. Phys.: Condens. Matter **6**, 8015-8022 (1994) (line 15909). Unlike KLT [41] for Theorem 7.7, [49] is **not recorded as inaccessible**: Tasaki's own bibliography prints a working DOI/link for it, so the correct framing is that [49] has **not yet been transcribed** into this repository as a math-before-code note — not that it is unavailable. Separately, [48] = T. Kennedy, H. Tasaki, *Hidden Z2 × Z2 symmetry breaking in Haldane-gap antiferromagnets*, Phys. Rev. B **45**, 304-307 (1992) (line 15911) is cited earlier in the chapter (p. 239) for introducing the Kennedy-Takayama nonlocal unitary transformation (§8.2.2, eq. (8.2.5)) and the hidden-Z2×Z2 picture generally — it is **not** Tasaki's cited proof source for (8.2.4) itself.
  The more decisive obstruction, beyond the missing path-integral development and the un-transcribed proof, is **statability**: footnote 13 (p. 239, re-verified against the rendered page) reads "To be rigorous the existence of the limits in (8.2.2) and (8.2.3) are not proved in general." Tasaki himself flags that the double limits defining `O_Néel` and `O_string` are not known to exist in general, so a fully faithful Lean formalization of the order parameters as literal limits (rather than as opaque markers) is not even statable in general in this repository at present, independent of any proof-transcription work. No named policy-exception class applies here (this is not perturbation theory as with Theorem 8.1, and not the C*-algebra/GNS framework as with Theorem 7.2): the obstruction is a genuine combination of (i) absent path-integral / positive-representation infrastructure, (ii) an un-transcribed external proof, and (iii) an unresolved existence gap for the underlying limits — most similar in kind to the Theorem 7.7 entry above, but for a different specific reason (a statability/existence gap, not source inaccessibility).
- **Re-check condition:** the disposition would change only when all of the following exist in reviewed form in this repository: (a) a rigorous path-integral / positive quantum-to-classical representation framework applicable to the λ-D model (or an alternative proof route not relying on one); (b) a math-before-code transcription of Kennedy-Tasaki [49] (or an equivalent independent proof of (8.2.4) and the footnote-14 companion statement); and (c) a resolution of the footnote-13 existence gap — either a proof that the limits (8.2.2)-(8.2.3) exist under the repository's hypotheses, or a restriction of the statement to a case where they provably exist. Partial progress on any one of (a)-(c) alone does not reopen this entry.
- **Tracking:** master tracker #4718 (strict book-order axiom discharge). No dedicated discharge issue exists for Theorem 8.3, and none is to be opened while the re-check condition above is unmet. Catalogue row: `docs/formalization/legacy/19-the-aklt-model-tasaki-7-1.md`, row 37 (`lambdaDChainHamiltonianS` / `tasaki_theorem_8_3`). Unlike Theorem 8.2, this row's **AXIOM** label is correct as printed — no status transition occurred here — so this entry proposes no catalogue edit.
  Two ancillary defects were found during this investigation and are recorded here for a future bundled fix; neither is fixed by this docs-only PR:
  ...
  (ii) `LatticeSystem/Quantum/SpinS/HiddenAntiferromagneticOrder.lean` is internally inconsistent about the status of Proposition 6.5: its module doc comment (line 39) still describes Proposition 6.5 as "recorded as a documented axiom", but the file's own trailing comment (lines 1042-1046) correctly states "This was formerly a documented axiom; it is now **proved** as `tasaki_prop_6_5_hhaf_spin_one` in the companion module `Quantum/SpinS/HiddenAntiferromagneticOrderUniqueness.lean`" (confirmed: `tasaki_prop_6_5_hhaf_spin_one` is a theorem at `HiddenAntiferromagneticOrderUniqueness.lean:1110`). Proposition 6.5 has no entry in this ledger, so this is a stale in-source doc comment, not a ledger correction; worth fixing separately.
