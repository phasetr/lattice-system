...
- **Consumers:** freshly re-verified (`grep -rn` over `LatticeSystem/`): `entanglementEntropyS` occurs only at its own declaration (`SPTTopologicalIndex.lean`, line 83) and in that module's header doc comment (line 28). There are **zero** consumers: no definition, theorem or axiom anywhere in the repository mentions it, the module header being prose only. (`tex/proof-guide.tex` mentions the name once in prose and the legacy catalogue row lists it; neither is a Lean consumer.)
