...
| `inner_basisVec_onSite_spinHalfOp3_mul_onSite_spinHalfOp3_basisVec` | generic `⟨basisVec σ, Ŝ^(3)_x · Ŝ^(3)_y · basisVec σ⟩ = spinHalfSign σ x · spinHalfSign σ y`. The diagonal-only spin-spin correlator on a basis state | `Quantum/SpinDot.lean` |
...
| `neelChainState_inner_onSite_spinHalfOp3_mul_onSite_spinHalfOp3_adjacent_eq_neg_one_quarter` | per-adjacent-bond `Ŝ^(3)_x · Ŝ^(3)_y` correlation on the Néel chain: `-1/4`. Matches the full `Ŝ_x · Ŝ_y` expectation since the off-diagonal `Ŝ^x·Ŝ^x + Ŝ^y·Ŝ^y` parts vanish on the diagonal (they map `|σ⟩` to `|swap σ⟩ ⊥ |σ⟩`) | `Quantum/NeelState.lean` |
| `neelChainState_inner_onSite_spinHalfOp3_mul_onSite_spinHalfOp3_wrap_eq_neg_one_quarter` | 1D Néel periodic chain: per-wrap-bond `Ŝ^(3)_x · Ŝ^(3)_y` correlation `-1/4` | `Quantum/NeelState.lean` |
| `neelSquareState_inner_onSite_spinHalfOp3_mul_onSite_spinHalfOp3_{horizontal,vertical}_{adjacent,wrap}_eq_neg_one_quarter` | 2D Néel: per-bond `Ŝ^(3)·Ŝ^(3)` correlation `-1/4` for every horizontal / vertical adjacent and wrap bond | `Quantum/NeelState.lean` |
| `neelCubicState_inner_onSite_spinHalfOp3_mul_onSite_spinHalfOp3_{x,y,z}_{adjacent,wrap}_eq_neg_one_quarter` | 3D Néel: per-bond `Ŝ^(3)·Ŝ^(3)` correlation `-1/4` for every x / y / z adjacent and wrap bond. Completes the `Ŝ^z·Ŝ^z` correlation coverage parity with the full `Ŝ·Ŝ` family from #273 | `Quantum/NeelState.lean` |
