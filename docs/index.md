...
| `singleClusterGSEnergyS_re_eq` / `singleClusterMaxEnergyS_re_eq` | **Explicit `.re` forms** as `ℝ` values: `Re(GS) = -(N/2)(zN/2+1)`, `Re(Max) = z·N²/4`. Useful for downstream real comparisons (γ-5 step 278) | `Quantum/SpinS/SingleClusterHamiltonian.lean` (PR pending) |
