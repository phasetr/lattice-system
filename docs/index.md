...
| `spinSOp1_commutator_spinSOp2` | **Cyclic SU(2) commutator** `[Ŝ^{(1)}, Ŝ^{(2)}] = i · Ŝ^{(3)}`. Derived from the Cartan relations by algebraic manipulation through `Ŝ^{(1)} = (Ŝ^+ + Ŝ^-)/2` and `Ŝ^{(2)} = (Ŝ^+ − Ŝ^-)/(2i)`: `(P+Q)(P-Q) − (P-Q)(P+Q) = -2[P, Q] = -4 Ŝ^{(3)}`, then scalar simplification `-1/I = I` (β-20 of Issue #458). The two remaining cyclic commutators `[Ŝ^{(2)}, Ŝ^{(3)}]` and `[Ŝ^{(3)}, Ŝ^{(1)}]` follow by analogous manipulations (deferred) | `Quantum/SpinS/CyclicCommutator.lean` |
