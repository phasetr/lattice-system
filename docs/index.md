...
| `neelStateOfS_totalSpinSSquared_expectation_re_gt_OpZ_sq` / `neelStateOf_totalSpinHalfSquared_expectation_re_gt_OpZ_sq` | **Strict spread in ℝ** of total Casimir on Néel: `Re <Φ_Néel \| (Ŝ_tot^(3))² \| Φ_Néel> < Re <Φ_Néel \| (Ŝ_tot)² \| Φ_Néel>` when `Λ` non-empty (and `0 < N` for spin-S). Direct corollary of γ-4 step 161 plus the operator decomposition `(Ŝ_tot)² = (Ŝ^(1))² + (Ŝ^(2))² + (Ŝ^(3))²`. Witnesses that the Néel state is not concentrated in a single `S_tot`-sector (γ-4 step 162) | `Quantum/SpinS/SublatticeCasimirNeel.lean` and `Quantum/MarshallLiebMattis/SublatticeCasimirNeel.lean` (PR #1209) |
