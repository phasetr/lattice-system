...
| `saturatedFerromagnetJointEigenspace_inf_magSubspaceS_eq_span_ladderIterateUp` | **per-sector identification = span (ladderIterateUp k)**: for every `k : Fin (Fintype.card V * N + 1)`, `joint ⊓ H_{m_max - k} = Submodule.span ℂ {ladderIterateUp V N k}`. Combines per-sector ≤ 1-dim (PR #2763) with `ladderIterateUp V N k ∈ joint ⊓ H_{m_max-k}` (PR #2762) and `ladderIterateUp V N k ≠ 0` (PR #895). Apply `Submodule.eq_of_le_of_finrank_le`. Identifies each ladder iterate as the unique (up to scaling) non-zero vector in its joint-magnetisation sector. Summing over the 2m_max + 1 spectrum values via PR #889 will give `joint = span(ladderIterateUp)` (next PR) (PR #2764) |
