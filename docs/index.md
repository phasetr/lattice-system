...
| `sublatticeSpinHalfSquared_eq_sum_dot` / `sublatticeSpinHalfSquared_mulVec_basisVec_const` / `_all_up` / `_all_down` | `(Ŝ_A)² = Σ_{x ∈ A} Σ_{y ∈ A} Ŝ_x · Ŝ_y` (specialisation `B = A` of the bilinear expansion), and the **maximum-spin Casimir eigenvalue on the all-aligned state**: `(Ŝ_A)² · \|s s … s⟩ = (\|A\|·(\|A\|+2)/4) · \|s s … s⟩` for any `s : Fin 2`. The all-up state simultaneously diagonalises `(Ŝ_tot)²`, `(Ŝ_A)²`, `(Ŝ_¬A)²` with respective max-spin eigenvalues `\|Λ\|·(\|Λ\|+2)/4`, `\|A\|·(\|A\|+2)/4`, `\|¬A\|·(\|¬A\|+2)/4` | `Quantum/MarshallLiebMattis/SublatticeSpinDot.lean` |
...
| `heisenbergToyHamiltonian_mulVec_basisVec_const` | explicit eigenvalue of `Ĥ_toy` on the all-aligned state: `Ĥ_toy · \|s s … s⟩ = (\|Λ\|·(\|Λ\|+2)/4 − \|A\|·(\|A\|+2)/4 − \|¬A\|·(\|¬A\|+2)/4) · \|s s … s⟩`. Computed from the closed form `Ĥ_toy = (Ŝ_tot)² − (Ŝ_A)² − (Ŝ_¬A)²` and the three Casimir maximum-spin eigenvalues | `Quantum/MarshallLiebMattis/ToyHamiltonianCasimir.lean` |
