...
| `singleClusterHamiltonianS_eigenvalue_pentamer` | **Pentamer eigenvalue from `Stot²` + leaf-leaf sum**: for z=4, joint eigenvector of `Stot² (=α)` and 6-pair sum `(=γ)` is an `H`-eigenvector at `(α − 5N(N+2)/4 − 2γ)/2`. Specialisation of γ-5 step 259 using γ-5 step 312 (γ-5 step 313) | `Quantum/SpinS/SingleClusterHamiltonian.lean` (PR pending) |
