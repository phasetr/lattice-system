...
| `singleClusterHamiltonianS_eigenvalue_trimer_top` | **Trimer top-spin sector eigenvalue at Max energy**: for z=2, joint eigenvector at `s_tot=3N/2` (max) and leaf triplet `s_R=N` is an `H`-eigenvector at `singleClusterMaxEnergyS 2 N = N²/2` (γ-5 step 295) | `Quantum/SpinS/SingleClusterHamiltonian.lean` (PR pending) |
