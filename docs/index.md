...
| `neelStateOfS_pair_span_le_magSubspaceS_sup` / `neelStateOf_pair_span_le_magnetizationSubspace_sup` | **Néel-pair span ⊆ direct sum of opposite-sign magnetization sectors**: `span ℂ {Φ_Néel(A), Φ_Néel(¬A)} ≤ magSubspace M_pos ⊔ magSubspace M_neg`. Direct via `Submodule.mem_sup_left/right` from γ-4 step 176 + 194 (γ-4 step 197) | `Quantum/SpinS/SublatticeCasimirNeel.lean` and `Quantum/MarshallLiebMattis/SublatticeCasimirNeel.lean` (PR #1244) |
