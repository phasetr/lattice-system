...
| `singleClusterHamiltonianS_eigenvalue_quartet_leaf_singlet` | **Quartet leaf-singlet sector eigenvalue = 0**: for z=3, joint eigenvector at `s_tot=N/2` and 3-pair sum `(-3N(N+2)/8)·v` (leaves in singlet `s_R=0`) gives `H · v = 0`. Decoupling sector (γ-5 step 321) | `Quantum/SpinS/SingleClusterHamiltonian.lean` (PR pending) |
