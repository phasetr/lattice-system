...
| `leafSpinSSquared_one_mulVec` / `singleClusterHamiltonianS_eigenvalue_dimer` | **Dimer eigenvalue from Stot² alone**: at z=1, `leafSpinSSquared` acts as the scalar `N(N+2)/4`, so any Stot² eigenvector is auto a joint eigenvector with `H · v = ((α − N(N+2)/2) / 2) • v` (γ-5 step 286) | `Quantum/SpinS/SingleClusterHamiltonian.lean` (PR pending) |
