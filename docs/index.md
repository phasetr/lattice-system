...
| `neelStateOfS_complement_orthogonal` / `neelStateOf_complement_orthogonal` and `neelConfigOfS_ne_complement` / `neelConfigOf_ne_complement` | **Néel-complement orthogonality**: `<Φ_Néel(A) \| Φ_Néel(¬A)> = 0` when `Λ` non-empty (and `0 < N` for spin-S). The two Néel states are basis vectors at distinct configurations (`neelConfigOf A` vs `neelConfigOf (¬A)` differ at every site), hence orthogonal (γ-4 step 171) | `Quantum/SpinS/SublatticeCasimirNeel.lean` and `Quantum/MarshallLiebMattis/SublatticeCasimirNeel.lean` (PR #1218) |
