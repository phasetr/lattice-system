...
`shastry_no_symmetry_breaking_1d` does, and states eq. (4.1.11) for all three Cartesian axes
(PR #5427) rather than the `α = 3` instance alone — the `α = 3` axis by that same contraposition,
the other two by reading off Tasaki's own fourth sentence on p. 77 (SU(2) invariance of the unique
ground state); `shastry_no_symmetry_breaking_1d` is unchanged. The corollary rests on no
susceptibility axiom of its own; it discharges nothing, and both Corollary 4.3 and Theorem 4.2
remain open. The assembly is tracked in
[#5416](https://github.com/phasetr/lattice-system/issues/5416), closed.
...
(`Quantum/HorschVonderLindenLowLyingState.lean`). The `L ↑ ∞` and `h ↓ 0` limits of Theorem 3.2
