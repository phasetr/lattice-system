...
Ô⁺ Γ_M⟩/L^d` in the window is within `ε` of `m∗` (math-note §3.4 refinement 2), which subsumes the
`θ=0` mean limit `⟨Ξ_0, Ô^(1) Ξ_0⟩/L^d → m∗` (eq. 5.3.7) **and** drives the eq. 5.3.8 second-moment
