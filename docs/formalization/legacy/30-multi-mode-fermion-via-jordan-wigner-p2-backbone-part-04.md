...
| Lean name | Statement | File |
|---|---|---|
| `symmetricRepulsiveHubbardHamiltonian_groundSubmodule_eq_uniform` | **Eq. (10.2.9): symmetric-interaction ground submodule reduces to the uniform-interaction one** (Tasaki, 1st ed. (2020), §10.2.2, p. 351, eq. (10.2.9); **PROVED**, `#print axioms` = std3, Issue #5320, PR #5320 PR-1): per site, `U(n̂↑−½)(n̂↓−½) = U n̂↑n̂↓ − (U/2)(n̂↑+n̂↓) + U/4`, so summing over all `N+1` sites gives `Ĥint^{sym}(U) = Ĥint^{unif}(U) − (U/2)N̂ + (U/4)|Λ|`, and hence, restricted to the `N̂ = Ne` electron-number sector, `Ĥ^{rep,sym}(U) = Ĥ^{rep,unif}(U) + c·1` with the scalar shift `c = −(U/2)Ne + (U/4)(N+1)`. Consequently, for every target energy `E`, the `E`-ground submodule of the symmetric-interaction Hamiltonian on the `Ne`-electron sector equals the `(E−c)`-ground submodule of the uniform-interaction Hamiltonian on the same sector — the two variants have the same ground submodule up to the constant energy shift `c`, justifying Tasaki's reduction of the uniform case (eq. (10.2.5)) to the symmetric case (eq. (10.2.6)) used throughout the proof of Theorem 10.4. Axiom-free; finite-dimensional linear algebra (per-site algebraic identity + `Finset.sum` bookkeeping), no operator-algebra dependency. | `Fermion/JordanWigner/Hubbard/LiebRepulsiveBalancedGround.lean` |
