...
| `minEnergyOn` / `minEnergyOn_isGroundEigenvalueOn` / `abs_minEnergyOn_sub_le_sum_entryNorms` / `Continuous.minEnergyOn_comp` | **Minimum energy on a subspace** (generic infrastructure for the Theorem 10.4 discharge arc, Issue #5320, PR #5321 PR-2; **PROVED**, `#print axioms` = std3): `minEnergyOn W H = inf {re⟪v,Hv⟫ \| v∈W, ‖v‖=1}` restricts the whole-space `hermitianMinEigenvalue` variational functional (`Quantum/SpinS/HermitianMinEigenvalueLipschitz.lean` / `HermitianMinEigenvalueContinuous.lean`) to unit vectors of a fixed nonzero subspace `W`. `minEnergyOn_isGroundEigenvalueOn` repackages the arc's own reachability lemma `exists_unit_eigenvector_min_energy_on_invariant` (`DegeneratePerturbation.lean`) as an `IsGroundEigenvalueOn` witness; the entry-norm Lipschitz bound and its parameter-continuity corollary mirror `abs_hermitianMinEigenvalue_sub_le_sum_entryNorms` / `Continuous.hermitianMinEigenvalue_comp`. Not itself a direct formalization of a numbered Tasaki statement — a reusable building block feeding the continuity arguments of Theorem 10.4's proof. Axiom-free. | `Math/MatrixAnalysis/MinEnergyOnSubspace.lean` |
