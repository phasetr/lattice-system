...
## Hard merge gate（rewrite program 全 PR、最優先）

PR #5480 を含む rewrite program のすべての PR は、ユーザーがその **対象 PR 番号**と
**現在の exact head SHA** を同じ確認で明示し、「その head を merge してよい」と現在形で
指示するまで merge してはならない。phase や branch による例外はない。merge、auto-merge、
merge queue への投入、protected trunk への direct push と force push はすべて禁止する。
CI の GREEN、review の APPROVE、過去の承認、別 PR への承認、設計承認、作業開始承認は merge
権限ではない。承認後に head SHA が変化した場合、その承認は失効し、新しい exact head SHA
への明示確認を取り直す。agent は PR template の merge-confirmation checkbox を自ら check
してはならない。

## Status / Decision

- Status: **R1 atomic complete-reset bootstrap 実装中**。本書の設計 decision はこの root 文書を
  正本とし、上記 hard merge gate は後続のすべての指示より優先する。
...
- 基準 revision: `01bcb49d49db92c225cfa74b74d409dd0a9c4edc`
  ...
  revision で得た。
- R1 は legacy Lean source/tests/docs/TeX/status/scripts/workflows を削除し、依存 pin を変えず
  scratch root、minimal CI、empty blueprint を同じ atomic bootstrap PR で成立させる。

本提案の decision は次の通りである。

1. 旧 `main` は履歴・比較・証明アイデアを読むための **frozen legacy** として残す。
   新系列に旧公開 API との互換性は要求しない。
2. R1 atomic bootstrap PR で、旧 `LatticeSystem/**`、tests、root aggregator、
...
- 再実装 trunk は **`rewrite-main`** とする。`rewrite-main` は上記 anchor から派生し、
  履歴比較可能性を保つ。
- `redesign/tasaki-reimplementation-plan` は `rewrite-main` への設計 PR として用いられ、本設計は
  R1 で root `DESIGN.md` へ移される。
- 元の設計 PR は設計文書だけを変更し、R1 bootstrap が reset を実装する。
...
一つの R1 bootstrap PR で、旧 `LatticeSystem/**` の production code と tests、
...
- 元の設計 PR では設計文書だけが変更され、R1 でこの root file へ移されている。
...
- reset 実装が元の設計 PR と分離された R1 bootstrap であることが明記されている。
...
## Documentation boundary after R1

R1 は legacy `tex/` と旧 docs/status 複製を全面削除し、本設計を root `DESIGN.md` へ移し、
README と CI を scratch rewrite する。reset 後は `docs/` / `tex/` を置かず、ユーザーの将来の
明示承認なしに新設しない。以後の status は claim registry/binding と Lean environment からだけ
生成する。
