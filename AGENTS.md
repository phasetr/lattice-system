...
3. The committed project documents:
   ...
   - `docs/formalization/legacy/` — interim formalization-status and theorem
     catalogue authority until Issue #5228 performs the structured cutover
...
- **Updating the authoritative page under `docs/formalization/legacy/` and
  `tex/proof-guide.tex`** in the same PR is mandatory whenever a new `def` /
  `theorem` / `lemma` is committed. `docs/index.md` is the landing page, not the
  theorem ledger. Issue #5228 alone changes this update path to the validated
...
4. Flag any missing row update in the authoritative
   `docs/formalization/legacy/` catalogue or stale `(PR pending)` tag (the
   author updates these in the next PR — that is expected). Issue #5228 alone
   cuts this reviewer check over to the validated structured catalogue.
