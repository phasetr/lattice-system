...
/-!
# Signature pins: general-graph generalization of the Theorem 2.4 engine (Red fixture)
...
`G : SimpleGraph Λ`, together with the explicit sign-gauge hypothesis
`hGbip : ∀ x y, G.Adj x y → A x ≠ A y`. No pin assumes connectivity: `G` is an arbitrary
`SimpleGraph Λ` constrained only by that gauge. Connectivity is needed one layer up, where the
reachability hypotheses these engines take are discharged (`ParityReachConnectedTotal.lean`).

The gauge is taken explicitly in all twelve for a uniform calling convention, not because it is
everywhere underivable: in the eleven that also carry `hJbip : ∀ x y, J x y ≠ 0 → A x ≠ A y` it
follows from `hJ_pos_G`, which makes `J x y` nonzero on an edge. Pin 4
(`neg_dressedHeisenbergSReMatrix_apply_pos_of_raiseLowerStepS_bipartite`) has no `hJbip`; there
the gauge is both underivable and indispensable, since on a same-sublattice bond the Marshall
sign product is `+1` rather than `-1` while the bare Heisenberg entry on a raise/lower witness is
strictly positive regardless of `A`, so the dressed entry comes out positive and the conclusion
fails.
...
Callers whose statements PR-2a leaves untouched are deliberately **not** pinned here, among them
the unconditional complete-bipartite engines
(`shiftedDressedAxisSwappedReMatrixOnParityBlock_isIrreducible`,
`..._isIrreducible_lambda_one_D_pos`, `..._isIrreducible_D_nonneg`), which stay at
`bipartiteCompleteGraphOf A` via `hA_ne`/`hB_ne`. Generalizing the twelve engines they call
changes only what their proofs must supply at the call site, where each now builds the gauge
inline from `bipartiteCompleteGraphOf_adj_sublattice_ne`; the build, not this fixture, holds those
call sites to the generalized signatures.
...
`G` whose edges join opposite sublattices. -/
...
`G` whose edges join opposite sublattices, `D.re ≥ 0`. -/
example (A : Λ → Bool) (G : SimpleGraph Λ) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hGbip : ∀ x y, G.Adj x y → A x ≠ A y)
    (hJ_pos_G : ∀ x y, G.Adj x y → 0 < (J x y).re)
...
general `G` whose edges join opposite sublattices. -/
...
a general `G` whose edges join opposite sublattices. -/
example (A : Λ → Bool) (G : SimpleGraph Λ) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hGbip : ∀ x y, G.Adj x y → A x ≠ A y)
    (hJ_pos_G : ∀ x y, G.Adj x y → 0 < (J x y).re)
...
reachability at a general `G` whose edges join opposite sublattices. -/
example (A : Λ → Bool) (G : SimpleGraph Λ) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hGbip : ∀ x y, G.Adj x y → A x ≠ A y)
    (hJ_pos_G : ∀ x y, G.Adj x y → 0 < (J x y).re)
...
at a general `G` whose edges join opposite sublattices, case (i.2) strict. -/
example (A : Λ → Bool) (G : SimpleGraph Λ) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hGbip : ∀ x y, G.Adj x y → A x ≠ A y)
    (hJ_pos_G : ∀ x y, G.Adj x y → 0 < (J x y).re)
...
general `G` whose edges join opposite sublattices, and an external total-reachability
hypothesis over `G`. -/
example (A : Λ → Bool) (G : SimpleGraph Λ) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hGbip : ∀ x y, G.Adj x y → A x ≠ A y)
...
irreducibility engine, at a general `G` whose edges join opposite sublattices. -/
example (A : Λ → Bool) (G : SimpleGraph Λ) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hGbip : ∀ x y, G.Adj x y → A x ≠ A y)
    (hJ_pos_G : ∀ x y, G.Adj x y → 0 < (J x y).re)
...
connected, every one of its edges joins opposite sublattices under this marking — so it satisfies
the sign gauge the pins above take — and it is genuinely a different graph from
...
for it via the unchanged theorems. The gauge conjunct is the one that makes the marking
load-bearing: it is false for the marking `{0, 1}`, for which the other three conjuncts still
hold. -/
example :
