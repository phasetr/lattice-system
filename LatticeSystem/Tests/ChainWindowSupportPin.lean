...
contains every site with `a ≤ z.val ≤ b`. For `a ≤ b`, `[a + 1, b]` omits both probes, so the
necessity direction accepts it, and it is the sufficiency direction that rejects it, at
`z.val = a`; the proviso is needed because at `b < a` both windows are empty and there is nothing
to reject, and the theorem assumes no `a ≤ b`. This pin is the guard: it writes the outside-window
condition out verbatim and holds it equivalent to support on the window, so the equivalence is
proved through a lemma that must hold of whatever `chainWindow` is defined to be, and the pinned
disjunction determines the set.
