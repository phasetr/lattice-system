...
and `E = 0`.  The guarantee is narrower than it may look: what the type checker holds is the
linkage through the `exact` in `fieldWitness_capstone`, so reproving that declaration by another
route would leave this conjunction standing on its own, no longer checked against the capstone.
What it does buy is real: the concrete hypotheses are a named, greppable statement rather than
anonymous goals inside a proof term; they are checked against the capstone for as long as
`fieldWitness_capstone` applies them; they are written out independently of the capstone's
signature and so survive a change there; and an orphaned copy would trip the repo's zero-reference
rule.  `hen` is met at equality and carries the upper half of eq. (3.4.12) as printed on p. 67,
not the halved bound of the `Ξ₊` instantiation. -/
