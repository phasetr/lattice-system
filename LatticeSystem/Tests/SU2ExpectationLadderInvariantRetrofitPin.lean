...
(`Math/MatrixAnalysis/LadderExpectationRatio.lean`), keeping both **types** unchanged and turning
only their proofs into instantiations of the generic lemma. The one binder-level change is a
rename: since
