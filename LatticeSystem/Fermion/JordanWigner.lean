...
| `Hubbard/TanakaTasakiModel.lean` | **Tasaki §11.5.4 the Tanaka–Tasaki model + Theorem 11.27 (AXIOMATIZED, Issue #4198)** — the §11.5 / Chapter 11 capstone: the heavily-decorated lattice `Λ = E×{1,2,3} ∪ I×{1,2}` (externals triplicated, internals duplicated), d=1 packed into `Fin(5K+5)` (`ttExtSite`/`ttIntSite`); special single-particle states `ttAlpha`/`ttBeta`/`ttDeltaP`/`ttDeltaI` (eqs. (11.5.19)–(11.5.22); `â` carries the `1/√(3+4ν²)` normalisation) + fermion ops `ttA/B/DeltaP/DeltaICreation/Annihilation` (via `spinfulCreationFromVector`); `ttHopping` (`Σ_{⟨p,q⟩}(−s â†â − t b̂†b̂) + u₁ Σ b̂†b̂ + u₂ Σ d̂†d̂`, eq. (11.5.24)) + `ttInteraction` (`U Σ n̂↑n̂↓`) + `ttHamiltonian`. **`axiom theorem_11_27`** (d=1, `u₁>2(|s|+2|t|)`, `K+1≤Ne≤2(K+1)` (Tasaki's `L^d≤N≤2L^d`) ⇒ in the limit `u₂,U↑∞` every ground state in `groundSubmoduleAtFilling (ttHamiltonian …) Ne` has maximum total spin `S_tot=Ne/2`, i.e. is an `(Ŝ_tot)²` eigenvector at `(Ne/2)(Ne/2+1)`; weaker than the full multiplet predicate since Tasaki claims only the spin, not the degeneracy; metallic when `1<Ne/L<2`). Model axiom-free; `theorem_11_27` documented axiom (Tasaki cites [63]). **Completes §11.5 / Chapter 11** |
