...
| `Hubbard/TJSectorHopAction.lean` | **Tasaki §11.5: the forward-hop matrix element on the sector basis** (Issue #4230 PR3c-4): `tJ_uphop_forward_mulVec`/`tJ_downhop_forward_mulVec` — for a forward allowed hop (`a.val < b.val`, source `a` of spin σ, target `b` empty), `ĉ†_{bσ}ĉ_{aσ}\|Φ_s⟩ = (-1)^(occupied modes strictly between (a,σ),(b,σ)) · \|Φ_{tJSiteHop s a b}⟩`, composing the single-hop action + config identity + sign parity. Axiom-free |
| `Hubbard/TJSectorHopNN.lean` | **Tasaki §11.5: the NN hop is sign-free on the sector basis** (Issue #4230 PR3c-5): `tJ_uphop_nn_mulVec`/`tJ_downhop_nn_mulVec` — for a forward nearest-neighbour hop (`b.val = a.val + 1`, non-wrap), the single intervening orbital is forced empty by the hop conditions, so the strictly-between exponent is `0` and `ĉ†_{bσ}ĉ_{aσ}\|Φ_s⟩ = \|Φ_{tJSiteHop s a b}⟩` (no sign). Axiom-free |
| `Hubbard/TJSectorHopBackward.lean` | **Tasaki §11.5: the leftward NN hop** (Issue #4230 PR-B7-3b): `tJ_uphop/downhop_backward_nn_mulVec` (`ĉ†_{aσ}ĉ_{bσ}\|Φ_s⟩ = \|Φ_{tJSiteHop s b a}⟩`, electron `b→a`, sign-free `+1` via `jwSign_mul_jwSign_update_backward`) + `_matrixElement` (`= [s'=tJSiteHop s b a]`). The leftward kinetic terms of the cyclic hopping. Axiom-free |
| `Hubbard/TJSectorHopBackwardWrap.lean` | **Tasaki §11.5: the leftward wrap hop** (Issue #4230 PR-B7-3c): `tJ_uphop/downhop_backward_wrap_mulVec` + `_matrixElement` — the leftward wrap hop `ĉ†_{0σ}ĉ_{Nσ}\|Φ_s⟩ = \|Φ_{tJSiteHop s b a}⟩` (electron `N→0`), sign-free `+1` for odd `Ne` (between = `Ne−1` via the backward sign + boundary sums). Axiom-free |
| `Hubbard/TJOccupationCount.lean` | **Tasaki §11.5: total occupation count + three-way mode split** (Issue #4230 PR3c-6): `tJConfigOf_total_count` (`∑_k (tJConfigOf s k).val = #↑+#↓`, the electron number) and `sum_split_le_between_ge` (model-agnostic: `∑_all = ∑_{k≤q} + ∑_{q<k<p} + ∑_{k≥p}`). Infrastructure for the wrap-bond hop sign `(-1)^(Ne−1)`. Axiom-free |
| `Hubbard/TJSectorHopWrap.lean` | **Tasaki §11.5: the wrap bond is sign-free for odd `Ne`** (Issue #4230 PR3c-7): `tJ_uphop_wrap_mulVec`/`tJ_downhop_wrap_mulVec` — for the cycle's wrap bond (`a.val=0`, `b.val=N`), source `a` of spin σ, target `b` empty, odd `Ne=#↑+#↓`, the strictly-between occupation is `Ne−1` (even, via the three-way split + total count + boundary sums), so `ĉ†_{bσ}ĉ_{aσ}\|Φ_s⟩ = \|Φ_{tJSiteHop s a b}⟩` (no sign). Axiom-free |
...
| `Hubbard/TJSectorExchange.lean` | **Tasaki §11.5: the exchange spin-flip on the sector basis** (Issue #4230 PR-B1/B2): `tJSpinSwap s i j` (swap the spins of sites `i,j`) + `tJConfigOf_tJSpinSwap` (config identity) + `fermionSiteSpinPlus_mul_Minus_mulVec_tJConfigOf` — for `s i=↓`, `s j=↑` (`i≠j`), `Ŝ⁺_iŜ⁻_j\|Φ_s⟩ = \|Φ_{tJSpinSwap s i j}⟩` with **net JW sign +1** (the two same-site creation/annihilation pairs each contribute a squared string sign `=1`, via `jwSign_succ_cancel_low/high`). Axiom-free |
| `Hubbard/TJKineticSector.lean` | **Tasaki §11.5: the kinetic sandwich reduction on the sector basis** (Issue #4230 PR-B3): `tJKinetic_sandwich_mulVec_tJConfigOf` — since `tJConfigOf s` is hard-core, the inner projection fixes `\|Φ_s⟩`, so `P̂hc·K·P̂hc\|Φ_s⟩ = P̂hc(K\|Φ_s⟩)`; `tJKinetic_sandwich_mulVec_mem` (the result is hard-core). Isolates the hopping matrix elements from the projection bookkeeping. Axiom-free |
| `Hubbard/TJKineticMatrixElement.lean` | **Tasaki §11.5: the forward hop matrix elements** (Issue #4230 PR-B4): `tJ_uphop_nn_matrixElement`/`tJ_downhop_nn_matrixElement` + wrap variants — `⟨Φ_{s'}\|ĉ†_{bσ}ĉ_{aσ}\|Φ_s⟩ = [s' = tJSiteHop s a b]` for a forward allowed NN/wrap hop, combining the sign-free hop actions with the basis orthonormality (`tJConfigOf_basisVec_inner`). The off-diagonal `−τ` kinetic entry. Axiom-free |
| `Hubbard/TJExchangeMatrixElement.lean` | **Tasaki §11.5: the exchange matrix element** (Issue #4230 PR-B5): `fermionSpinFlip_matrixElement` — `⟨Φ_{s'}\|Ŝ⁺_iŜ⁻_j\|Φ_s⟩ = [s' = tJSpinSwap s i j]` for an antiparallel pair (`s i=↓`, `s j=↑`, `i≠j`), combining the sign-free exchange action with the basis orthonormality. The off-diagonal `−J/2` exchange entry. Axiom-free |
| `Hubbard/TJDiagonalMatrixElement.lean` | **Tasaki §11.5: the diagonal terms have no off-diagonal matrix element** (Issue #4230 PR-B6): `fermionSiteNumber_mulVec_basisVec`/`fermionSiteSpinZ_mulVec_basisVec` (diagonal actions) + `diagonal_offdiag_matrixElement_eq_zero` (a scalar-acting operator's off-diagonal ME vanishes) + `fermionSiteNumber_mul_offdiag_matrixElement_eq_zero`/`fermionSiteSpinZ_mul_offdiag_matrixElement_eq_zero` — the `n̂_xn̂_y` and `Ŝ³_xŜ³_y` terms contribute `0` off-diagonal. Axiom-free |
