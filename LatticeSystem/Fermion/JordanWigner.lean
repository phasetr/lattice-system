...
| `Hubbard/TJModel.lean` | **Tasaki §11.5.2 the ferromagnetic t-J model (eq. (11.5.4)) + Proposition 11.24 (AXIOMATIZED)**: `fermionSiteNumber N i` (`n̂_x = n̂_{x↑}+n̂_{x↓}`); **`tJHamiltonian N G τ J`** = `−τ P̂hc (Σ_{⟨x,y⟩,σ}ĉ†_{x,σ}ĉ_{y,σ}) P̂hc + J Σ_{⟨x,y⟩}(n̂_x n̂_y/4 − Ŝ_x·Ŝ_y)` (hopping = `hubbardKineticOnGraph` sandwiched by `hubbardHardcoreProjection`, exchange via `fermionSpinDot`; ordered-pair bond sums, `τ,J>0`). Filling-fixed ground-subspace infra: `tJFillingHardcoreStates M Ne` (unit vectors at fixed `N̂=Ne` in the hard-core subspace `H_{Ne}^hc`), `tJGroundEnergyAtFilling H Ne` (ground energy), `tJGroundSubmodule H Ne` (`H`-eigenspace at the ground energy ⊓ `N̂=Ne` ⊓ `hubbardHardcoreSubspace`). **`axiom proposition_11_24`** (d=1 periodic `cycleGraph (N+1)`, `Ne<L` odd ⇒ `IsMaximalSpinMultipletSubmodule N (tJGroundSubmodule …) Ne` — ground states `S_tot=Ne/2` **and** `Ne+1`-fold degeneracy, both via the shared predicate reused from Mielke 11.13 / general flat-band 11.15) — Perron–Frobenius/spin-charge-separation proof deferred. Model + ground-subspace infra axiom-free (Issue #4198) |
