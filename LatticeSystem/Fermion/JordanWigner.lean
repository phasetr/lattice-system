...
| `Hubbard/TJModel.lean` | **Tasaki §11.5.2 the ferromagnetic t-J model (eq. (11.5.4)) + Proposition 11.24 (AXIOMATIZED)**: `fermionSiteNumber N i` (`n̂_x = n̂_{x↑}+n̂_{x↓}`); **`tJHamiltonian N G τ J`** = `−τ P̂hc (Σ_{⟨x,y⟩,σ}ĉ†_{x,σ}ĉ_{y,σ}) P̂hc + J Σ_{⟨x,y⟩}(n̂_x n̂_y/4 − Ŝ_x·Ŝ_y)` (hopping = `hubbardKineticOnGraph` sandwiched by `hubbardHardcoreProjection`, exchange via `fermionSpinDot`; ordered-pair bond sums, `τ,J>0`). Filling-fixed ferromagnetism infra: `chargeSpinSector M Ne twoS` (unit vectors at fixed `N̂=Ne` AND `(Ŝ_tot)²=(twoS/2)(twoS/2+1)`), `sectorMinEnergyAtFilling H Ne twoS` (`E_min(N_e,S)`), `exhibitsFerromagnetismAtFilling H Ne twoSmax`. **`axiom proposition_11_24`** (d=1 periodic `cycleGraph (N+1)`, `Ne<L` odd ⇒ `exhibitsFerromagnetismAtFilling` at `S_max=Ne/2`, i.e. ground states `S_tot=Ne/2`, `Ne+1`-fold) — Perron–Frobenius/spin-charge-separation proof deferred. Model + sector infra axiom-free (Issue #4198) |
