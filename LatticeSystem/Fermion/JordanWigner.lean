...
| `Hubbard/TJSectorExchange.lean` | **Tasaki §11.5: the exchange spin-flip config identity** (Issue #4230 PR-B1): `tJSpinSwap s i j` (swap the spins of sites `i,j`) + `tJConfigOf_tJSpinSwap` — for `s i=↓`, `s j=↑`, the spinful occupation of `tJSpinSwap s i j` is `tJConfigOf s` with the four orbitals `(j↑,j↓,i↓,i↑)` updated to `(0,1,0,1)` (the config produced by the ladder `Ŝ⁺_iŜ⁻_j`, up to the JW sign). Axiom-free |
