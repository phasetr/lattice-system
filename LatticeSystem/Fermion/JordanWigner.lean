...
| `Hubbard/MielkeTheorems.lean` | **Tasaki §11.3.2 Theorems 11.12 & 11.13 (AXIOMATIZED)**: `mielkeFlatBandDim` `D(Λ̃,B̃)` (`=\|B̃\|-\|Λ̃\|+1` bipartite else `\|B̃\|-\|Λ̃\|`), `mielkeSingleElectronOp` (the 11.3.32 single-electron operator on the line graph); the line graph is mathlib `SimpleGraph.lineGraph` (realized via `SimpleGraph.Iso G (lineGraph Gbase)`). `axiom mielke_theorem_11_12` (flat band, **connected** base: single-electron kernel `finrank = D`) and `axiom mielke_theorem_11_13` (Mielke ferromagnetism: biconnected base, `N=D` ⇒ ground subspace `finrank = N+1` + all maximal spin `S_max=N/2`). Both deferred as documented axioms — Tasaki defers 11.12's proof to §11.3.3 and states 11.13 without proof (Theorem 11.8 policy). The Mielke Hamiltonian model + symmetries are axiom-free (Issue #4177) |
| `Hubbard/MielkeIncidenceMatrix.lean` | **Tasaki §11.3.3 (incidence-matrix `SᴴS` factorisation)**: `mielkeSingleElectronOpOn` (single-electron operator over an arbitrary finite vertex type; `mielkeSingleElectronOp` is now the `Fin (M+1)` wrapper), `mielkeIncidence` (`S = √t·` mathlib `incMatrix`, restricted to genuine edges), and `mielkeIncidence_conjTranspose_mul_self`: `SᴴS = mielkeSingleElectronOpOn (lineGraph G) t` for `t ≥ 0` (eqs. (11.3.36)–(11.3.39)). Presents the line-graph operator as a PSD Gram matrix whose kernel is `ker S` — the algebraic core for discharging the Theorem 11.12 axiom. First PR of the §11.3.3 thread (Issue #4180) |
