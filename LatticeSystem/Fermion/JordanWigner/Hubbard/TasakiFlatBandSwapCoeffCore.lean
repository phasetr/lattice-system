...
Split out of `TasakiFlatBandSwapCoeff.lean` (periodic build-speed refactor) so the heavier
coefficient-extraction / `finrank ≤ 1` capstone compiles against a smaller dependency set.
