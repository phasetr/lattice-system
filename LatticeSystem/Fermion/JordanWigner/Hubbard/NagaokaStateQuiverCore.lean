...
/-!
# Tasaki §11.2.2: state-quiver core — edges, reachability, and hole transport

This is the lower-level core of the one-hole state quiver of `−M` (Tasaki §11.2.2), split out of
`NagaokaStateQuiver.lean` for incremental build speed.  It develops the foundations used by the
"15-puzzle" exchange argument:
...
* **Edges** — `−M_{(y,τ),(x,σ)} > 0` iff the hole hops `x → y` along a present bond
  ...
  `StateReach` with `refl`/`trans`/`symm`/`holeHop`.
* **Sector lift** — quiver edges preserve magnetization, so a full-quiver path lifts to the
  magnetization-sector quiver (`exists_sectorPath_of_path`), reducing the connectivity condition to
  state-quiver reachability (`nagaokaConnectivity_of_reach`).
* **Loop trips and spin permutations** — circling a length-3 loop transposes two spins
  (`StateReach.transposition`, `swapHoleSpin`); a length-4 loop 3-cycles three spins
  (`StateReach.threeCyclePerm`, `cyc3HoleSpin`), with the footnote-14 once/twice identities.
* **Controlled hole transport** — `holeWalkTransport` records the exact configuration carried by the
  hole along a walk, with its `append`/`reverse`/off-support/congruence lemmas — the machinery the
  round-trip "15-puzzle" exchange (in `NagaokaStateQuiver.lean`) is built on.
