...
theorem hubbardSector_minEnergy_mul_le_rayleighOnVec_of_isHermitian
    ...
    (hermitianMinEigenvalue (configSectorCompress_isHermitian (hubbardNumberSectorPred N Ne) hA)) *
        (dotProduct (star v) v).re ≤ rayleighOnVec A v := by
  set hHW := configSectorCompress_isHermitian (hubbardNumberSectorPred N Ne) hA with hHWd
  ...
  calc hermitianMinEigenvalue hHW * (dotProduct (star v) v).re
      ≤ rayleighOnVec (configSectorCompress N (hubbardNumberSectorPred N Ne) A) c := hvar
...
theorem mulVec_eq_smul_of_hubbardSector_rayleighOnVec_eq_min
    ...
    (heq : rayleighOnVec A v =
      (hermitianMinEigenvalue (configSectorCompress_isHermitian (hubbardNumberSectorPred N Ne)
        hA)) * (dotProduct (star v) v).re) :
    A.mulVec v
      = ((hermitianMinEigenvalue (configSectorCompress_isHermitian (hubbardNumberSectorPred N Ne)
          hA) : ℝ) : ℂ) • v := by
  set hHW := configSectorCompress_isHermitian (hubbardNumberSectorPred N Ne) hA with hHWd
  set c := configSectorCoeff N (hubbardNumberSectorPred N Ne) v with hc
  have hvW : v ∈ hubbardSectorWSubmodule N Ne := (mem_hubbardSectorWSubmodule_iff Ne).mpr hv
  have hsupp : ∀ w, ¬ hubbardNumberSectorPred N Ne w → v w = 0 :=
    hubbardNumberSector_supported_of_mem Ne hvW
  have hexp : v = configSectorExpansion N (hubbardNumberSectorPred N Ne) c :=
    configSector_completeness (hubbardNumberSectorPred N Ne) v hsupp
  have hcc : (dotProduct (star c) c).re = (dotProduct (star v) v).re := by
    rw [hc, ← configSectorExpansion_dotProduct_self (hubbardNumberSectorPred N Ne), ← hexp]
  -- The compression Rayleigh quotient attains its minimum at `c`.
  have heqc : rayleighOnVec (configSectorCompress N (hubbardNumberSectorPred N Ne) A) c =
  ...
  have hceig : (configSectorCompress N (hubbardNumberSectorPred N Ne) A).mulVec c
  ...
  have hApres : ∀ w, ¬ hubbardNumberSectorPred N Ne w →
      A.mulVec (configSectorExpansion N (hubbardNumberSectorPred N Ne) c) w = 0 := by
    rw [← hexp]; exact hubbardNumberSector_supported_of_mem Ne (hAW v hvW)
  have hlift := configSectorExpansion_of_compress_eigen (hubbardNumberSectorPred N Ne) hApres hceig
  rw [← hexp] at hlift
  rw [hlift, hexp]
