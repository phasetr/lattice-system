...
* **Proposition 2** (unique strict minimality): if `H` is Hermitian, commutes with `N̂`, `Ŝ³`, `Ŝ²`,
  and has a *unique* normalized ground state `φ` on `K`, then some occupied Casimir sector `K_c`
  (`K_c ≠ ⊥`) contains `φ` and attains the ground energy, and it is the only occupied one that
  does — every other occupied sector is strictly higher
...
is an *occupied* Casimir eigenvalue `c` (`K_c ≠ ⊥`) which `φ` itself occupies (`φ ∈ K_c`, i.e.
`Ŝ² φ = c • φ`) and whose sector-restricted minimum energy `minEnergyOn K_c H` attains the ground
energy `E`, and it is unique among occupied sectors: every other occupied Casimir sector `K_{c'}`
...
theorem exists_unique_casimir_sector_strict_min {N : ℕ} {H : ManyBodyOp (Fin (2 * N + 2))}
    ...
    ∃ c : ℂ, numberSpinZCasimirSectorEuclidean N L m₀ c ≠ ⊥ ∧
      φ ∈ numberSpinZCasimirSectorEuclidean N L m₀ c ∧
  ...
  have hKcne : numberSpinZCasimirSectorEuclidean N L m₀ c ≠ ⊥ := by
    ...
    exact hφne hφKc
  refine ⟨c, hKcne, hφKc, le_antisymm ?_ (hEle c hKcne), ?_⟩
