...
/-!
# Hubbard spin-symmetry auxiliaries: up-spin sector (foundation)

Foundational layer extracted from `SpinSymmetryAux.lean` for build speed.  This file
collects the Hermiticity of `N_↑`/`N_↓`, the adjoint relations for spinful operators, the
injectivity of the up/down site maps, and the up-spin commutators / same-species hopping
commutativity (culminating in `fermionTotalUpNumber_commute_hubbardHamiltonian`).

The down-spin commutators / same-species hopping commutativity and the U(1)×U(1)
corollary (`Ŝ^z_tot` commutes with `Ĥ`) are kept in the capstone module
`SpinSymmetryAux.lean`.
-/
