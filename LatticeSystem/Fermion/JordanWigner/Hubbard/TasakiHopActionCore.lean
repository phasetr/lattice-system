...
/-!
# Tasaki-basis hopping signs: parity machinery (foundation)

Foundational layer extracted from `TasakiHopAction.lean` for build speed.  This file proves the
supporting Jordan–Wigner string-sign / site-counting / parity-exponent facts used to evaluate the
uniform-sign hole-filling action: `jwSign_eq_neg_one_pow`, `sum_spinful_reindex`,
`hubbardTasakiBasisSign_eq`, and the source/target hop string signs `hop_jwSign_source`/
`hop_jwSign_target`.

The boxed eq. (11.2.4) uniform-sign result `hubbardTasakiHop_mulVec` is kept in the capstone module
`TasakiHopAction.lean`.

Tracked in Issue #4130. Reference: Tasaki, *Physics and Mathematics of Quantum Many-Body Systems*,
1st edition, §11.2, eq. (11.2.4), pp. 383-384.
-/
