...
/-!
# General flat-band ferromagnetism, disconnected case: foundation

Foundational layer extracted from `GeneralFlatBandDisconnected.lean` for build speed
(Tasaki §11.3.4 Theorem 11.17).  This file collects the kinetic-annihilation and
ground-membership lemmas, the disconnection-cut combinatorics, the canonical
Slater-sum representation vanishing lemmas, and the per-block weight-state machinery
that feed the disconnected dimension bound `generalFlatBand_disconnected_finrank_gt`
and the classification `generalFlatBand_theorem_11_17` kept in the capstone module
`GeneralFlatBandDisconnected.lean`.
