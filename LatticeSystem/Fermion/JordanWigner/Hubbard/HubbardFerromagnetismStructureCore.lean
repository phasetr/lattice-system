...
/-- **Unfolded membership in `hubbardEigenspaceAt`.**  `v ∈ G` iff `v` is an `H`-eigenvector
at `E₀` and an `N̂`-eigenvector at the electron number `Ne`. -/
theorem mem_hubbardEigenspaceAt {E₀ : ℂ} {Ne : ℕ}
    {v : (Fin (2 * N + 2) → Fin 2) → ℂ} :
    v ∈ hubbardEigenspaceAt t U E₀ Ne ↔
      (hubbardHamiltonian N t U).mulVec v = E₀ • v ∧
        (fermionTotalNumber (2 * N + 1)).mulVec v = (Ne : ℂ) • v := by
  rw [hubbardEigenspaceAt, Submodule.mem_inf,
...
needed) and with `N̂`, so it maps the `E₀`-energy `Ne`-electron eigenspace into itself. -/
theorem fermionTotalSpinZ_mulVec_mem_hubbardEigenspaceAt {E₀ : ℂ} {Ne : ℕ}
    {v : (Fin (2 * N + 2) → Fin 2) → ℂ} (hv : v ∈ hubbardEigenspaceAt t U E₀ Ne) :
    (fermionTotalSpinZ N).mulVec v ∈ hubbardEigenspaceAt t U E₀ Ne := by
  rw [mem_hubbardEigenspaceAt] at hv ⊢
  obtain ⟨hH, hN⟩ := hv
  refine ⟨?_, ?_⟩
...
theorem fermionTotalSpinPlus_mulVec_mem_hubbardEigenspaceAt {E₀ : ℂ} {Ne : ℕ}
    {v : (Fin (2 * N + 2) → Fin 2) → ℂ} (hv : v ∈ hubbardEigenspaceAt t U E₀ Ne) :
    (fermionTotalSpinPlus N).mulVec v ∈ hubbardEigenspaceAt t U E₀ Ne := by
  rw [mem_hubbardEigenspaceAt] at hv ⊢
  obtain ⟨hH, hN⟩ := hv
  refine ⟨?_, ?_⟩
...
theorem fermionTotalSpinMinus_mulVec_mem_hubbardEigenspaceAt
    (hJ : ∀ i j, star (t i j) = t j i) (hU : star U = U) {E₀ : ℂ} {Ne : ℕ}
    {v : (Fin (2 * N + 2) → Fin 2) → ℂ} (hv : v ∈ hubbardEigenspaceAt t U E₀ Ne) :
    (fermionTotalSpinMinus N).mulVec v ∈ hubbardEigenspaceAt t U E₀ Ne := by
  rw [mem_hubbardEigenspaceAt] at hv ⊢
  obtain ⟨hH, hN⟩ := hv
  refine ⟨?_, ?_⟩
...
eigenstate and an `Ŝ³ = μ` eigenstate, so each supported configuration has electron number `Ne`
and `Ŝ³` count `μ = (#↑) − Ne/2`, placing `μ` in the weight set — contradiction unless the
vector is `0`. -/
theorem hubbardEigenspaceAt_inf_eigenspace_eq_bot {E₀ : ℂ} (Ne : ℕ) (μ : ℂ)
    (hμ : ∀ a : Fin (Ne + 1), μ ≠ (((a : ℝ) - (Ne : ℝ) / 2 : ℝ) : ℂ)) :
    hubbardEigenspaceAt t U E₀ Ne ⊓
      Module.End.eigenspace (fermionTotalSpinZ N).mulVecLin μ = ⊥ := by
  rw [Submodule.eq_bot_iff]
  intro v hv
  rw [Submodule.mem_inf] at hv
  obtain ⟨hvG, hveig⟩ := hv
  rw [mem_hubbardEigenspaceAt] at hvG
  obtain ⟨_, hN⟩ := hvG
...
the supremum of `G` over all `Ŝ³` eigenspaces collapses to the `Ne + 1` occurring half-integer
weights (off-weight blocks are `⊥` by `hubbardEigenspaceAt_inf_eigenspace_eq_bot`). -/
theorem hubbardEigenspaceAt_eq_iSup_weight {E₀ : ℂ} (Ne : ℕ) :
    hubbardEigenspaceAt t U E₀ Ne =
      ⨆ a : Fin (Ne + 1), hubbardEigenspaceAt t U E₀ Ne ⊓
        Module.End.eigenspace (fermionTotalSpinZ N).mulVecLin
          (((a : ℝ) - (Ne : ℝ) / 2 : ℝ) : ℂ) := by
  refine le_antisymm ?_ (iSup_le (fun _ => inf_le_left))
  have hinv : ∀ x ∈ hubbardEigenspaceAt t U E₀ Ne,
      (fermionTotalSpinZ N).mulVecLin x ∈ hubbardEigenspaceAt t U E₀ Ne := by
    intro x hx
    rw [Matrix.mulVecLin_apply]
    exact fermionTotalSpinZ_mulVec_mem_hubbardEigenspaceAt t U hx
  have htop : ⨆ μ : ℂ,
  ...
  have hdecomp := Submodule.eq_iSup_inf_genEigenspace
    (p := hubbardEigenspaceAt t U E₀ Ne)
    (f := (fermionTotalSpinZ N).mulVecLin) 1 hinv htop
  rw [show (⨆ μ : ℂ, hubbardEigenspaceAt t U E₀ Ne ⊓
        Module.End.genEigenspace (fermionTotalSpinZ N).mulVecLin μ 1) =
      ⨆ μ : ℂ, hubbardEigenspaceAt t U E₀ Ne ⊓
  ...
  · rw [hubbardEigenspaceAt_inf_eigenspace_eq_bot t U Ne μ (not_exists.mp hμ)]
    exact bot_le
...
`v` (`(Ŝ_tot)² v = S(S + 1) v`) at real weight `Ŝ³ v = sz • v` with `−S ≤ sz < S` is *not*
annihilated by `Ŝ⁺`: were `Ŝ⁺v = 0`, `v` would be a highest-weight state, forcing
`S(S + 1) = sz(sz + 1)` (general highest-weight Casimir), i.e. `sz = S` (ruled out by `sz < S`) or
`sz = −(S + 1)` (ruled out by `sz ≥ −S`). -/
