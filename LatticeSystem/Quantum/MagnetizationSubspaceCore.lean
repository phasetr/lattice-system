...
/-!
# Magnetization sectors: definition and internal direct sum (foundation)

Foundational layer extracted from `MagnetizationSubspace.lean` for build speed.  This file defines
the magnetization-`M` subspace `magnetizationSubspace M` (the `Ŝ_tot^(3)`-eigenspace for eigenvalue
`M`), its membership characterisations, the disjointness of distinct sectors, the spanning property
(`iSup_magnetizationSubspace_eq_top`), and the internal-direct-sum decomposition
(`magnetizationSubspace_isInternal`).

The ground-state ladder / Néel / singlet–triplet membership facts and the sector-preservation
theorems for commuting operators, the Heisenberg Hamiltonian, and the ladder operators (which depend
on `SpinDot.lean`) are kept in the capstone module `MagnetizationSubspace.lean`.
-/
