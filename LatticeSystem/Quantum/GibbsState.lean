...
* `gibbsExp_commute_self`, `gibbsState_commute_self` —
...
`exp(-β H) · H = H · exp(-β H)`. -/
theorem gibbsExp_commute_self (β : ℝ) (H : ManyBodyOp Λ) :
...
theorem gibbsState_commute_self (β : ℝ) (H : ManyBodyOp Λ) :
    Commute (gibbsState β H) H := by
  unfold gibbsState
  exact (gibbsExp_commute_self β H).smul_left _
...
theorem gibbsExpectation_star_of_isHermitian {H O : ManyBodyOp Λ}
    ...
    star (gibbsExpectation β H O) = gibbsExpectation β H O := by
  have hρ : (gibbsState β H).IsHermitian := gibbsState_isHermitian hH β
  unfold gibbsExpectation
  rw [← Matrix.trace_conjTranspose, Matrix.conjTranspose_mul, hO.eq, hρ.eq,
