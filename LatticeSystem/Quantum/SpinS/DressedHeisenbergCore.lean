...
/-!
# Dressed (Marshall-conjugated) Heisenberg operator and complex matrix: foundation

Foundational layer extracted from `DressedHeisenberg.lean` for build speed.  This file
develops the dressed Heisenberg operator `dressedHeisenbergS` (diagonal/off-diagonal
form, reality, additivity in `J`, magnetization-sector and Hermiticity properties) and
its complex matrix form `dressedHeisenbergSMatrix` (mulVec on basis vectors, sector
membership, commutation with `Ŝ³_tot`, Hermiticity on chains / periodic chains).

The real-part matrix `dressedHeisenbergSReMatrix` and its symmetry/swap lemmas are kept
in the capstone module `DressedHeisenberg.lean`.
