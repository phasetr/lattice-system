import LatticeSystem.Quantum.SpinS.FourGapEqCardSqSubTwoBiwNormSq
import LatticeSystem.Quantum.SpinS.AllUpAddAllDownExpectationEqVariationalGap

/-!
# `4 · (⟨Φ_↑⟩ + ⟨Φ_↓⟩).re = (|Λ|·N)² − (2·‖biw‖)²` unconditionally

Combines PR #3256 + PR #3215 (saturated sum = gap):

  `4 · (⟨Φ_↑|Ĥ_toy|Φ_↑⟩ + ⟨Φ_↓|Ĥ_toy|Φ_↓⟩).re = (|Λ|·N)² − (2·‖biw‖)²`

unconditionally.

Tracked as part of Tasaki §2.5 Theorem 2.3 / γ-4 (Issue #412).
-/

namespace LatticeSystem.Quantum

open Matrix

variable {Λ : Type*} [Fintype Λ] [DecidableEq Λ]

set_option linter.style.longLine false in
/-- **`4·saturated_sum = (|Λ|·N)² − (2·‖biw‖)²`** unconditionally. -/
theorem heisenbergToyHamiltonianS_four_allAligned_zero_add_last_expectation_re_eq_card_sq_sub_two_biw_norm_sq
    [Nonempty Λ] (A : Λ → Bool) (N : ℕ) :
    4 * (dotProduct (star (allAlignedStateS Λ N (0 : Fin (N + 1))))
            ((heisenbergToyHamiltonianS (Λ := Λ) A N).mulVec
              (allAlignedStateS Λ N (0 : Fin (N + 1)))) +
          dotProduct (star (allAlignedStateS Λ N (Fin.last N)))
            ((heisenbergToyHamiltonianS (Λ := Λ) A N).mulVec
              (allAlignedStateS Λ N (Fin.last N)))).re =
      ((Fintype.card Λ : ℝ) * (N : ℝ)) ^ 2 -
        (2 * ‖bipartiteImbalanceWeight (Λ := Λ) A N‖) ^ 2 := by
  have h1 := heisenbergToyHamiltonianS_four_variational_gap_re_eq_card_sq_sub_two_biw_norm_sq
    (Λ := Λ) (A := A) (N := N)
  have h2 := heisenbergToyHamiltonianS_allAligned_zero_add_last_expectation_re_eq_variational_gap_re
    (Λ := Λ) (A := A) (N := N)
  linarith

end LatticeSystem.Quantum
