...
/-!
# Tasaki §2.5 Theorem 2.3 outside-ground predicted-GS lowered-Marshall wrappers

This module contains the lowered-Marshall final-wrapper suffix split from
`Theorem23OutsideGroundPredictedGS.lean`. The base predicted-GS module keeps
the left-endpoint threaded predicted-GS wrappers based on off-`A` dominance,
while this module packages the lowered-Marshall and outside-real-sector
boundaries consumed by the cross-ladder tail.
