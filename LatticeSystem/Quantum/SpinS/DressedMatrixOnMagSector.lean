...
(PR #862) into the form

  ∃ μ < c, ∃ v > 0,
    Φ σ := ((sign A σ.1).re * v σ : ℂ)
    heis_sec_C.mulVec Φ = μ • Φ ∧
    ∀ {μ'} {W : ... → ℂ},
      heis_sec_C.mulVec W = μ' • W →
      (∀ σ, 0 < (sign A σ.1).re * (W σ).re) →
      μ' = μ ∧ ∃ r > 0, ∀ σ, (W σ).re = r * ((sign A σ.1).re * v σ)

This is the COMPLEX-Hilbert-space form of Tasaki §2.5 Theorem 2.2:
...
theorem marshallLiebMattis_spinS_heisenbergSector_complexGroundState_full
  ...
  -- The reference complex eigenvector Φ σ := ((sign A σ.1).re * v σ : ℂ).
  -- Its real part is (sign A σ.1).re * v σ, which is Marshall-positive.
  have hΦ_marshall_pos : ∀ σ, 0 < (marshallSignS A σ.1).re *
    ...
    rw [← mul_assoc, hsq, one_mul]
    exact hv_pos σ
  -- Apply complex Marshall-positive uniqueness with Φ as the first argument.
