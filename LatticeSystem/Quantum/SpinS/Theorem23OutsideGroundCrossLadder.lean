...
This module contains the sublattice-component, joint-component, lowered-joint,
and packed cross-ladder final-wrapper chain split from
`Theorem23OutsideGround.lean`. The re-embedded cross-ladder and source-weight
tail is split further into `Theorem23OutsideGroundCrossLadderReembedded.lean`,
and the unpacked lowered-joint callback boundary is isolated in
`Theorem23OutsideGroundCrossLadderUnpacked.lean`. This lets the intermediate
cross-ladder API elaborate separately from the unpacked and re-embedded
final-wrapper tails.
