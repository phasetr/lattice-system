...
and unpacked lowered-joint final-wrapper chain split from
`Theorem23OutsideGround.lean`. The re-embedded cross-ladder and source-weight
suffix is split further into `Theorem23OutsideGroundCrossLadderReembedded.lean`,
letting this intermediate cross-ladder API elaborate separately from the
re-embedded final-wrapper tail.
