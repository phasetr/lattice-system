...
This module contains the left-endpoint predicted-GS final-wrapper suffix split
from `Theorem23OutsideGround.lean`. The base outside-ground module keeps the
outside-sector lower-bound, sector-minimality, and common-energy
final-packaging layers, while
`Theorem23OutsideGroundPredictedCasimirThreaded.lean` contains the
left-endpoint threaded predicted-Casimir boundary consumed by this module.
The lowered-Marshall predicted-GS suffix is split further into
`Theorem23OutsideGroundPredictedGSLoweredMarshall.lean`.
