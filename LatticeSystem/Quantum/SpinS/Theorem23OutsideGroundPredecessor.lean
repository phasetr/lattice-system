...
This module contains the predecessor-specialized source-weight and
positive-source final-wrapper chain for the Tasaki §2.5 Theorem 2.3 proof
chain. The lowerable and explicit lowerable suffix is split into
`Theorem23OutsideGroundPredecessorLowerable.lean`, and the real source-weight
and raising-source suffix is split further into
...
set_option linter.style.longLine false in
/-- **Tasaki §2.5 Theorem 2.3 predecessor-specialized source-weight boundary
with outside-sector ground energies**: this replaces the unpacked re-embedded
source-weight callback in the outside-sector final wrapper by the
predecessor-specialized coefficient form.

The predecessor-specialized source-weight callback is first converted to the
unpacked re-embedded source-weight coefficient callback. The resulting
...
set_option linter.style.longLine false in
/-- **Tasaki §2.5 Theorem 2.3 positive-source predecessor boundary with
outside-sector ground energies**: this replaces the predecessor-specialized
signed coefficient callback in the outside-sector final wrapper by the
sign-normalized positive-source coefficient form.

The positive-source callback is first rewritten to the signed predecessor
