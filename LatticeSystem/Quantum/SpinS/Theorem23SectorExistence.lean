...
the per-sector Theorem 2.2 reuse wrapper, and the adjacent sector-existence
wrappers split from `Theorem23.lean`. Keeping this layer separate lets the
adjacent common-energy and dominance APIs elaborate without the
sector-existence/final-statement tail. The predicted-GS interval-chain suffix
is split further into `Theorem23SectorExistenceInterval.lean`.
-/
