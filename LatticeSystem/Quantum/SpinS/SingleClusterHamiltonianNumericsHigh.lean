...
`singleClusterGSEnergyS` and `singleClusterMaxEnergyS` for `N ≥ 29`
(spin `S = N/2 ≥ 29/2`). The pre-`N=29` specialisations live in the
companion `SingleClusterHamiltonianNumerics.lean`.

This file imports the main `SingleClusterHamiltonian` directly (not
the lower-N numerics file) so both numerics files can elaborate in
