...
Foundational layer extracted from `DressedHeisenbergRaiseLower.lean` for build speed.  This
file develops the complex-matrix dressed Heisenberg behaviour on raise/lower steps (with the
private Marshall-sign witness lemma).

The real-matrix versions and the negation (positive entries on raise/lower steps) are kept
in the capstone module `DressedHeisenbergRaiseLower.lean`.
-/
