...
`N = 116..131` (spin `S = N/2 ∈ {58, 117/2, …, 131/2}`). The
`N = 1..28` specialisations live in
...
Refactor #14 (after spin-70 pentamer at γ-5 step 1137) was an
evaluate-only iteration: `InfiniteHigh` measured ~6.8 s user CPU at
2401 lines (N=116..140), still well under the historical ~10 s split
trigger. Refactor #15 (after spin-74 heptamer at γ-5 step 1187) split
`InfiniteHigh` into `InfiniteHigh` (N=116..131) and a new
`TransfiniteHigh` (N ≥ 132) as the file reached ~8.6 s user CPU at
3208 lines (N=116..148), matching the refactor #13 split trigger.
-/
