...
This module contains the single-site lowering component formula and
Marshall-signed local lowering identities used by the coefficient and
local-difference layers of Tasaki §2.5 Theorem 2.3. It is split from
`Theorem23Local.lean` so the core local ladder and site-sum expansion
layer can elaborate separately from the lowering component suffix. The
weak boundary-inclusive sign-sum bounds are split into
`Theorem23LocalLoweringSignSum.lean`, and the single-site
positivity/negativity theorems are split into
