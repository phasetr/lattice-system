...
Foundational layer extracted from `Problem25dBalancedSectorWitness.lean` for build speed.  This file
proves the strict signed ladder entries from raise/lower data and the balanced-sector witness.

The balanced-sector dot-product wrapper is kept in the capstone module
`Problem25dBalancedSectorWitness.lean`.
-/
