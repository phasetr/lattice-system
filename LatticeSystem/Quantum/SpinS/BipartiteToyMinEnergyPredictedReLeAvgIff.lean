...
The canonical predicted-min energy `(pmA).re` sits at or below the
orientation-pair average exactly when `A` is the lighter sublattice.
At `N ≥ 1`:

  `(predicted_min A).re ≤ ((predicted_min A).re + (predicted_min ¬A).re) / 2
   ↔ |A| ≤ |¬A|`.

This is `x ≤ (x + y) / 2 ↔ x ≤ y` composed with PR #3070.
...
theorem bipartiteToyMinEnergyPredicted_re_le_avg_complement_re_iff_cardA_le_cardNotA
    (A : Λ → Bool) (N : ℕ) (hN : 0 < N) :
