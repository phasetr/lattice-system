...
`singleClusterGSEnergyS` and `singleClusterMaxEnergyS` for `N = 99..115`
(spin `S = N/2 ∈ [99/2, 115/2]`). The `N = 1..28` specialisations live in
...
The further split from `HyperHigh` to `InfiniteHigh` was introduced as
refactor #13 (50-PR cadence) when `HyperHigh` reached ~8.9 s user CPU
at 3240 lines covering N=99..132. After the split, `HyperHigh`
covers only N=99..115 (back to ~6 s) and `InfiniteHigh` covers
N=116..132 (~6 s) so each file stays well under the ~10 s trigger.
-/
