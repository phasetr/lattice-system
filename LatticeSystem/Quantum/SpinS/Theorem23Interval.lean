import LatticeSystem.Quantum.SpinS.Theorem23IntervalCallbacks
...
This module contains the basic interval-chain wrappers used by the final Tasaki
§2.5 Theorem 2.3 packaging modules. The named callback proposition API is split
into `Theorem23IntervalCallbacks.lean`, the joint-component and lowered-joint
cross-ladder interval-wrapper suffix is split into `Theorem23IntervalJoint.lean`,
and the predicted-GS-aware local-comparison suffix is split into
`Theorem23IntervalPredictedGS.lean`, so this module can stay focused on the
