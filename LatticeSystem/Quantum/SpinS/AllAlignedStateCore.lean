...
/-!
# Spin-`S` all-aligned (saturated ferromagnet) state — core
(Tasaki §2.4 generalised to spin-`S`)

Core layer: the all-aligned configuration/state, magnetization properties, the
highest/lowest-weight Heisenberg and `(Ŝ_tot)²` eigenvector results, and the
ladder annihilation lemmas.  The Heisenberg- and Casimir-eigenvalue preservation
results along the (iterated) total-spin ladder are in `AllAlignedState`.
-/
