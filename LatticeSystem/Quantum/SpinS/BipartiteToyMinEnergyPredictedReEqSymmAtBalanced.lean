...
At balanced `|A| = |¬A|`, both predicted min energies equal the
symmetric form:
- `(predicted_min A).re = −|A|·|¬A|·N²/2 − |¬A|·N`
- `(predictedSymm A).re = −|A|·|¬A|·N²/2 − min(|A|, |¬A|)·N
                        = −|A|·|¬A|·N²/2 − |A|·N` (since |A| = |¬A|)
                        = `(predicted_min A).re` since `|A| = |¬A|`.

  `(predicted_min A).re = (predictedSymm A).re`   at `|A| = |¬A|`.

Direct corollary of PR #2999 (`predicted_min A.re ≤ predictedSymm.re`)
+ the symmetric form's definition: at balanced, the diff vanishes.

The two orientations coincide at balanced; both equal the symmetric
prediction.
...
/-- **At balanced**: `(predicted_min A).re = (predictedSymm A).re`. -/
theorem bipartiteToyMinEnergyPredicted_re_eq_predictedSymm_re_of_balanced
    (A : Λ → Bool) (N : ℕ)
