import LatticeSystem.Quantum.SpinS.HiddenAntiferromagneticOrderUniquenessCore

/-!
# Tasaki §6.3 Proposition 6.5: Perron–Frobenius uniqueness route (kink ergodicity)

The Perron–Frobenius development toward the unique-ground-state clause of Proposition 6.5.  Building
on the combinatorial core (`HiddenAntiferromagneticOrderUniquenessCore.lean`: kink moves,
balanced-sector connectivity and PF bound, Marshall-gauge transfer, Néel witness), this file
analyses the magnetization-`±1` sectors and their energy lower bound, the strict balanced ground
energy (`hhafDressedMatrix0_ground_lt_neg_L`), the classification of hidden-AFM sectors, the block
of the restricted matrix by magnetization sector, the ground-eigenspace `finrank ≤ 1`, and the
capstone `tasaki_prop_6_5_hhaf_spin_one`.
