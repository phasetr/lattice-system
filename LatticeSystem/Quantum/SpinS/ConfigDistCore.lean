...
Foundational layer extracted from `ConfigDist.lean` for build speed.  This file builds the
existence of over/under sites for equal-magnetization configuration pairs and the distance
reduction by a raise/lower step.

The complete-graph reachability for equal-magnetization configurations is kept in the
capstone module `ConfigDist.lean`.
-/
