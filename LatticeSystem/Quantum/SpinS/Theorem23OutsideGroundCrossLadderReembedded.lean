...
this module packages the re-embedded cross-ladder source-sector site-sum and
source-weight final boundaries. The unpacked re-embedded source-weight
final-wrapper suffix is split further into
`Theorem23OutsideGroundCrossLadderReembeddedUnpacked.lean`.
