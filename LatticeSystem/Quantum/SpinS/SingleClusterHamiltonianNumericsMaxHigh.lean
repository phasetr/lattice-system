...
`singleClusterGSEnergyS` and `singleClusterMaxEnergyS` for `N = 78..98`
(spin `S = N/2 ∈ {39, …, 49}`). The `N = 1..28` specialisations live in
...
N=78..98 entries had been appended; the further split into
`HyperHigh` was introduced as refactor #11 when `MaxHigh` reached
~9.8 s user CPU after the N=99..115 entries had been appended (see
