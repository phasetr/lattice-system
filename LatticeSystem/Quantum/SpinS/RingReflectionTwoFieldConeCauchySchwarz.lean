...
approximant `U_{m}(x,y) = (g(x)·θ(g(y))·∑ᵢ wᵢ·(θ(Cᵢ(y))·Cᵢ(x)))^m` — left-supported kinetic family
`g`, nonnegative cone weights `wᵢ ≥ 0`, FIELD-DEPENDENT left-supported crossing generators `Cᵢ(z)`
(the reflected pile carrying `Cᵢ(y)`, the non-reflected pile `Cᵢ(x)`) — the real part of the trace
pairing obeys the finite Gram Cauchy–Schwarz bound

    `(Re Tr U_{m}(x,y))² ≤ Re Tr U_{m}(x,x) · Re Tr U_{m}(y,y)`,

where the diagonal slots carry the field-matched crossings `Tr U_{m}(x,x)` with `θ(Cᵢ(x))·Cᵢ(x)` and
`Tr U_{m}(y,y)` with `θ(Cᵢ(y))·Cᵢ(y)`.
...
limit in the next PR of the Theorem 4.2 arc.  The field-free bound is the constant specialization
`C i z := C i`. -/
theorem twoField_product_pairing_cauchySchwarz (m : ℕ)
    ...
    (hC : ∀ i z, SupportedOnLeftS n N (C i z)) (x y : Fin (2 * n) → ℝ) :
    (((g x * ringReflectionThetaS n N (g y)
        * ∑ i, (w i : ℂ) • (ringReflectionThetaS n N (C i y) * C i x)) ^ m).trace.re) ^ 2
      ≤ ((g x * ringReflectionThetaS n N (g x)
          * ∑ i, (w i : ℂ) • (ringReflectionThetaS n N (C i x) * C i x)) ^ m).trace.re
        * ((g y * ringReflectionThetaS n N (g y)
          * ∑ i, (w i : ℂ) • (ringReflectionThetaS n N (C i y) * C i y)) ^ m).trace.re := by
