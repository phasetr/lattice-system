...
This module contains the predecessor-difference callback layer downstream
of `Theorem23LocalCoefficientRaisingSource.lean`. The lowered site-sum
dominance layer is isolated in `Theorem23LocalDifferenceSiteSum.lean`, the
raising-direction site-sum layer is isolated in
`Theorem23LocalDifferenceRaising.lean`, and the final Marshall-positivity
wrappers are isolated in
