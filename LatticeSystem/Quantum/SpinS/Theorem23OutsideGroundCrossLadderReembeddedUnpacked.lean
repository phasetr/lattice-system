...
/-!
# Tasaki §2.5 Theorem 2.3 unpacked re-embedded source-weight final wrapper

This module contains the unpacked re-embedded source-weight final-wrapper
suffix split from `Theorem23OutsideGroundCrossLadderReembedded.lean`. The
parent re-embedded module keeps the basic re-embedded cross-ladder
source-sector site-sum wrapper and the re-embedded source-weight cross-ladder
wrapper, while this module packages the unpacked-callback re-embedded
source-weight final boundary consumed by the predecessor-specialized tail.
...
set_option linter.style.longLine false in
/-- **Tasaki §2.5 Theorem 2.3 unpacked re-embedded source-weight boundary with
outside-sector ground energies**: this replaces the unpacked cross-ladder
coefficient callback in the outside-sector final wrapper by the scalar
re-embedded source-weight coefficient form.

The source-weight callback is first converted to the unpacked cross-ladder
coefficient dominance callback. The resulting common-energy chain is then
combined with `tasaki23OutsideGroundEnergyLowerFamilyCallback` through the
standard outside-sector sector-minimality bridge. -/
theorem
