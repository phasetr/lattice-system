...
This module contains the base adjacent-sector energy comparison step and the
with-nonzero adjacent-sector packages split from
`Theorem23LocalDifferenceMarshall.lean`. Keeping this suffix in its own module
lets the local predecessor-difference, raising site-sum, Marshall-positivity,
and energy identification layers elaborate separately. The Casimir-non-vanishing
and site-sum-positivity adjacent-sector packages are split further into
`Theorem23LocalDifferenceEnergyCasimir.lean`.
