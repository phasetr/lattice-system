...
`N = 132..148` (spin `S = N/2 ∈ {66, 133/2, …, 74}`). The
...
at 2466 lines (N=132..157), still under the ~8.6 s split trigger.
Refactor #17 (after spin-165/2 pentamer at γ-5 step 1287) split
`TransfiniteHigh` into `TransfiniteHigh` (N=132..148) and a new
`AbsoluteHigh` (N ≥ 149) as the file reached ~8.7 s user CPU at
3274 lines (N=132..165), matching the refactor #13 / #15 split
trigger.
-/
