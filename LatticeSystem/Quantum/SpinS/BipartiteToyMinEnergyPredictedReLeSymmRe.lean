...
/-!
# `(predicted_min_energy A N).re ≤ (predictedSymm A N).re`

Both quantities have real part:
- `(predicted_min A).re = −|A|·|¬A|·N²/2 − |¬A|·N` (PR #2778, simplified).
...
/-- **`(predicted_min A).re ≤ (predictedSymm A).re`** unconditionally. -/
theorem bipartiteToyMinEnergyPredicted_re_le_predictedSymm_re
    (A : Λ → Bool) (N : ℕ) :
  ...
  simp only [Complex.sub_re, Complex.neg_re, Complex.mul_re, Complex.natCast_re,
    Complex.natCast_im, Complex.div_ofNat_re]
