...
Core (foundational) lemmas of the Tasaki §2.5 Theorem 2.4 case-(ii) bridge:
the axis-swapped parity-block raw-support-path lemmas, the submatrix
`finrank <= 1` at the full ground energy from PF/min path data, and the
parity-block PF-min-path constructions (from raw support, and from
reachability).  The packaged general spin-S case-(ii) target uniqueness /
zero-magnetization endpoints are assembled in
`AnisotropicHeisenbergSpinSCaseIIBlockPFMin`.
