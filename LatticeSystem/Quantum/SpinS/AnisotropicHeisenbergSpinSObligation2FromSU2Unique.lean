...
`hermitianMinEigenvalue Ĥ`. -/
theorem anisotropicHeisenbergS_eigenspace_finrank_le_two_at_global_min_general
  ...
  classical
  have hbound := anisotropicHeisenbergS_eigenspace_finrank_le_two_unconditional_general
    A hJim hJnn hJpos hJself hJbip hlam hlb hub hDim hDpos hc_strict hA_ne hB_ne hN
...
theorem anisotropicHeisenbergS_eigenspace_finrank_le_two_at_global_min_path_general
    {N : ℕ} (A : Λ → Bool) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hJpos : ∀ x y, (bipartiteCompleteGraphOf A).Adj x y → 0 < (J x y).re)
    (hJself : ∀ x, J x x = 0) (hJbip : ∀ x y, J x y ≠ 0 → A x ≠ A y)
...
hypothesis below the first crossing, and the general spin-`S` `finrank <= 2`
bound along the path. -/
theorem anisotropicHeisenbergS_obligation_2_axiomatic_sup_crossing_hne_general
    {N : ℕ} (A : Λ → Bool) {J : Λ → Λ → ℂ}
    (hJim : ∀ x y, (J x y).im = 0) (hJnn : ∀ x y, 0 ≤ (J x y).re)
    (hJpos : ∀ x y, (bipartiteCompleteGraphOf A).Adj x y → 0 < (J x y).re)
    (hJself : ∀ x, J x x = 0) (hJbip : ∀ x y, J x y ≠ 0 → A x ≠ A y)
