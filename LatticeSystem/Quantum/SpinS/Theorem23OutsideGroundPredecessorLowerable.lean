...
/-!
# Tasaki §2.5 Theorem 2.3 outside-ground predecessor lowerable wrappers

This module contains the lowerable and explicit lowerable positive-source
predecessor final-wrapper suffix split from
`Theorem23OutsideGroundPredecessor.lean`. The base predecessor module keeps
the source-weight and positive-source final wrappers, while this module
converts those final boundaries to lowerable and attached lowerable
coefficient comparisons consumed by the real source-weight and raising-source
layers.
...
set_option linter.style.longLine false in
/-- **Tasaki §2.5 Theorem 2.3 lowerable positive-source predecessor boundary
with outside-sector ground energies**: this replaces the unfiltered
positive-source predecessor callback in the outside-sector final wrapper by the
lowerable-site filtered positive-source coefficient form.

The lowerable callback is first rewritten to the unfiltered positive-source
coefficient comparison by the existing sector-minimality wrapper. The resulting
common-energy chain is then combined with
`tasaki23OutsideGroundEnergyLowerFamilyCallback` through the standard
outside-sector sector-minimality bridge. -/
theorem
    tasaki_2_5_theorem_2_3_of_left_endpoint_threaded_predictedGS_of_unpacked_reembedded_source_weight_predecessor_lowerable_positive_source_coefficient_lt_of_outside_sector_ground_energy_lower_bound
    (A : V → Bool) {J : V → V → ℂ} (N : ℕ) (c : ℝ)
    (hBA :
      (Finset.univ.filter (fun x : V => (! A x) = true)).card ≤
        (Finset.univ.filter (fun x : V => A x = true)).card)
...
set_option linter.style.longLine false in
/-- **Tasaki §2.5 Theorem 2.3 explicit lowerable positive-source predecessor
boundary with outside-sector ground energies**: this replaces the
sector-minimality callback in the explicit lowerable predecessor wrapper by the
standard outside-sector lower-bound family.

The explicit lowerable callback is first rewritten to the lowerable filtered
positive-source comparison by the existing sector-minimality wrapper. The
resulting common-energy chain is then combined with
`tasaki23OutsideGroundEnergyLowerFamilyCallback` through the standard
outside-sector sector-minimality bridge. -/
theorem
    tasaki_2_5_theorem_2_3_of_left_endpoint_threaded_predictedGS_of_unpacked_reembedded_source_weight_predecessor_explicit_lowerable_positive_source_coefficient_lt_of_outside_sector_ground_energy_lower_bound
    (A : V → Bool) {J : V → V → ℂ} (N : ℕ) (c : ℝ)
    (hBA :
      (Finset.univ.filter (fun x : V => (! A x) = true)).card ≤
        (Finset.univ.filter (fun x : V => A x = true)).card)
