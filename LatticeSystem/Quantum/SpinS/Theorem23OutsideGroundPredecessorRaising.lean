...
positive-source coefficients and outside-sector ground energies**: this
replaces the sector-minimality callback in the explicit lowerable
predecessor boundary by the standard outside-sector lower-bound family.

The admissible-sector lower bound is obtained from the common-energy chain
produced by the explicit lowerable coefficient callback, while sectors
outside `tasaki23GroundStateSectors` are handled by
