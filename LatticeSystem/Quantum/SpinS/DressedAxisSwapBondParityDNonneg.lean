...
bond-only parity move on a graph `G` whose edges join opposite sublattices, the
shifted dressed matrix entry is strictly positive under the strict
`-1 < lambda < 1` condition and only the non-strict `D.re >= 0` condition. -/
theorem shiftedDressedAxisSwappedReMatrix_apply_pos_of_bondParityStepS_bipartite
  ...
  · exact shiftedDressedAxisSwappedReMatrix_apply_pos_of_raiseLowerStepS_bipartite
      A hJim hJnn hGbip hJ_pos_G hJself hJbip hlam hlb (le_of_lt hub) hDim hDnn c hRL
  · exact shiftedDressedAxisSwappedReMatrix_apply_pos_of_parityBondStepS_bipartite
      A hJim hJnn hGbip hJ_pos_G hJself hJbip hlam hlb hub hDim hDnn c hPB

/-- **Block matrix power positivity from bond-only parity reachability with
`D >= 0`**.  For any pair of parity-block configurations that are bond-only
reachable along a graph `G` whose edges join opposite sublattices, some power of
the shifted parity-block submatrix is strictly positive at that pair. -/
