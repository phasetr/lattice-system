...
and joint sublattice-Casimir structure used by the adjacent-sector chain. The
lowered joint-magnetization extractor lemmas (which unpack
`tasaki23LoweredJointMagSubspace` membership into Casimir / `magSubspaceS`
components) are split into `Theorem23PredictedLadderJointExtract.lean`.
