...
Combines PR #3200 (`2·⟨Φ_↑⟩.re = gap.re`) + PR #3278 (`gap ≤ (|Λ|·N/2)²`).
...
/-- **`2·⟨Φ_↑⟩.re ≤ (|Λ|·N/2)²`** unconditionally. -/
theorem heisenbergToyHamiltonianS_two_allAligned_zero_expectation_re_le_half_card_sq
    [Nonempty Λ] (A : Λ → Bool) (N : ℕ) :
    2 * (dotProduct (star (allAlignedStateS Λ N (0 : Fin (N + 1))))
            ((heisenbergToyHamiltonianS (Λ := Λ) A N).mulVec
              (allAlignedStateS Λ N (0 : Fin (N + 1))))).re ≤
      ((Fintype.card Λ : ℝ) * (N : ℝ) / 2) ^ 2 := by
  have h_eq := heisenbergToyHamiltonianS_two_allAligned_zero_expectation_re_eq_variational_gap_re
    (Λ := Λ) (A := A) (N := N)
