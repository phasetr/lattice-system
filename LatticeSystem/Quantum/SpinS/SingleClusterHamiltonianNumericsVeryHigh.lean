...
`singleClusterGSEnergyS` and `singleClusterMaxEnergyS` for `N ≥ 39`
(spin `S = N/2 ≥ 39/2`). The `N = 1..28` specialisations live in
`SingleClusterHamiltonianNumerics.lean` and the `N = 29..38`
specialisations in `SingleClusterHamiltonianNumericsHigh.lean`.

This file imports the main `SingleClusterHamiltonian` directly (not
the lower-N numerics files) so all three numerics files can elaborate
