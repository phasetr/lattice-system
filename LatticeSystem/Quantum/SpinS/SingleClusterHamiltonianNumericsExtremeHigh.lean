...
`60 ≤ N ≤ 77` (spin `30 ≤ S = N/2 ≤ 77/2`). The `N = 1..28`
...
from `ExtremeHigh` to `MaxHigh` at the `N = 78` boundary was the
50-PR build-performance cadence refactor #9, triggered when
`ExtremeHigh` reached ~10.5 s user CPU after N=60..98 entries had
been appended (see `.self-local/docs/refactoring-plan-2026-04-22.md`
§A).
-/
