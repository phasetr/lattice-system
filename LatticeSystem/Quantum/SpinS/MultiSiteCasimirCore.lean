...
/-!
# Multi-site Casimir: decomposition and two-site specialisation (foundation)

Foundational layer extracted from `MultiSiteCasimir.lean` for build speed.  This file builds
the general multi-site Casimir decomposition and its two-site specialisation (`Λ = Fin 2`).

The two-site spin-`S` Heisenberg eigenvalue formula from Casimir is kept in the capstone
module `MultiSiteCasimir.lean`.
-/
