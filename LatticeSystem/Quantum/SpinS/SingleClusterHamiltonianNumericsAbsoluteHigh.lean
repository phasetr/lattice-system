...
`149 ≤ N ≤ 165` (spin `S = N/2` with `149/2 ≤ S ≤ 165/2`). The
...
**Refactor #19 (split, γ-5 step 1385):** `AbsoluteHigh` reached
3213 lines / ~9.08 s user CPU after N=149..181 entries had been
appended (at the refactor #17 split threshold). Split off
`SingleClusterHamiltonianNumericsOmegaHigh.lean` to receive the
`N ≥ 166` entries; this file is now frozen at N=149..165.
-/
