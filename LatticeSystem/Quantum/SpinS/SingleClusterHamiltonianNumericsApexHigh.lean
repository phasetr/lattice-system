...
`N = 199..214` (spin `S = N/2 ∈ [199/2, 107]`). The `N = 1..28`
...
and the `N ≥ 215` in `SingleClusterHamiltonianNumericsZenithHigh.lean`.
...
PRs since refactor #23: PRs #2654..#2701). Kept as a single file.

**Refactor #25 (split, γ-5 step 1679 close):** `ApexHigh` reached
3133 lines / 8.50 s user CPU after N=199..230 entries had been
appended (32 spin rows; closes spin-115 row at z=6, N=230).
Approaching the refactor #21 split threshold (3213 lines / 9.08 s),
the file was split at the spin-115 row boundary into a frozen
`ApexHigh` (N=199..214, 16 spin rows) and a new
`SingleClusterHamiltonianNumericsZenithHigh.lean` (N=215+, active
growth target). 48 numerics PRs since refactor #24:
PRs #2703..#2750. Each half is ~1580 lines / well under the split
threshold, restoring parallel-elaboration headroom.
-/
