...
/-!
# Tasaki §2.5 Theorem 2.3 — the toy-Hamiltonian ground state is a joint Casimir eigenvector
...
unlike with the general Heisenberg Hamiltonian).  The result: the per-sector
Marshall-positive toy ground state is a simultaneous eigenvector of all three
Casimirs.  Pinning these eigenvalues to the predicted values is the remaining
...
/-- **The toy-Hamiltonian ground state is a simultaneous eigenvector of
`(Ŝ_tot)²`, `(Ŝ_A)²`, `(Ŝ_¬A)²`**: the per-sector Marshall-positive ground state
of `Ĥ_toy = heisenbergHamiltonianS (bipartiteCoupling A)` is a joint Casimir
eigenvector.  All three Casimirs commute with `Ĥ_toy` (it is their linear
